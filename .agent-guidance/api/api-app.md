...
| ------------------------------------------------------------ | ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/`, `/health/api`, `/health/liveness`, `/health/controller` | `apiHealth`, `apiLiveness`, `controllerHealth` | API-process and controller heartbeat health reporting; `/health/liveness` is the dependency-free process check, and authenticated callers receive the fuller diagnostic payloads |
...
| `apps/api/src/middleware/`          | architecture | documented | [API App](./api-app.md#runtime-and-middleware)         | Request observability, bearer-token auth middleware.                                                                              |
