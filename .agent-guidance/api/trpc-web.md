...
| **tasks**                | Task management and queries                                | `list`, `byId`, `messageEnvelopes`, `generateSummary`, `recentPullRequests`, `delete`, `updateTitle`, `search`, `pins`, `setPinned`                                                            |
| **artifacts**            | Task artifact access                                       | `byPath`, `versions`, `forTask`                                                                                                                                                                |
| **cloudJobs**            | Task launch and routing helpers                            | `routeHomeTask`, `createStandardTask` (web-created launches pass `queueEnabled: false`, accept an optional top-level `model`, and return the immediate `{ id, taskId }` task result), `cancel` |
...
| **backgroundAgents**     | Alias of the automations router for older callers          | `getSettings`, `listSlackChannels`, `updateSettings`, `triggerAgent`                                                                                                                           |
| **automations**          | Background-agent and reviewer automation settings          | `getSettings`, `listSlackChannels`, `updateSettings`, `triggerAgent`                                                                                                                           |
...
export async function deleteTasksCommand(
  ...
  return { success: true, deletedCount: result.deletedTasks.length };
}
```
...
- `setupNew.startOnboardingTask`
  ...
  - opens or reuses a Slack DM, posts the kickoff message, and starts a real `CloudTaskType.SlackAppMention` task with synthetic Slack event payload
  - persists an explicit automatic attribution override on that synthetic Slack launch so onboarding analytics, admin history, and task detail views treat the kickoff as platform-started setup work instead of an unlinked human Slack launch
...
- `taskSuggestions.list`
  ...
  - launches a hidden `CloudTaskType.SuggestedTasks` task when the org does not already have a batch or in-flight task for the current setup selection
  - reads `background_agent_settings.suggesterInstructions` and injects those preferences into the suggester prompt alongside the setup-specific guidance
  ...
  - reads the persisted batch from PostgreSQL `task_suggestions` after the task submits it through the task-scoped `submit_task_suggestions` MCP tool
...
describe('deleteTasksCommand', () => {
  it('deletes tasks and tracks deletion', async () => {
