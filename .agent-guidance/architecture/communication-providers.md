...
- When no active job exists, Telegram private messages and group messages that mention the configured bot start work. The handler routes the request with `buildTelegramRoutingContext()`, starts a `CloudTaskType.StandardTask` through `enqueueCloudTask()`, and stores provider-neutral Telegram metadata on the task payload.
