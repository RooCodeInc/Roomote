...
- `Settings > Integrations` shows deployment-level Teams bot readiness from `TEAMS_BOT_*` env vars, while `Settings > Personal` owns per-user Microsoft Teams account linking through `ROOMOTE_AUTH_MICROSOFT_*` and Better Auth's `microsoft-entra-id` provider.
