...
Zero is Path C (CLI in the runtime, users connect their own wallets). The remote MCP endpoint is `https://mcp.zero.xyz`, and Zero advertises protected-resource metadata with OAuth dynamic client registration plus PKCE on `https://api.zero.xyz`. Roomote treats Zero as a standard upstream OAuth proxy with `connectionScope: 'user'`: an admin enables Zero, then each user links their own Zero account so wallet spend stays user-owned. Roomote does **not** bake the Zero CLI into every sandbox image: `runTask` calls `mcpConnections.isOrgEnabled('zero')`, and only then installs the Zero CLI and materializes the hidden packaged `zero` skill. Prefer the CLI for capability discovery and paid calls; use the MCP connector for authentication and funding. Zero has no Roomote-managed read-only tool allowlist because the CLI is the primary path and wallet spend is gated by the user's Zero account plus per-call `--max-pay`. The catalog entry also injects agent-facing `instructions` when the MCP server is attached.
