...
- Microsoft Teams setup requires only the Microsoft sign-in fields (`ROOMOTE_AUTH_MICROSOFT_*`); the same app doubles as the Teams bot at runtime, so dedicated `TEAMS_BOT_*` values are optional dual-app overrides rather than a second setup write
