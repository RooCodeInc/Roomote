...
- For boolean setting cards in `apps/web`, match the neighboring settings and use the shared `Section` so the icon/title header is divided from the body. In the established explanatory-toggle pattern, place the `Switch` on the left before the description in a `flex gap-3` row rather than right-aligning it after the copy.
