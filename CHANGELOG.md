...
This release expands where and how Roomote agents work with Goal Mode, Box compute, remote MCP access, richer connected actions, and stronger task reliability.

### Highlights

- Keep long-running work moving toward a defined objective with bounded Goal Mode continuations.
- Run task environments on Box and diagnose environment health with the new Doctor workflow.
- Connect OAuth-capable coding clients to Roomote through one stable remote MCP URL.
- Route work with custom workspace rules and use new connected actions across communication channels and X.
