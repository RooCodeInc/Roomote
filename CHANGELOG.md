...
- Roomote 1.0 is the first public product release: the monorepo is open so people can try the product, self-host, and send feedback on web tasks, cloud agents, and chat integrations.
...
- Background automations work across communication surfaces and source-control providers: triage, audit, announcer, and manager-stats can report to Slack, Teams, or Telegram; open-PR listing, merged-PR facts, conflict resolution, and digests cover GitHub, GitLab, Gitea, Azure DevOps, and Bitbucket; the automations page shows destinations and only surfaces capability badges when coverage is limited.
- Environments support Docker Compose and Dockerfile projects (renamed from container projects): projects run on Modal VM sandboxes, stream logs into the task Logs panel, start without blocking eligible tasks, and receive higher sandbox memory when nested Docker is required.
- Local Docker can be toggled on or off from the Local Docker settings surface, and Experimental settings add a deployment-level Code Mode toggle.
- Onboarding environment setup runs inside the normal task flow with observable setup completion, setup logs in the Logs tab, non-blocking first-time hosted compute provisioning, hidden excluded sandbox providers, and completion messaging that no longer pushes users back to a separate /setup page.
