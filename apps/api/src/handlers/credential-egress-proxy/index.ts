...
import { authorizeProxy } from '@roomote/sdk/server/credential-egress';
...
/**
 * Credential egress substitution proxy: `/api/credential-egress/<upstream path>`.
 *
...
const LOG_PREFIX = '[credential-egress-proxy]';
...
type Allowed = Extract<CredentialEgressAuthorization, { allowed: true }>;
...
export function presentedSubstitute(
  ...
  return SUBSTITUTE_SHAPE.test(value) &&
    value.startsWith(CREDENTIAL_EGRESS_SUBSTITUTE_PREFIX)
    ? value
    : null;
}
...
interface CredentialEgressProxyOptions {
...
export function createCredentialEgressProxy(
  options: CredentialEgressProxyOptions = {},
) {
  const doFetch = options.fetch ?? undiciFetch;
  const mountPath = options.mountPath ?? CREDENTIAL_EGRESS_PROXY_PATH;
  const createAgent =
    options.createAgent ??
    (() =>
      new Agent({
  ...
  const denied = (
    ...
    return c.json({ error: 'credential_egress_denied' }, 403);
  };
  ...
  const rejected = (
    ...
    return c.json({ error: 'credential_egress_upstream_rejected' }, 502);
  };
  ...
  app.all('/*', async (c) => {
    ...
    if (!(CREDENTIAL_EGRESS_METHODS as readonly string[]).includes(method))
      return c.json({ error: 'method_not_allowed' }, 405);
    const egressMethod = method as CredentialEgressMethod;
  ...
  const proxy = async (
    ...
    method: string,
    egressMethod: CredentialEgressMethod,
    ...
    const [grant] = await db
      ...
      .where(
        eq(
          ...
          hashCredentialEgressSubstitute(substitute!),
        ),
      );
    if (!grant) {
...
export const credentialEgressProxy = createCredentialEgressProxy();
 ...
 */
export function credentialEgressProxyHostAlias(
  ...
  return async (c, next) => {
    ...
    if (
      ...
      url.pathname.startsWith(`${CREDENTIAL_EGRESS_PROXY_PATH}/`) ||
      url.pathname === CREDENTIAL_EGRESS_PROXY_PATH
    )
      return next();
    url.pathname = `${CREDENTIAL_EGRESS_PROXY_PATH}${url.pathname}`;
