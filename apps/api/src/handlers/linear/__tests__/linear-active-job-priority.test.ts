// pnpm --filter @roomote/api test src/handlers/linear/__tests__/linear-active-job-priority.test.ts
//
...
const { createLinearAgentJobMock } = vi.hoisted(() => ({
  createLinearAgentJobMock: vi
    .fn()
    .mockResolvedValue({ status: 'ok', jobId: 77, taskId: 'task-77' }),
}));
...
vi.mock('@roomote/linear', async (importOriginal) => {
  ...
  return {
    ...
    createLinearAgentJob: createLinearAgentJobMock,
...
describe('CLO-1133: active job takes priority over routing confirmation and elicitation', () => {
  let app: Hono;
  ...
  it('delivers free-text reply to active job even when routing confirmation key exists in Redis', async () => {
    setupDbMocks();
    ...
    // Active job exists (e.g. waiting on ask_followup_question)
    vi.mocked(findActiveLinearJob).mockResolvedValue({
      id: 42,
      machineId: 'machine-1',
      taskId: 'task-1',
    } as Awaited<ReturnType<typeof findActiveLinearJob>>);
    ...
    // The message MUST be queued to the active job
  ...
  it('queues request_user_input answers for active jobs instead of plain follow-up prompts', async () => {
    setupDbMocks();
  ...
  it('delivers free-text reply to active job even when a pending elicitation selection exists', async () => {
    setupDbMocks();
    ...
    // findPendingSelection should NOT have been checked (active job short-circuits)
    expect(findPendingSelection).not.toHaveBeenCalled();
  });

  it('starts a routed task when no active job exists', async () => {
    setupDbMocks();

    vi.mocked(findActiveLinearJob).mockResolvedValue(null);
    ...
    expect(createLinearAgentJob).toHaveBeenCalled();
    expect(queueLinearMessage).not.toHaveBeenCalled();
  });
});
