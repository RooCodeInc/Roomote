...
import type { McpConnectionNotionConfig, McpToolPolicy } from '@roomote/types';
import { isMcpToolAllowed } from '@roomote/types';
import { z } from 'zod';
...
function registerSearchTool(
  ...
  registerIfAllowed(server, policy, toolName, () => {
    server.registerTool(
      toolName,
      {
        title: 'Search Notion',
    ...
    server.registerTool(
      ...
      async ({ id, object_type: objectType, start_cursor, page_size }) => {
        ...
        if (objectType === 'data_source') {
          const dataSource = await notionApiRequestJson<
            Record<string, unknown>
          >({ config, path: `data_sources/${encodedId}` });
...
export function registerNotionTools(
  ...
  registerSearchTool(server, config, policy);
  registerFetchTool(server, config, policy);
  registerQueryDataSourceTool(server, config, policy);
  registerGetCommentsTool(server, config, policy);
  registerCreatePagesTool(server, config, policy);
  registerUpdatePageTool(server, config, policy);
  registerAppendBlocksTool(server, config, policy);
  registerCreateCommentTool(server, config, policy);
}
