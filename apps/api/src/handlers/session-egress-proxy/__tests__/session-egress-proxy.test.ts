...
const base = SESSION_EGRESS_PROXY_PATH;
...
async function auditRows(ref = secretRef) {
  return db.execute<{
    ...
    sql`select phase, decision, reason, destination, workload_id from session_egress_audit where secret_ref = ${ref} order by created_at, phase`,
  );
}
...
beforeEach(async () => {
  ...
  app.route(base, createSessionEgressProxy());
  userIds = [];
  sessionIds = [];
  taskIds = [];
  minted.length = 0;
...
afterEach(async () => {
  ...
  await db
    ...
    .where(
      or(
        inArray(sessionEgressAudit.workloadId, ownWorkloads),
        inArray(sessionEgressAudit.secretRef, ownSecrets),
      ),
    );
  await db
...
it.each([
  [
    'revoked grant',
    'grant_revoked',
    () => revokeSessionSecret(context, { secretRef }),
  ],
  [
    'finished run',
    'session_unavailable',
  ...
  [
    'terminated workload',
    'workload_inactive',
    () => terminateSessionEgressWorkload(workloadId, 'stopped'),
  ],
  [
    'detached run',
    'session_unavailable',
...
it('refuses redirects so the credential never follows a Location header', async () => {
  ...
  expect(await response.json()).toEqual({
    error: 'session_egress_upstream_rejected',
  });
