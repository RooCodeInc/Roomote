...
function buildSlackRequestUserInputTaskUrl(activeJob: ActiveSlackJob): string {
  const origin = Env.ROOMOTE_APP_URL;
  const payload =
    activeJob.payload && typeof activeJob.payload === 'object'
      ? (activeJob.payload as { webPath?: unknown })
      : null;
  const webPath =
    typeof payload?.webPath === 'string' && payload.webPath.startsWith('/')
      ...
      : null;
  const baseUrl =
    webPath || activeJob.taskId
      ? `${origin}${webPath ?? `/task/${activeJob.taskId}`}`
      : `${origin}/task`;
...
async function handlePendingRequestUserInputReply(params: {
  ...
  if (!parsedReply) {
    ...
    if (parsedCurrentQuestionReply.resolution === 'cancelled') {
      ...
      apiLogger.debug(
        `✅ Successfully cancelled request_user_input for active job ${activeJob.id}`,
      );
    ...
    if (nextQuestionIndex >= pendingRequest.questions.length) {
      ...
      apiLogger.debug(
        `✅ Successfully queued final request_user_input answer for active job ${activeJob.id}`,
      );
      deliveryTracker.track(event.ts);
      return 'handled';
    }
  ...
  apiLogger.debug(
    `✅ Successfully queued request_user_input answer for active job ${activeJob.id}`,
  );
  deliveryTracker.track(event.ts);
  return 'handled';
}

export async function processActiveJobMessage(
  event: SlackEvent,
  slack: SlackNotifier,
  userId: string,
  activeJob: ActiveSlackJob,
  ...
  try {
    ...
    const [promptReadyThreadMessages, normalizedMessageText, trackedBotReply] =
      await Promise.all([
        getPromptReadyThreadMessages({
          ...
          threadTs: threadId,
          botUserId,
          startedMessageJobId: activeJob.id,
    ...
    try {
      ...
      await clearLatestUserMessage(activeJob.id);
    } catch (error) {
    ...
    apiLogger.debug(
      `✅ Successfully queued message for active job ${activeJob.id}`,
    );
  } finally {
