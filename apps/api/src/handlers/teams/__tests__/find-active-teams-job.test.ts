// pnpm --filter @roomote/api test src/handlers/teams/__tests__/find-active-teams-job.test.ts
//
...
describe('buildTeamsJobMatchConditions', () => {
  it('normalizes a thread-scoped inbound conversation id to the bare channel form', () => {
    const { conversationMatch } = buildTeamsJobMatchConditions({
  ...
  it('matches the inbound thread id against the stored root activity ids unchanged', () => {
    const { threadMatch } = buildTeamsJobMatchConditions({
      conversationId: '19:conv@thread.tacv2;messageid=root-1',
      threadId: 'root-1',
    });
