 ...
 * snapshot-resumable completed) Teams task run for an inbound activity.
 *
 ...
 * sides keeps the follow-up associated with the original task run. Thread
 * disambiguation still relies on the root activity id (threadMatch).
 */
export function buildTeamsTaskRunMatchConditions(input: {
...
export async function findActiveTeamsTaskRun(input: {
  conversationId: string;
  threadId?: string;
}) {
...
export async function findCompletedTeamsTaskRunWithSnapshot(input: {
  conversationId: string;
  threadId?: string;
}) {
  const { teamsProviderMatch, conversationMatch, threadMatch } =
...
/**
 * Finds the most recent Teams task run for a conversation thread regardless
 ...
 * replies.
 */
export async function findLatestTeamsThreadTaskRun(input: {
  conversationId: string;
  threadId: string;
}) {
  const { teamsProviderMatch, conversationMatch, threadMatch } =
