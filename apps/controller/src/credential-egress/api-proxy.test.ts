...
import type { CredentialEgressWorkloadRegistration } from '@roomote/types';
...
const baseUrl = 'https://api.roomote.test/api/credential-egress';

function lifecycle(
  outcome: Awaited<ReturnType<CredentialEgressLifecycle['register']>> = {
    status: 'registered',
  ...
  } as unknown as CredentialEgressLifecycle & {
...
const input = (cycle: CredentialEgressLifecycle) => ({
  lifecycle: cycle,
...
describe('API-proxy admission', () => {
  it('builds substitute-only client configuration with one base URL and no proxy settings', () => {
    const env = buildCredentialEgressApiProxyWorkerEnv({
      registration,
      baseUrl,
    });
    expect(env).toEqual({
      ROOMOTE_SERVICE_BASE_URL: baseUrl,
      ROOMOTE_CREDENTIAL_EGRESS_SERVICES: JSON.stringify([
        {
  ...
  it('derives the base URL from the API origin or the dedicated proxy host', () => {
    ...
    expect(
      resolveCredentialEgressApiProxyBaseUrl({
        TRPC_URL: 'https://api.roomote.test',
        R_CREDENTIAL_EGRESS_PROXY_HOST: 'egress.roomote.test',
      }),
  ...
  it('waits for bootstrap, registers, publishes bound to the nonce, then renews', async () => {
    ...
    await expect(admitCredentialEgressApiProxy(input(cycle), d)).resolves.toBe(
      registration,
    );
  ...
  it('never registers when the run stops or bootstrap never completes', async () => {
    ...
    await expect(
      admitCredentialEgressApiProxy(input(silent), d),
