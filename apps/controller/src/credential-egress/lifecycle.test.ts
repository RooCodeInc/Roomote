...
import {
  CredentialEgressLifecycle,
  type CredentialEgressLifecycleDependencies,
} from './lifecycle';

const workloadId = '11111111-1111-4111-8111-111111111111';
const sessionId = '22222222-2222-4222-8222-222222222222';
const registration: CredentialEgressWorkloadRegistration = {
  workloadId,
  sessionId,
  generation: 1,
...
function dependencies(): CredentialEgressLifecycleDependencies {
  return {
    apiProxyBaseUrl: 'https://api.roomote.test/api/credential-egress',
    client: {
...
describe('controller-owned Credential egress lifecycle', () => {
  ...
  it('does not mint a lease or substitute during normal bootstrap preflight', async () => {
    const deps = dependencies();
    expect(
      await new CredentialEgressLifecycle(deps).needsBootstrapAdmission(
        1,
        'docker',
      ),
    ).toBe(true);
  ...
  it.each(['docker', 'daytona', 'e2b', 'blaxel', 'box', 'azure'] as const)(
    ...
    async (provider) => {
      const deps = dependencies();
      const result = await new CredentialEgressLifecycle(deps).register({
  ...
  it.each([
    ...
    async (provider) => {
      ...
      expect(plan.bootstrapEnv).toEqual({
        ...
        ROOMOTE_CREDENTIAL_EGRESS_BOOTSTRAP_NONCE:
      ...
      expect(admitApiProxy).toHaveBeenCalledWith({
        ...
        taskRun: { id: 1, taskId: 'task1' },
        provider,
        nonce: plan.bootstrapEnv.ROOMOTE_CREDENTIAL_EGRESS_BOOTSTRAP_NONCE,
        baseUrl: 'https://api.roomote.test/api/credential-egress',
        resume: false,
      });
      expect(
  ...
  it('lets a launcher name the proxy address its sandbox can reach', async () => {
    const admitApiProxy = vi.fn().mockResolvedValue(registration);
    const lifecycle = new CredentialEgressLifecycle({
      ...dependencies(),
      admitApiProxy,
    });
    const plan = await lifecycle.planApiProxy({
      ...
      provider: 'docker',
      baseUrl: 'http://api:3001/api/credential-egress',
    });
    await plan.admit();
    expect(admitApiProxy).toHaveBeenCalledWith(
