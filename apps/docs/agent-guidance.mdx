...
icon: scroll-text
description: Add deployment-wide instructions that Roomote should consider across sessions and tasks.
---
...
After you save changes, send a new message in a session or start a small
new task and check whether the guidance shows up in the behavior you expected.
Roomote reloads the guidance for each turn, so saved changes apply to its next
