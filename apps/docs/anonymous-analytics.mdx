...
- **Usage events** — page views (as route patterns like `/task/[taskId]`,
  ...
  provider used. Fast session events include bounded outcome, retry count, and
  phase-duration measurements, plus whether a turn is the session's first human
  message, but not conversation or message identifiers.
