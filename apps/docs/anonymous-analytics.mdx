...
- **Usage events** — page views (as route patterns like `/task/[taskId]`,
  ...
  whether it was configured before the wizard. Fast mode turn events include
  only terminal outcome, aggregate phase timings, retry and tool
  classifications, model provider, source surface, and release metadata.
  Other events include non-identifying facts like the harness, model, source
  surface, and sandbox provider used.
