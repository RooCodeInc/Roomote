...
the report links back to that session in the web app. The Session transcript
shows the automation's configured prompt, and its normal composer lets a signed-in
teammate continue the run from the dashboard even when the automation owns the
Session. Replies on all four
