...
Email destinations use one exact account email whose verification status is
explicitly recorded by Roomote. Sender addresses shown as **Linked** under
Personal settings are not eligible: that flow proves mailbox possession for
starting work by email but does not mark another account email as verified.
The current account model records verification for one account email. If that
