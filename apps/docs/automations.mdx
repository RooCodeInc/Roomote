...
normal Session. The item is marked accepted after the Session starts. When the
source task has one Roomote-created deliverable pull request, merging that pull
request also marks the item accepted automatically. Use **Clear** when no
follow-up is needed, or **Clear all** to dismiss the current queue.
...
**Platform Issue Alerts** are enabled by default. Tasks and Fast Sessions use
