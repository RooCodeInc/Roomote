...
An admin can enable **Results** deployment-wide under **Settings > Experimental**
to collect durable
...
default destination, does not post Slack digests, does not implement the fix,
and does not open a pull request automatically. Azure DevOps work items and
...
or **DM me**. Choose **Default** to use the deployment's default destination.
Roomote reacts to provider-deduplicated push
...
The **Default destination** at the top of the page receives reports from
automations that do not have an explicit destination. An admin can choose a
Slack or Discord channel or direct message, or an Email recipient. A destination
saved on an individual automation takes precedence, and existing custom
automations keep the destination selected when they were created.
...
The manager section controls the deployment's default destination plus
recurring manager-facing updates and suggestions.

| Automation                   | What it posts                                                    | Typical cadence                             |
| ---------------------------- | ---------------------------------------------------------------- | ------------------------------------------- |
| **Default destination**      | The fallback destination for automations without their own       | Configure once                              |
...
Set **Default destination** first. Choose a Slack or Discord channel or direct
message, or an Email recipient, for manager-facing posts, suggestions,
summaries, and setup alerts. Make sure Roomote can reach the destination before
you save it.

For Slack, the first account connection after app installation automatically
sets up public `#roomote-managers` if no default destination is configured.
Existing automation destinations are unchanged, and no automations
are enabled by channel setup. See [Slack setup](/providers/communications/slack#automatic-default-destination)
...
100%. The automation-specific destination wins, followed by the default
destination and then the supported primary Teams, Telegram, or Discord
conversation fallback. The check is deterministic, supports **Run now**, and
never starts a Roomote task or session.
...
Slack, Discord, or Email destination wins, followed by the default destination.
If neither destination is configured, Roomote sends the alert directly to active
...
such as `1.10.5` to `1.11.2`. Roomote announces the matching stable major or
minor release, using its authored summary and complete highlight list, and
links to that release's full notes. If the selected release has neither an
authored summary nor highlights, Roomote silently completes the announcement
instead of retrying it; missing or unreadable release data remains retryable.
Leave its destination on **Default** to use the standard automation destination,
or choose a Slack, Microsoft Teams, Telegram, or Discord channel or direct
message, or an Email identity. **Run now** sends a sample from the latest stable
major or minor release notes without changing the automatic announcement
baseline.
...
explicit destination when one is set, otherwise to the deployment's default
destination.
