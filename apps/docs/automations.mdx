...
| **Provider Usage Limits**    | Warnings when an inference-provider quota approaches exhaustion | Hourly                                        |
...
use the normal Teams, Telegram, or Discord primary-conversation fallback), and
set the warning threshold from 5% through 95% in 5% increments. Roomote checks
hourly, posts one consolidated automation result card when a provider crosses
the threshold in a quota cycle, and sends a fresh critical alert if the quota
later reaches 100%. The check is deterministic and never starts a Roomote task
or session.
