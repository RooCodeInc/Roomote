...
| **Triage GitHub Issues**   | Posts an implementation plan when a GitHub issue is opened/reopened | Get a grounded plan on the issue without auto-opening a PR           |
...
For **Triage GitHub Issues**, turn the toggle on once GitHub is connected and
the repos you care about have configured Roomote environments. When an issue is
opened or reopened, Roomote starts a task immediately, investigates the issue,
and posts a concrete plan as a comment on that GitHub issue. It does not need a
Manager Channel, does not post Slack digests, does not implement the fix, and
does not open a pull request automatically.
...
**Suggest Ideas** can also report to Telegram as a first-class destination.
When you select Telegram, Roomote creates a sticky **Suggest Ideas** forum
topic in your primary Telegram chat (create once, reuse on later runs) and
posts digests there. You cannot pick an existing Telegram thread — Roomote
owns the recurring topic. Other automations treat Telegram as a fallback
surface when no Slack destination is available.
...
GitLab, and Azure DevOps. CI Failure Triage reacts immediately to GitHub
webhooks rather than running on a daily or weekly schedule.
