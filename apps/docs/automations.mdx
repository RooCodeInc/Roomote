...
DevOps builds, and Bitbucket Pipelines. Triage Issues supports GitHub, GitLab,
and Gitea issues.
...
Actions, GitLab Pipeline, Azure DevOps `build.complete`, and Bitbucket
Pipelines commit-status webhooks rather than running on a daily or weekly
schedule.
...
builds, and Bitbucket Pipelines). When a failure persists (it is not already fixed by a newer run and is
