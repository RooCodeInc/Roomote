...
- optional **launch criteria** in plain language, such as “only investigate a
  new regression affecting active users.” The session first gathers current
  evidence with its normal read-only integrations, then Jev checks the findings,
  recent automation results, and bounded tool results against the criteria
...
Launch criteria are checked before delegated work or a destination reply starts.
A confident decision that the criteria are not met ends that occurrence quietly;
no task or destination root is created. Uncertain or unavailable judgments
continue the run. More detailed typed checks can be supplied through the
automation management API; their `onUncertain` setting can explicitly skip an
ambiguous run.
