...
Select a result to read its full report. Reports link to the source task or
session and, when available, its pull request, plan, or artifact. Suggested work
offers **Start investigation**, where you can edit the prepared prompt and
choose a model before starting a normal session. The item is accepted only
after that session starts. Use **Clear** when no follow-up is needed, or **Clear
all** to dismiss the current queue.
