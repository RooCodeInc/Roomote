...
nature. CI Failure Triage supports GitHub Actions, GitLab Pipelines, and
Azure DevOps builds. Triage Issues supports GitHub, GitLab, and Gitea issues.
...
Actions, GitLab Pipeline, and Azure DevOps `build.complete` webhooks rather
than running on a daily or weekly schedule.
...
branch (GitHub Actions workflow runs, GitLab Pipelines, and Azure DevOps
builds). When a failure persists (it is not already fixed by a newer run and is
...
`build.complete` service hook when Azure DevOps repositories are synced.
