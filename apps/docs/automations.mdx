...
hourly, posts an alert card for each provider that crosses the threshold in a
quota cycle, and sends a fresh critical alert if the quota later reaches 100%.
The check is deterministic and never starts a Roomote task or session.
