...
**Provider Usage Limits** do not create a task or session, so their cards are
...
report destination. Runs scoped to **All repositories** pin each suggestion to a
concrete repository, then start it in a matching named environment when one is
available. Those runs prepare every active repository, so they can take longer
in large deployments. Use a named environment when the automation only needs a
specific repository set.
...
connected ChatGPT subscription when configured.
...
| **Provider Usage Limits**    | Warnings when an inference-provider quota approaches exhaustion | Every 15 minutes, hourly, or daily           |
...
**Provider Usage Limits** is enabled at an hourly cadence with an 85% threshold
when it first appears, preserving the provider warning behavior from earlier
Roomote releases. Admins can disable it, choose a Slack or Discord channel (or
use the normal Teams, Telegram, or Discord primary-conversation fallback), change the
maximum alert cadence to every 15 minutes, hourly, or daily, and set the warning
threshold from 50% through 95% in 5% increments. Roomote posts one consolidated
automation result card per due check, deduplicates a threshold crossing within
the provider's quota period, and sends a fresh critical alert if the quota later
reaches 100%. The check is deterministic and never starts a Roomote task or
session.
...
uses only versions those policies allow. When every repository is scanned
