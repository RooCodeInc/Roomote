...
nature. CI Failure Triage supports GitHub Actions and GitLab Pipelines.
Triage Issues supports GitHub, GitLab, and Gitea issues.
...
Actions and GitLab Pipeline webhooks rather than running on a daily or weekly
schedule.
...
branch (GitHub Actions workflow runs and GitLab Pipelines). When a failure
persists (it is not already fixed by a newer run and is not a one-off flake),
Roomote starts a fix task that reproduces the failing job inside the
repository's configured environment, finds the root cause, opens a PR with the
fix, and posts one summary to the Manager Channel when it finishes. Only
repositories that belong to a configured Roomote environment are triaged. For
GitLab, ensure Pipeline Hooks are enabled on the webhook Roomote receives.
