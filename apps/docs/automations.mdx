...
For **Merge announcer**, connect a source-control provider, choose a report destination or
configure the shared Manager Channel, and turn the toggle on. Roomote reacts to
provider-deduplicated push webhooks for each active repository's current default branch,
uses the deployment helper model to write a brief commit summary, and includes
the pusher and commit authors. Feature-branch pushes and branch deletions are
ignored. GitHub, GitLab, Azure DevOps, Bitbucket, and Gitea are supported.
Reports can use Slack, Microsoft Teams, Telegram, or Discord through
the normal automation destination fallback.
