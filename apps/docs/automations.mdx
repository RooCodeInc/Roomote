...
- optional **launch criteria** in plain language, such as “only investigate a
  new regression affecting active users.” This is an off-by-default experiment:
  a deployment admin can enable **Custom automation launch criteria** under
  **Settings → Experimental**. The session then gathers current evidence with
  its normal read-only integrations, and Jev checks the findings, recent
  automation results, and bounded tool results against the criteria
...
When the experiment is enabled, launch criteria are checked before delegated
work or a destination reply starts. A confident decision that the criteria are
not met ends that occurrence quietly; no task or destination root is created.
Uncertain or unavailable judgments continue the run. More detailed typed checks
can be supplied through the automation management API or MCP tool; their
`onUncertain` setting can explicitly skip an ambiguous run. This experiment
applies only to custom automations; channel auto-start launch criteria remain
