...
for each run. A Fast run with a Slack direct-message destination posts to the
automation owner's DM and requires that owner to link their Slack account and
make the bot reachable there. Fast runs without a destination are stored as
channel-less Fast conversations; their output will be available in the upcoming
Fast runs view. Until that view ships, use a supported Slack or Discord
destination when the result must be visible outside the automation's latest-run
status.
