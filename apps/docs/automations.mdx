...
| **Triage GitHub Issues**   | Immediate plan comment when a GitHub issue is opened or reopened | Immediate via webhook                       |
...
with. Triage Dependabot Alerts, Triage CodeQL Alerts, Triage GitHub Issues, and CI Failure Triage are
...
GitLab, and Azure DevOps. Triage GitHub Issues and CI Failure Triage react immediately
to GitHub webhooks rather than running on a daily or weekly schedule.
...
**Triage GitHub Issues** runs the moment a GitHub issue is opened or reopened on a repository covered by a configured Roomote environment. It investigates the issue and posts a concrete implementation plan as a comment on that issue. It does not implement the fix or open a pull request automatically.
