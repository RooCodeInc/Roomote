...
In **Settings > Automations > CI Failure Triage**, choose **All repositories**
to retain the shared destination, or **Selected repository groups** to limit
triage to specific repositories. Add a group, select its repositories, and
choose its own Slack, Discord, Teams, or Telegram destination. Each repository
can belong to only one group. Provider and host labels distinguish repositories
with the same name on different source-control instances.

Selected groups apply to both webhook failures and **Run now**. Removing all
groups selects no repositories; it does not restore **All repositories**.
...
repository's group destination. CI Failure Triage is webhook-driven; scheduled
checks do not scan repositories.
