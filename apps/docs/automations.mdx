...
| **Issue Fixer**              | Immediate fix task when a GitHub issue is opened or reopened      | Immediate via webhook                       |
...
with. Triage Dependabot Alerts, Triage CodeQL Alerts, Issue Fixer, and CI Failure Triage are
...
GitLab, and Azure DevOps. Issue Fixer and CI Failure Triage react immediately
...
**Issue Fixer** starts a focused implement-changes task the moment a GitHub
issue is opened or reopened on a repository covered by a configured Roomote
environment. It opens a PR that references the issue when the fix is clear and
high-confidence, rather than waiting for a scheduled batch scan.
