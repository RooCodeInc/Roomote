...
run when it receives an HTTP `POST`. The request body is ignored; the automation
uses its saved prompt. Keep the URL private like a password. Disabling webhooks,
rotating the URL, or disabling the automation revokes it. Webhook runs require
an enabled automation with an active owner and use the same launch limits as
manual runs.
