...
the report links back to that session in the web app. Slack and Discord replies
continue the Fast session directly in chat. Teams and Telegram can deliver the
same reports and continue them from the web app, but inbound chat replies do
not resume Fast yet because those webhook paths do not currently carry the
Fast session identity; they continue through the provider's normal task-routing
flow instead. Runs with no report destination remain stored conversations and
do not post to chat.
