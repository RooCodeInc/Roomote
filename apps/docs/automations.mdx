...
environment or across all active repositories. A run can skip or fail before
task creation when its configuration or launch state prevents it from starting.
...
| **Alert on Config Errors**   | Configuration issues reported by Roomote tasks                   | Immediate when reported                     |
...
**Alert on Config Errors** is enabled by default and can be disabled from its
automation card. Alerts use the automation's own channel first, then the shared
Manager Channel. If neither is configured, Roomote direct-messages deployment
admins who have a linked account on a connected communications provider.
