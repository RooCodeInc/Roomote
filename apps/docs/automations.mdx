...
and either executes the prompt there in **Fast** without starting a sandbox, or
launches a normal task in the selected environment (or across all active
repositories) as the Session's delegated work. A Fast run can still delegate a
normal task when repository or workspace execution is required. Either way the
Session owns the run: the task reports back to it, and the Session is what
talks to the report destination once the work settles.
...
automation's configured prompt and, for environment runs, the delegated task
card, and its normal composer lets a signed-in teammate continue the run from
the dashboard. Replies on all four providers continue the Session directly in
chat. Roomote verifies the active provider installation, conversation, and
linked user before resuming. Runs with no report destination remain stored
