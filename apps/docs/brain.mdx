...
"nothing collected yet" from "nothing to collect from". **Backfilling** means
that historical sweep is actively running; rolling checkpoints and ongoing
source streams remain active ingestion instead of appearing to backfill
forever. Slack and GitHub automatically resume their history sweeps when a new
public channel or repository becomes available after the previous sweep
finished.
