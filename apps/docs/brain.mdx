...
- **people identities**, projected from Roomote accounts, linked provider
  ...
  across sources even when they do not have Roomote accounts
...
- **Back up the Brain volume.** Its `/data/brain` directory is the Markdown
  corpus, including pages produced by nightly synthesis. Postgres holds the
  searchable index, extracted facts, and durable maintenance jobs; keeping it
  in the normal database backup makes restores faster, but it is not the only
  copy of the source pages.
