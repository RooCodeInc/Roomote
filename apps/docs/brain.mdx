...
- **pull requests** from your connected source-control provider, including
  their descriptions and labels. Very long descriptions are kept as an excerpt
  with a link back to the full pull request
...
recorded, how many are waiting, and how many failed. Its actions are safe repair
tools for each queue state.
**Ingest task history** enqueues memories that predate the Brain's one-time
backfill. If a task completed after that checkpoint without being queued, the
page reports it separately as a recording gap and offers to queue the missing
memory. **Retry failed** hands memories that exhausted their attempts back to
the queue with a fresh budget, which is the fix after an outage. All three
actions are safe to run twice: memories are filed under a slug derived from the
run, so a repeat rewrites the same page rather than adding another.
