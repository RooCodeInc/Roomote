...
summary to the default destination. The team finds a green PR waiting instead of a
red branch. This is a built-in automation, so the recipe is one toggle plus
clear instructions.

- **Trigger**: Webhook
- **Setup time**: About 10 minutes
- **Requires**: Admin access, source control, default destination
- **Serves**: Engineers
...
- A destination configured under **Default destination**
- An environment for every repository you want triaged. Repositories outside an environment are skipped.

## Steps

1. Set the destination under **Settings > Automations > Default destination**.
...
- Route the summary to a dedicated `#ci-triage` channel instead of the default destination.
