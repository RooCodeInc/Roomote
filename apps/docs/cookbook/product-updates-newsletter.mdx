...
- The Resend MCP server configured on the **Integrations** page, with a Resend API key
...
and integration are configured. Admins manage integration setup, deployment
settings (including the shared scheduling timezone), and all custom automations,
including those without a creator.
