...
- **Requires**: Member access, a configured environment
...
- One [custom automation](/automations#custom-automations) per chore
- One environment per automation
- A report destination such as `#eng-maintenance`

Members can create and manage their own custom automations. Admins can manage
all custom automations, including those without a creator. Built-in automations
and the deployment's shared scheduling timezone remain admin-only.

## Steps

1. Under **Settings > Automations**, select **New** to create a custom automation.
