...
across tasks, sessions, and automations. It reports cost in US dollars and
...
- how much judgment-model routing and review pre-screening contribute to usage
...
Judgment-model calls appear as non-task inference under the `judgment_model`
source. Primary decisions and optional shadow evaluations are separate requests,
so both contribute when shadow mode is enabled. Roomote records provider-reported
tokens and cost when available; missing metadata leaves those values at zero
without changing the decision or its fallback behavior.
