...
| `R_JUDGMENT_MODEL`             | Optional          | Selects the [judgment model](/models#judgment-model): `typesafe`, `vercel` (Jev through Vercel AI Gateway, using `AI_GATEWAY_API_KEY`), or `off`. Overrides the choice in **Settings > Models**. |
