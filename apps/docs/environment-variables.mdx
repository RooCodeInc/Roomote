...
| `R_TYPESAFE_API_KEY`           | Optional          | TypeSafe key for the optional [judgment model](/models#optional-judgment-model). Bounded routing decisions try one fast typed call first and fall back to the helper model when it is unsure or fails. The key stays on the control plane. |
