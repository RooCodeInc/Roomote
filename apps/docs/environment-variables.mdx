...
| `R_JUDGMENT_UPSTREAM_URL`      | Optional          | Base URL of a Roomote-run [judgment model](/models#judgment-model) that answers the same typed decisions request as Jev at `/v1/decisions`. Used only when the Roomote judgment model is selected in **Settings > Models** or with `R_JUDGMENT_MODEL=roomote`. |
