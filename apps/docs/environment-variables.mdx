...
For a custom multi-service deployment, provide the same `JOB_AUTH_PRIVATE_KEY`
and `JOB_AUTH_PUBLIC_KEY` values to the web, API, controller, and BullMQ
services. The preview proxy needs the public key. BullMQ runs session turns that
mint MCP and other task-scoped tokens, so omitting either job-auth key there can
make connected integrations unavailable even when the other services start.
...
| `R_JUDGMENT_UPSTREAM_URL`      | Optional          | Base URL of a Roomote-run [judgment model](/models#judgment-model) that answers the same typed decisions request as Jev at `/v1/decisions`. Select the Roomote judgment model in **Settings > Models** or set `R_JUDGMENT_MODEL=roomote` to route decisions to this endpoint. With `R_JUDGMENT_SHADOW=on` and Jev selected, it also receives decision state and questions for comparison. |
...
| `R_JUDGMENT_SHADOW`            | Optional          | `on` compares each Jev judgment with the Roomote-run upstream and logs per-question agreement for calibration. Select Jev as the judgment route to collect comparison data. |
