...
| `R_VOICE_OPENAI_API_KEY`       | Optional          | OpenAI key dedicated to the live voice conversation feature (realtime transcription and spoken replies). Falls back to `OPENAI_API_KEY` when unset. The key stays on the control plane; browsers receive only short-lived transcription-scoped tokens and synthesized audio. |
