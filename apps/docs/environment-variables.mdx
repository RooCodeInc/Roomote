...
| Env var                              | Required          | Used for                                                                                                                                 |
| ------------------------------------ | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `DEFAULT_COMPUTE_PROVIDER`           | Optional          | Runtime default sandbox provider when no admin default is saved. Supported values are `roomote-cloud`, `docker`, `modal`, `e2b`, `daytona`, and `blaxel`. |
| `EXCLUDED_COMPUTE_PROVIDERS`         | Optional          | Comma-separated provider IDs to hide or exclude from default selection.                                                                  |
| `ROOMOTE_CLOUD_URL`                  | Roomote Cloud     | Roomote Cloud control-plane URL. Set with `ROOMOTE_CLOUD_DEPLOYMENT_TOKEN`.                                                              |
| `ROOMOTE_CLOUD_DEPLOYMENT_TOKEN`     | Roomote Cloud     | Deployment-scoped credential used for task admission and managed sandbox lifecycle.                                                     |
