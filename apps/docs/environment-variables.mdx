...
| `R_TRIAL_OPENROUTER_API_KEY`           | Optional     | Fallback OpenRouter key a hosting platform can inject for free-trial credits. Used only while no `OPENROUTER_API_KEY` is configured (environment or saved), so connecting your own key always wins. |
