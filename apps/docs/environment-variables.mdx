...
| `R_SESSION_EGRESS_ALLOWED_ORIGINS` | Required for Session egress, API only | JSON array of exact HTTPS origins permitted through the Iron-based credential-substituting gateway, such as `["https://api.example.com","https://uploads.example.com:8443"]`. Empty or unset denies all destinations. Scheme, hostname, and effective port must match exactly; wildcards, URL paths, credentials, queries, fragments, IP literals, and private-address resolution are rejected. Each entry only enables a separately owner-approved credential bound to that same origin. This does not restrict unrelated outbound networking. |
