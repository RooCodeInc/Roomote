...
For Compose deployments, add the DSNs to the deployment environment file and
recreate the affected services. `SENTRY_DSN` configures the API, controller,
BullMQ, and task workers unless a matching service-specific DSN overrides it.
The web service uses `NEXT_PUBLIC_SENTRY_DSN` separately.
