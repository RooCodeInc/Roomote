...
| `R_WEB_SHUTDOWN_DRAIN_MS` | Optional           | Same window for Fast turns owned by the web process. Defaults to `R_API_SHUTDOWN_DRAIN_MS`, then `20000`; values that exceed the available pre-Next handoff window are capped so unfinished turns are handed to the durable queue before Next.js exits. |
| `R_WEB_SHUTDOWN_HARD_TIMEOUT_MS` | Optional    | Total web shutdown deadline, including the Fast-turn drain and Next.js server cleanup. Defaults to `28000`; values below `6000` fall back to that default so shutdown always retains a 1-second Fast handoff margin and the final 5 seconds for Next.js cleanup before a forced exit. |
