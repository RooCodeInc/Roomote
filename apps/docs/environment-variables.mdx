...
| `R_API_SHUTDOWN_DRAIN_MS` | Optional           | Milliseconds the API allows in-flight Fast turns to finish during shutdown before aborting the remainder. Defaults to `20000`; set to `0` to restore immediate aborts. Keep it below the hosting platform's SIGTERM-to-SIGKILL grace period.                                                                                                                                                                                              |
| `R_FAST_DURABLE_ADMISSION_DISABLED` | Optional   | Set to `true` to disable durable admission and recovery for new Fast turns. This also disables durable inference-retry scheduling. Use only as a temporary rollback switch.                                                                                                                                                                                                                                                             |
| `R_FAST_DURABLE_RETRY_DISABLED` | Optional       | Set to `true` to keep inference retry waits in the current process instead of parking replay-safe Fast turns durably. Durable admission remains enabled.                                                                                                                                                                                                                                                                                 |
...
| `R_SANDBOX_OPENROUTER_API_KEY`         | Preview inference | Dedicated spend-capped OpenRouter key served only through the parent inference gateway to Roomote deployments running in environment-task sandboxes. Set it on the parent deployment, never in an environment definition. |
