...
| `R_AGENTMAIL_KEY_SCOPE`        | Optional          | `inbox` when the AgentMail API key is inbox-scoped, so the webhook is managed on the inbox. Detected and saved by the settings UI; set it only for env-var-only setups. |
