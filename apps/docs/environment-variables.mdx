...
| `R_WEB_SHUTDOWN_DRAIN_MS` | Optional           | Same window for the web service, which executes the Fast turns submitted from the web UI. Defaults to `R_API_SHUTDOWN_DRAIN_MS`, then `20000`. The hosting platform must give the web process a SIGTERM grace period longer than this window; Railway's default is 0 seconds. |
