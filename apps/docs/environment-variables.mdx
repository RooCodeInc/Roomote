...
| `ROOMOTE_APP_TMPFS_SIZE`  | Self-host Compose | Size of the temporary filesystem used by the web, API, controller, and bullmq services for session runtime files and caches. Defaults to `512m`; changing it requires recreating the affected containers.                                                                                                                                                                                                                                |
| `R_API_SHUTDOWN_DRAIN_MS` | Optional           | Milliseconds the API allows in-flight session turns to finish during shutdown before aborting the remainder so they resume on the next process. Defaults to `20000`; set to `0` to abort immediately. Keep it below the hosting platform's SIGTERM-to-SIGKILL grace period. Also the fallback for `R_BULLMQ_SHUTDOWN_DRAIN_MS`. |
| `R_BULLMQ_SHUTDOWN_DRAIN_MS` | Optional        | Same window for the bullmq service, which executes the session turns the queue resumes. Defaults to `R_API_SHUTDOWN_DRAIN_MS`, then `20000`. |
| `R_FAST_DURABLE_RETRY_DISABLED` | Optional       | Set to `true` to keep inference retry waits in the current process instead of parking in-flight session turns durably. Durable admission remains enabled.                                                                                                                                                                                                                                                                              |
...
These opt-in settings configure [HTTP integrations](/integrations/http-integrations)
for sessions and sandbox agents. They are independent of the curated integration
catalog's enablement policy.

| Env var | Required | Used for |
| --- | --- | --- |
| `R_HTTP_INTEGRATIONS_ENABLED` | Optional | Enables operator-manifest integrations. Defaults to `false`. The shared API broker remains available for owner-approved integration keys without a manifest. Set consistently on API, web, and background control-plane services. |
...
| `R_CREDENTIAL_EGRESS_PROXY_HOST` | Optional, API and controller | Dedicated hostname, for example `egress.example.com`, at whose root the API serves the credential egress proxy so SDK clients that allow only a host override can use it. Point the name at the API service and set the same value on the controller, which delivers it to sandboxes as their base URL. The `/api/credential-egress` path on the API host keeps working either way. |
...
| `R_ORCHESTRATION_MODEL`                | Optional     | Model for session answers and coordination of delegated task launches. Defaults to `R_MODEL`.                                           |
...
| `R_ELEVENLABS_VOICE_ID`        | Optional          | ElevenLabs voice ID used for feature-demo narration. Required alongside the API key for narration to be available.                                                       || `R_VOICE_OPENAI_API_KEY`       | Optional          | OpenAI key dedicated to GPT-Live voice conversations. Requires project access to `gpt-live-1`; the general `OPENAI_API_KEY` is not used for Voice. The key stays on the control plane; browsers receive only a server-negotiated WebRTC session answer. |
| `R_TYPESAFE_API_KEY`           | Optional          | TypeSafe key for the optional [judgment model](/models#judgment-model). A key alone turns on Jev via TypeSafe unless `R_JUDGMENT_MODEL` or Settings chooses otherwise. The key stays on the control plane. |
| `R_JUDGMENT_MODEL`             | Optional          | Selects the [judgment model](/models#judgment-model): `roomote` (using `R_JUDGMENT_UPSTREAM_URL`), `typesafe`, `openrouter` (using `OPENROUTER_API_KEY`), `vercel` (using `AI_GATEWAY_API_KEY`), or `off`. Overrides the choice in **Settings > Models**. |
| `R_JUDGMENT_UPSTREAM_URL`      | Optional          | Base URL of a Roomote-run [judgment model](/models#judgment-model) that answers the same typed decisions request as Jev at `/v1/decisions`. Set automatically on Roomote Cloud. Used by default when no TypeSafe key is configured and no other judgment model is selected. |
| `R_JUDGMENT_UPSTREAM_API_KEY`  | Optional          | Bearer credential for `R_JUDGMENT_UPSTREAM_URL`. Leave unset for a private-network upstream without auth. The credential stays on the control plane. |
| `R_JUDGMENT_SHADOW`            | Optional          | `on` also scores every Jev judgment with the Roomote-run upstream and logs how the two agree, without changing the answer Roomote acts on. Calibration evidence for the hosted model; only affects deployments that already send decision text to Jev. |
| `R_JUDGMENT_CAPTURE`           | Optional          | `on` writes every answered [judgment](/models#judgment-model) decision, with its state scrubbed of credential shapes and structured personal data, to the deployment's artifact bucket under `judgment-capture/`, for building a training set. Keeps decision text, so only for deployments you own. |
| `R_FAST_BROWSER_PROVIDER`      | Optional          | Enables the Fast `browse` tool, which gives every Session a private browser without a sandbox. Set to `browseruse` for Browser Use cloud browsers (requires `R_BROWSER_USE_API_KEY`) or `cdp` to attach to a Chrome DevTools endpoint you run, such as the optional `browserless` service in the Compose stack (requires `R_FAST_BROWSER_CDP_URL`). Unset disables the tool. |
...
| `R_FAST_BROWSER_CDP_URL`       | Optional          | `ws://` or `wss://` DevTools URL for `R_FAST_BROWSER_PROVIDER=cdp`, including the endpoint's token (`ws://browserless:3000/?token=...`). Use a service that starts a fresh browser per connection so Sessions never share cookies. The Compose stacks ship one behind the `browser` profile. |
...
| `R_NOTION_CLIENT_ID`      | Optional | Notion public-connection OAuth client ID. Set with `R_NOTION_CLIENT_SECRET` to allow session-initiated Notion authorization. |
| `R_NOTION_CLIENT_SECRET`  | Optional | Notion public-connection OAuth client secret. Register `<public Roomote URL>/api/mcp-oauth/callback` in Notion. |
...
| `R_TRUSTED_PROXY_CLIENT_IP_HEADER`        | Optional         | Header used for client-IP abuse limits: `fly-client-ip`, `x-forwarded-for`, or `x-real-ip`. Set this only when the deployment ingress overwrites that header; otherwise callers share a safe fallback bucket. |
