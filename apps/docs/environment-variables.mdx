...
| `R_CUSTOM_MCP_DISABLED` | Optional | Set to `true` or `1` to disable admin-configured custom MCP servers, including their Settings UI, proxy routes, and delivery to tasks. Defaults to `false`. |
| `R_CUSTOM_MCP_ALLOWED_PRIVATE_CIDRS` | Optional | Comma-separated private CIDR ranges that custom MCP servers may reach in addition to public addresses. Leave unset to keep private and special-use ranges blocked. |
...
| `WEBHOOK_RETENTION_DAYS` | Optional   | Days to retain recorded webhook payloads before BullMQ cleanup deletes them. Defaults to `3`.                                                  |
...
| Env var                              | Required          | Used for                                                                                                                                 |
| ------------------------------------ | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `DEFAULT_COMPUTE_PROVIDER`           | Optional          | Runtime default sandbox provider when no admin default is saved. Accepted values are `docker`, `modal`, `e2b`, `daytona`, `box`, `azure`, and the Roomote Cloud-only `roomote`. |
...
| `DOCKER_WORKER_PIDS_LIMIT`           | Optional          | Process limit per Docker task. The runtime default is `2048`; installer-managed production Compose sets `512`.                          |
...
| `MODAL_BASE_IMAGE_REF`               | Modal infrastructure | Worker-compatible base image reference. Process env, release-image derivation, or installer/deployer synchronization owns this value; Settings does not collect it. |
| `MODAL_REGISTRY_USERNAME`            | Optional          | Process-only registry username used when Modal pulls a private base image.                                                               |
| `MODAL_REGISTRY_PASSWORD`            | Optional          | Process-only registry password or token used when Modal pulls a private base image.                                                      |
| `MODAL_ECR_OIDC_ROLE_ARN`            | Optional          | Process-only AWS IAM role ARN used for Modal ECR OIDC access. Set together with `MODAL_ECR_REGION`.                                      |
| `MODAL_ECR_REGION`                   | Optional          | Process-only AWS region for Modal ECR OIDC access. Set together with `MODAL_ECR_OIDC_ROLE_ARN`.                                          |
...
| `E2B_TEMPLATE_ID`                    | E2B infrastructure | Worker template ID. Process env or Roomote's artifact provisioning owns this value; Settings does not collect it.                       |
...
| `DAYTONA_SNAPSHOT_NAME`              | Daytona infrastructure | Worker snapshot name. Process env or Roomote's artifact provisioning owns this value; Settings does not collect it.                  |
...
| `BLAXEL_IMAGE`                       | Blaxel infrastructure | Worker-compatible sandbox image. Process env or Roomote's artifact provisioning owns this value; Settings does not collect it.       |
...
| `TELEGRAM_API_BASE_URL`         | Optional          | Telegram Bot API base URL. Defaults to `https://api.telegram.org`.                |
