...
| `SANDBOX_OPENROUTER_API_KEY`           | Preview inference | Dedicated spend-capped OpenRouter key that the launcher maps to `OPENROUTER_API_KEY` while preparing an environment workspace. Set it on the parent controller, never in an environment definition. |
