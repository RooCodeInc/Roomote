...
| `DOCKER_WORKER_DISK_LIMIT`    | Optional          | Writable-layer disk limit per Docker task. Defaults to `20g` and requires storage-driver quota support.                        |
...
| `DOCKER_WORKER_EGRESS_POLICY` | Optional          | `internet` (default, public egress with private and metadata ranges blocked) or `none` (no external egress).                   |
...
| `TEAMS_BOT_NAME`                       | Optional          | Teams bot display name for generated app packages and invocation guidance.      |
