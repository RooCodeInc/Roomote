...
| `R_WEB_SHUTDOWN_DRAIN_MS` | Optional           | Same window for Fast turns owned by the web process. Defaults to `R_API_SHUTDOWN_DRAIN_MS`, then `20000`; unfinished turns are handed to the durable queue before Next.js exits. |
