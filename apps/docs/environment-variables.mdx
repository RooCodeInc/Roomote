...
| `R_SESSION_EGRESS_API_PROXY_ENABLED` | Optional, API and controller | Set to `true` to let attached coding runs on compute providers without a per-workload connector (Modal, Roomote Cloud) receive substitute tokens and call owner-approved services through the API's session egress proxy. Off by default; those runs then get no tokens and the Session shows why. |
