...
task. It tells Roomote which repositories, if any, to clone, what services to
start, what environment variables are available, and what guidance it should
follow.
...
Open **Settings > Environments > New** and optionally choose the repositories
for the environment. Leave the selection empty when the workspace only needs
services, tools, variables, integrations, skills, or agent guidance. Add any
setup guidance you already know, then select **Start Agent** so Roomote can
define a reusable workspace configuration. If you already know the
configuration you need, select **Enter YAML directly** instead.
