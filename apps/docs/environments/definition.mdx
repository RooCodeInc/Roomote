...
When a Roomote agent starts a task, the [compute provider](/compute) gives it a
...
| --- | --- | --- |
| Base machine | [`image` / `build`](https://containers.dev/implementors/json_reference/#image-specific) | Provided by the [compute provider](/compute) worker image, not set here |
