...
[environment](/environments): which repositories, if any, to clone, which
services to start, which tools and environment variables to provide, which
ports to preview, and what guidance every task in that environment should
receive.
...
| `repositories`            | list              | no       | Repositories to clone. Defaults to none. See [Repositories](#repositories).                                                |
...
`repositories` is optional and defaults to an empty list. Omit it for an
environment that only needs repository-independent services, tools,
environment variables, integrations, skills, ports, or agent guidance. An
empty list is also accepted. Each listed repository is cloned onto the shared
workspace.
