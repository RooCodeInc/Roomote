...
Self-hosted deployments can check environment definitions into git alongside
their deployment configuration. This gives environment changes the same review
and rollback workflow as application infrastructure.
...
To apply the checked-in files, mount the directory read-only into the Roomote
API container and point `ROOMOTE_ENVIRONMENTS_DIR` at the mounted path:
...
Roomote does not clone or poll a separate configuration repository. The
deployment must check out the files and mount them, or supply their contents
through `ROOMOTE_ENVIRONMENTS_YAML`. See [Environments](/environments#provision-environments-declaratively)
for reconciliation behavior and [Self-hosting](/self-hosting) for deployment
options.
