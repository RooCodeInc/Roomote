...
`repositories` is a required list, but it can be empty. Use `repositories: []`
for an environment that only needs repository-independent services, tools,
environment variables, integrations, skills, ports, or agent guidance. Each
listed repository is cloned onto the shared workspace.
