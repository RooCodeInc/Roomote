...
Messages waiting for a later turn appear above the web reply box as queued.
The sender can delete an individual queued message before Roomote delivers it;
once the agent has received the message, it can no longer be deleted from the
queue.
