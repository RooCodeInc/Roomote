...
## Pull request actions without a workspace

Ask a Session to update a pull request's title or description, close or reopen
it, or add, edit, or reply to a pull request comment. Fast can perform these bounded
GitHub, GitLab, and Bitbucket actions directly, without starting a coding task. An explicit request
to close or update an identified pull request authorizes that action; Roomote
reports the provider result. Bitbucket supports replies to existing comment
threads, but reopening a declined pull request is not supported by these tools;
Roomote reports that capability gap without claiming success.

You must be an authenticated Roomote member. Specify the provider, active
connected repository, and pull request number. Writes use the deployment's
provider credential, which must have permission for the requested action.
GitHub additionally requires a non-suspended App installation. A linked personal
provider account and Roomote administrator access are not required. Fast records
these tool calls in its audit trail.
Existing repository, PR, issue, and Actions reads remain available as before.

These direct tools do not merge pull requests, delete branches, push code,
change a PR's base branch, or perform repository administration. Repository
implementation and validation still use a coding task.
