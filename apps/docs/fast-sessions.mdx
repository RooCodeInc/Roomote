...
With the optional [GitLab API tools](/providers/source-control/gitlab#optional-gitlab-api-tools-for-fast)
configured, Fast can read bounded repository content and commit history, inspect
merge requests, and make supported MR updates, notes, and replies through the
API without cloning. Available tools depend on deployment configuration; this
does not enable the full upstream GitLab catalog. Local edits, commands, and
validation still delegate to tasks, and structured reviews follow the same
Review Code pipeline.
...
The official [GitLab MCP](https://docs.gitlab.com/user/model_context_protocol/mcp_server/)
and [Bitbucket Cloud MCP](https://support.atlassian.com/atlassian-ai-gateway/docs/supported-tools/#Bitbucket-Cloud-tools)
are not enabled for this direct Fast path. Their compatibility with Roomote's
existing credentials has not been verified; existing task workflows are unchanged.
GitLab's optional operator-run route is described in
[GitLab API tools](/providers/source-control/gitlab#optional-gitlab-api-tools-for-fast).
