...
[Gitea](/providers/source-control/gitea#native-pull-request-merging-in-fast),
[Bitbucket Cloud](/providers/source-control/bitbucket#api-first-work-in-fast),
and [Azure DevOps](/providers/source-control/azure-devops#native-pull-request-merging-in-fast).
...
Fast reports success or retries an ambiguous result.
...
CSV and TSV files open as bounded table previews; switch to **Source** when you
need the complete loaded text. The viewer reports malformed or truncated data
without discarding the values it can display.
...
From any Fast surface, you can ask Roomote to read an authorized Slack or
...
Fast can also post an explicitly requested update to Slack from another chat
surface. Outside a Slack conversation, name the Slack workspace and destination.
Public channels require the Roomote app to be a member; private channels also
require your linked Slack account to be a member, and direct messages require
workspace-scoped linked identities for both participants.
