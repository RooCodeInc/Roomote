...
For public `github.com` repositories, active Roomote members can read exact
source paths, issues, and pull requests without a GitHub connection or personal
account linkage. Public reads include bounded PR search in one repository,
diffs, reviews, ordinary discussion, status, and check runs, but not anonymous
code or repository search, Actions, or review threads. See
[public GitHub reads](/providers/source-control/github#public-github-reads-in-fast)
for supported tools and limits. Private reads and writes still need an eligible
connection; access denials are not retried anonymously.
