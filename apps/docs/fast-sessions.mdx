...
requires execution access and terminally cancels the displayed task run,
shutting down that run's sandbox without stopping the parent Session or sibling
tasks. Cancellation errors appear on the card; check the resulting task status
