...
### Code Mode Integrations (Experimental)

An administrator can enable **Code Mode Integrations** under **Settings >
Experimental**. When enabled, Sessions can search and call each connected
integration's tools through OpenCode's confined code-mode runner instead of the
on-demand dispatcher. This can make multi-tool integration work more efficient,
while the same connection, authorization, and visibility rules still apply.

The experiment is off by default. Turn it off to return Sessions to the normal
integration dispatcher; no connection or permission changes are made either way.
