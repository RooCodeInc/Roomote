...
Fast uses existing integration APIs for bounded repository exploration when the
available tools can answer the question, including GitHub, GitLab, and Bitbucket
where configured. Reading files and directories, searching code, or inspecting
commits, pull/merge request diffs, and reported CI results does not require a
coding task, local clone, or workspace provisioning. Available reads depend on
the connected integration's tools and permissions. Reading CI results is not the
...
For example, ask "Where is request authentication handled on the develop
branch?" or "Summarize the changes in this pull request." Roomote uses immutable
