...
## GitHub actions without a workspace

Ask a Session to update a pull request's title or description, close or reopen
it, or add a comment to an issue or pull request. Fast can perform these bounded
GitHub API actions directly, without starting a coding task. An explicit request
to close or update an identified pull request authorizes that action; Roomote
reports the result after GitHub confirms it.

The repository must be active and connected through a non-suspended GitHub App
installation. Your linked GitHub account must have write access to that
repository, and the App must have permission for the requested action. Roomote
checks access for each write and does not use a different user's access when a
check fails. Reconnect your linked account if its GitHub username has changed.
