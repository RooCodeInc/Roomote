...
without setting up an environment for it. Roomote sees the deployment's active
...
own avatar is omitted from the viewer list. If you can manage the session,
select its title to rename it. The **More actions** menu can stop every active
task in the session without deleting its work, or archive the session and its
attached tasks. Archiving is reversible, keeps artifacts and memories, and
cancels the session's scheduled wakeups.
