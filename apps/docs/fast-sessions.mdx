...
Roomote can also send an explicitly requested Slack or Telegram direct message
to your linked account, or post an explicitly requested update to Slack from
another chat surface. Outside a Slack conversation, name the Slack workspace and
destination. Roomote can discover linked direct-message recipients only in Slack
workspaces where your account is linked. Public channels require the Roomote app
to be a member; private channels also require your linked Slack account to be a
member, and direct messages to another member require workspace-scoped linked
identities for both participants.
