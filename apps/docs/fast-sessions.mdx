...
## Code Mode Integrations experiment

A deployment admin can enable **Code Mode Integrations** under **Settings >
Experimental**. When enabled, sessions discover and call each connected
integration through OpenCode's confined code-mode runner and tool search instead
of the standard on-demand integration dispatcher. Existing authorization,
visibility, and credential-mediation rules do not change.

An integration connected during a session normally becomes available in the
same turn; if the running server cannot mount it, the next turn picks it up. If
two integration IDs would map to the same code-mode tool namespace, Roomote
keeps that conversation on the standard dispatcher instead.
...
Select the generated title in the header to rename the session. A title you set
manually stays in the workspace and Sessions list instead of being replaced by
later generated titles.
...
The session overflow menu in the workspace and each manageable Sessions-list
row also offers **Stop**, **Archive**, and **Delete**. Stopping a session pauses
all of its active tasks without permanently canceling them. Archiving is
reversible and hides the session and its linked tasks from ordinary lists and
search until it is unarchived. Deleting a session uses the existing confirmation
...
Admins can enable **Task communication triage** under **Settings > Experimental**
to decide which delegated-task updates should reach the session between reports.
With a judgment model configured, Roomote keeps routine activity quiet and
surfaces blockers, changed assumptions, consequential choices, useful
milestones, or off-track work. The setting is off by default. Without a
configured judgment model, activity digests are dropped and explicit task
reports continue as before.
...
The Sessions page shows the list by default. An administrator can enable the
**Sessions board** under **Settings > Experimental** to add a board view with
**Active**, **Needs input**, **Blocked**, **Ready**, and **Done** columns. The
setting applies deployment-wide: members see the board only for sessions they
can already access.

Admins can separately enable **Session status judgment** to let the deployment's
configured judgment model classify settled session outcomes. The judgment uses
recent visible conversation and child-task summaries. Active work and structured
input requests continue to use their deterministic status; uncertain judgments
fall back to the existing status. A Done column is informational and never
closes, cancels, or archives a session or task. Both experiments are off by
default.
