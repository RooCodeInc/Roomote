...
repository starts the execution directly and opens its task workspace. Roomote
still creates the owning Session so the execution remains part of the same
shared history.
...
task in the Session, labels which task produced each item, and opens previews
without leaving the Session. Individual task details continue to show their own
artifacts.
...
Replay-safe Fast turns also survive API or worker interruptions. Roomote
durably admits the turn before acknowledging it, then resumes unfinished work
on a live process without repeating the acknowledgement or already-completed
side effects. If an inference provider asks Roomote to retry later, replay-safe
turns can wait durably and resume at the scheduled time; longer waits use one
visible retry notice that the eventual answer replaces.

### Message Suggestions

Admins can enable **Message Suggestions** under **Settings > Experimental**.
When enabled, an empty task or Session composer can show one contextual
follow-up after an agent turn completes. Press **Tab** or select the suggestion
to insert it, press **Escape** to dismiss it, or start typing to replace it with
your own message.
...
requested. The setting is off by default.
