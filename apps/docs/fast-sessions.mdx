...
also supports requested changes to an existing pull request's title or body,
close/reopen, top-level issue or PR comments, and replies to existing PR review
comments directly in Fast. Writes require an active Roomote member, an active
connected repository, and the GitHub App installation's applicable permission;
no personal GitHub OAuth connection is required. This bounded path does not
merge or create PRs, change files or branches, manage reviewers or draft status,
