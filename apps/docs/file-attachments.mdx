...
Audio transcription and video descriptions use a separate **Audio and video
model** in **Settings > Models**, defaulting to **Same as coding model**. Choose
a model that supports the input you use. Roomote warns when its model catalog
shows that audio or video input is unsupported, and does not silently switch to
the Vision or coding model if media processing fails.
