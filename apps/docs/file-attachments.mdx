...
Attachments sent to a session stay available as conversation context. When Roomote
...
| Code            | `.html`, `.htm`, `.css`, `.js`, `.jsx`, `.ts`, `.tsx`, `.py`, `.r`, `.ipynb`, `.rb`, `.go`, `.rs`, `.java`, `.c`, `.h`, `.cpp`, `.cc`, `.cxx`, `.hpp`, `.sql` |
| Spreadsheets    | `.csv`, `.tsv`, `.xlsx`, `.xls`                                                                                                               |
| Documents       | `.pdf`, `.docx`                                                                                                                               |
| Presentations   | `.pptx`                                                                                                                                       |

An attached `.r` script can also drive an environment recipe: Roomote detects
the direct packages it uses and proposes an `r-bioconductor` environment, then
resolves those packages from the official CRAN and Bioconductor repositories.
See the [environment lifecycle](/environments#environment-lifecycle).
...
Audio transcription and video descriptions are opt-in. In **Settings > Models**,
turn on **Also use for audio and video** under **Vision model** and choose a
model that supports both inputs. When the setting is off, Roomote does not send
audio or video attachments to an inference model. When it is on, those
attachments use only the Vision model. Existing deployments need to enable this
setting after upgrading to restore audio and video processing.
...
## Open and share artifacts

Artifact creation results can include two links:

- `viewUrl` opens the artifact in its owning task or session, where you can
  continue to use the surrounding workspace context.
- `standaloneViewUrl` opens the document, image, or file on its own page. Use it
  when someone needs a direct link to the artifact without navigating through
  the parent task or session.

Both links remain inside Roomote's authenticated access boundary. The recipient
must be signed in to the deployment and already have permission to view the
owning task or session; a link does not make the artifact public or grant new
access. The standalone page also supports available artifact versions and a
link back to the owning task or session when that context is useful.

When an agent needs to inspect an existing artifact, it can use the
`open_artifact` capability with the exact owning task or session and stored
path. The capability currently returns bounded content for supported text
artifacts up to 64 KiB; binary or oversized artifacts should be opened through
their artifact link or downloaded instead. Access is checked against the
owning task or session before content is read.
...
available on the current surface, Roomote shares a direct link to open the file
on its own page, without an error or unavailability explanation. Roomote only
describes a video as attached when native delivery succeeds.
