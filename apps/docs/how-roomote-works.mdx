...
and which environment fits, and it says so before delegating. In Slack and
Telegram, some requests still show a confirmation card you can accept, cancel,
or correct by replying naturally. When you start from the dashboard, choose
the environment yourself or leave the selector on **Fast** to let the Session
decide. Linear can also suggest an environment from issue context.
