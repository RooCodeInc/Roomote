...
| Email         | Email the deployment's AgentMail inbox from a verified account address | Replies in the same email thread and task view |
...
| GitHub        | Pull request events or `@`-mentions in PR and issue comments | GitHub comments, reviews, and task view |
...
| Bitbucket     | Pull request events or `@`-mentions when configured        | Bitbucket activity and task view        |
...
needed. In Slack, Telegram, and Discord, you can use the confirmation controls
or reply naturally to accept, cancel, or correct that suggestion. When you
start from the dashboard, choose the environment yourself or use **Auto** when
you want Roomote to route the task. Linear can also suggest an environment from
issue context.

If a Slack request is ambiguous, Roomote shows the environment picker so you can
choose where the task should run. If automatic routing is temporarily
unavailable, the picker includes a warning instead of silently falling back.
Choose an environment to continue; if the warning persists, ask an admin to
check the deployment's inference provider.
