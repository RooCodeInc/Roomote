...
3. While you are viewing the Session, the key form opens automatically. If you
   dismiss it, an **Add your <service> key** card remains at the end of the
   conversation, and the agent's secure link opens the same form. Enter the
   key there; the form bypasses chat and is excluded from capture and replay.
   If the agent asked for write methods, the form shows them and you confirm
   them.
