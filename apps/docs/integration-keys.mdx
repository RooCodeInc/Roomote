...
first reuses a suitable connected integration. If none is connected, it checks
bounded official provider documentation for a compatible hosted remote MCP
server. Integration keys are the fallback when no suitable connectable MCP is
available, or when you explicitly ask to use the service's API. A missing
installed connector alone does not skip MCP discovery.
