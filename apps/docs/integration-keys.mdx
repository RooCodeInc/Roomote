...
When you ask to connect a service without choosing a connection type, Roomote
first reuses a suitable connected integration. If no suitable integration is
connected for the requested service, it checks bounded official provider
documentation for a compatible hosted remote MCP server. Integration keys are
the fallback when no suitable connectable MCP is available, or when you
explicitly ask to use the service's API. A missing installed connector alone
does not skip MCP discovery.
...
1. You explicitly choose API access, or Roomote determines that no suitable
   connectable hosted MCP server is available for a service you hold a key for,
   such as
...
If remote MCP setup instead returns an authorization link or asks for manual
client registration, that setup is still pending. Roomote does not create a
duplicate integration-key approval, and an authorization denial is not bypassed
through the API-key route.
