...
built-in integration for. Any member can add a remote server, the same way
they add an [integration key](/integration-keys): shared with everyone under
**Integrations**, or private under **Personal Settings → Personal MCP
servers**. You can also ask Roomote in a session to connect a service's MCP
server. Remote servers become available to both sessions and sandbox tasks.
Local stdio servers need a task sandbox because Roomote launches their
process inside that workspace, and only deployment admins can add them.
...
sandbox agents and sessions use an authenticated Roomote proxy, and the proxy injects the
...
- OAuth tokens refresh centrally, so long-running tasks keep working across
  token expiry.
- Per-tool choices (the **Manage tools** dialog) are enforced at the proxy:
  disabled tools are hidden from agents and blocked even if called directly,
  and a task cannot run an **Always ask** tool until its session owner allows
  that exact call.
...
needing reconnection: agents get a clear error pointing at the **Integrations**
page, or at Personal Settings for a private server, and the server card shows
**Connect** again.
...
When you add a remote server you choose who can use it:

- **Everyone in this deployment** (the default) shares it with every member.
  Agents in every session and task can use its tools, acting with the account
  that authorized it. It appears on the **Integrations** page. The member
  who added it and deployment admins can edit, connect, or remove it; everyone
  else sees it read-only.
- **Only me** keeps it private. Only your own sessions and tasks receive it,
  it appears under **Personal Settings → Personal MCP servers**, and nobody
  else, admins included, can see or manage it. Roomote checks the owner on
  every request, so a personal server's credentials are never used on another
  member's behalf.

The owner, or an admin for a shared server, can change this later from the
server's dialog. Roomote moves it between the two pages and keeps its
connection, so you do not authorize again. A shared server goes private to the
member who added it. Personal server names only need to be unique among your
own servers, and each member can keep up to 10. A personal server stops
working if its owner's account is deactivated.

## Scoping and precedence

Shared custom servers apply to every task. A personal server with the same
name takes precedence for its owner, and an environment's own
[`mcpServers`](/environments/definition#mcp-servers) entry with the same name
takes precedence in that environment, so the more specific scope wins. Names
that collide with built-in integrations (for example `linear` or `github`)
...
- Any member can add a remote server and manages the ones they added.
  Deployment admins manage every shared server and are the only ones who can
  add local (stdio) servers. Personal servers are managed by their owner alone.
- `R_CUSTOM_MCP_DISABLED=true` disables the feature entirely: custom MCP
  servers on the **Integrations** page and in Personal Settings, the proxy
  routes, and delivery to tasks. It is independent of
