...
HTTP integrations let sessions and sandbox agents call an API without
...
For a general "connect to X" request, Roomote checks the read-only built-in
catalog first, including disconnected providers. It uses that provider's native
setup path when present, then checks for an official remote MCP server, and uses
HTTP integrations or an integration key only when neither applies. An MCP that
requires provider approval, an allowlisted client, a beta or plan, or a token
you hold is offered as an option rather than treated as immediately connectable.
An explicit request for direct API access keeps that choice. Discovery does not
start setup, and permission denials or pending or denied authorization are not
bypassed.
...
broker also serves [Integration keys](/integration-keys), which are individually
approved by the Session owner and do not require an operator manifest.

For integration keys only, leave `R_HTTP_INTEGRATIONS_ENABLED` unset or `false` and
...
required. See [Integration keys self-hosting](/integration-keys#self-hosting).
...
2. Set `R_HTTP_INTEGRATIONS_ENABLED=true` on the Roomote control-plane services
   that run the API, resolve task configuration, or execute sessions
...
dynamic-only mode. Disable operator mode intentionally if only integration keys
are wanted.
...
- A session uses its acting user's authentication. Sandbox requests use their
...
server exposes `list_integrations` and `integration_request` to both sessions and
...
for owner-approved integration keys, but no operator manifest entries are loaded.
This does not cancel an already-running operator request. Revoke personal
integration keys under **Personal Settings → Personal integrations**;
administrators revoke deployment-wide keys under **Settings → Integrations**.
Those changes are checked live before dispatch and before returning upstream
responses, without reloading the manifest or restarting.
