...
for that service, then the service's official remote MCP server, and uses HTTP
integrations or an integration key when neither applies or when you ask for the
service's API.
