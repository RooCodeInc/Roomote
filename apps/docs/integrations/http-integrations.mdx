...
This does not cancel an already-running operator request. Revoke integration keys
separately under **Settings → Integrations → Your integrations**; those changes
are checked live before dispatch and before returning upstream responses, without
reloading the manifest or restarting.
