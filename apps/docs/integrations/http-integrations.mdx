...
HTTP integrations or an integration key only when neither applies. An explicit
request for direct API access keeps that choice. Discovery does not start setup,
and permission denials or pending or denied authorization are not bypassed.
