...
You do not need to know MCP terminology. Roomote prepares a secure Settings
link, preferring a built-in integration when one is available. Preparing the
link does not save, authorize, or verify a connection.
...
For a service outside the built-in catalog, the link opens
[custom setup](/integrations/custom-mcp-servers). An admin needs an existing
compatible remote MCP service URL from the provider, entered in secure
Settings. An ordinary API URL is not enough: this flow does not invent
endpoints, build API adapters, or host a server for you.
...
operation before reporting what it can access. Running tasks may need a new
task to pick up a newly added integration. Fast reloads its catalog on later
turns, but cached results can delay visibility; a later turn or new Session
may be needed, and neither guarantees an immediate refresh. Missing tools
