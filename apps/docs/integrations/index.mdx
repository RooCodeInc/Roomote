...
This curated catalog is disabled by default. Deployment operators enable it
by setting `R_CURATED_INTEGRATIONS_ENABLED=true` and restarting Roomote.
While disabled, none of these integrations can be configured or used;
existing connections remain stored but inactive and are restored when the
policy is enabled. This policy does not affect the separate provider
categories above or MCP servers defined on an environment.
