...
`get_partnership_guide`. Then send Roomote a message beginning with
`Agent (on behalf of user):` and ask it to load its packaged
`roomote-partnership` skill by exact name with `list_skills`, then `load_skill`.
Read Roomote's reply and confirm that it reports an actual successful load and
its current state. A skill listing or mention is not a load; if loading is
unavailable or fails, report that honestly before claiming the paired workflow
is established.

Use this client-initiated sequence for reciprocal judgment, not routine task
dispatch or self-contained local work. Relay substantive exchanges as
`Agent → Roomote` and `Roomote → Agent`, read the full reply before answering,
and keep the final answer self-contained.
...
When relaying between an MCP client and Roomote, summarize substantive messages
with the labels `Agent → Roomote` and `Roomote → Agent`. Relay questions or input
