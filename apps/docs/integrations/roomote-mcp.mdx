...
Roomote also includes a small stdio bridge for clients or workflows that
require a local command transport. Build `packages/mcp-server` from a Roomote
checkout and follow its package README to configure Claude Code or Cursor.

The bridge requires `ROOMOTE_URL` and an existing user-scoped bearer token in
`ROOMOTE_ACCESS_TOKEN`. It does not currently run or refresh the browser OAuth
flow, so prefer the hosted `/mcp` endpoint above when your client supports
remote MCP OAuth.

When using the hosted `/mcp` configurations above, Roomote opens a browser so
you can sign in and approve the connection. During OAuth dynamic client
registration, the client sends Roomote its callback URL in `redirect_uris`.
Roomote validates and saves that URL, then only sends the result to the same
registered URL after approval. Hosted OAuth connections do not require you to
