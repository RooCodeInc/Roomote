...
with the labels `Client → Roomote` and `Roomote → Client`. Relay questions or input
...
metadata and bounded content for supported text formats. Binary, unsupported, or
oversized artifacts should be opened through their artifact link or downloaded
instead. Roomote checks access to the owning task or Session before reading content.
