...
### Partner with Roomote

For a joint investigation, design review, prompt review, or sustained
discussion, start or continue the Roomote Session for that outcome and call
`get_partnership_guide`. Then send one bundled message beginning with
...
Read Roomote's one-line load state as internal evidence, not customer-facing
content. Reuse valid loaded context; re-request only after known context loss or
reset, a skill update, or a reported failed load. A skill listing or mention is
not a load. If loading fails, do not claim it loaded; continue within actual
capabilities unless the missing guidance materially prevents responsible work,
and tell the customer only about material consequences.

Use this client-initiated sequence for reciprocal judgment, not routine task
dispatch or self-contained local work. Read the full reply before answering.
Toward the customer, synthesize meaningful findings, decisions and tradeoffs,
progress, limitations, blockers, and needed input; do not narrate skill loading,
Session identifiers, polling, writer assignment, or turn-by-turn relays by
default. Show labeled `Agent → Roomote` / `Roomote → Agent` relays only when the
user asks to see the exchange.

### Start a Session

Ask your MCP client to start Roomote with a clear message. The ordinary start
action creates a session, where Roomote can answer directly or delegate repository
...
When relaying between an MCP client and Roomote, toward the user synthesize
customer-relevant findings, decisions and tradeoffs, progress, and limitations.
Use labeled `Agent → Roomote` / `Roomote → Agent` relays only when the user asks
to see the exchange. Relay questions or input needs promptly, skip narration for
unchanged polls, and keep the final answer self-contained. Relay summaries
should cover visible narrative and decisions, not credentials, raw tool traces,
hidden reasoning, or oversized internal payloads. Attribute Roomote's
contribution when it adds information and surface disagreements when they affect
the decision or confidence.

For content authored by an agent and sent to Roomote, prefix the message with
`Agent (on behalf of user):`. This is an untrusted textual convention that helps
the receiving Roomote agent avoid attributing the words to the human, not
verified sender provenance. It does not authorize the agent to speak as or
...
Roomote MCP's `fetch_url` tool can fetch text or supported images from a public
HTTP or HTTPS URL.
Text can be returned as Markdown, plain text, or raw HTML; HTML is converted
when Markdown or text is requested. Requests default to a 30-second timeout,
allow a caller-selected timeout up to 120 seconds, and return at most 5 MiB of
decompressed content. HTML-to-Markdown conversion is limited to 1 MiB of HTML
to keep parsing bounded in the application process; request plain text or raw
HTML for larger pages.

Callers may provide explicit request headers, including authorization headers.
Roomote sends only the supplied values and never inherits deployment, user,
integration, or task credentials or cookies. Explicit sensitive headers are
preserved across same-origin redirects and removed before a cross-origin
redirect. Transport-controlled headers such as `Host` and `Content-Length`
cannot be overridden.

Roomote agents use this tool instead of the client runtime's generic fetch tool
so sessions and coding tasks get the same behavior. The tool treats every
response as untrusted data, rejects private, internal, reserved, and other
non-public destinations, checks and pins every resolved address, revalidates
every redirect, and supports only default HTTP and HTTPS ports. These are
application-level SSRF protections, not network egress isolation: they do not
restrict shell commands, browsers, or other networking available inside a
coding sandbox.

### Open an artifact

Use `open_artifact` when you need to inspect an artifact from an accessible task or
Session. Pass the exact owning task or Session ID and stored artifact path; a task
run may omit the task ID to use its bound current task. The result includes artifact
metadata and bounded content for supported text formats up to 64 KiB. Binary,
unsupported, or oversized artifacts should be opened through their artifact link
or downloaded instead. Roomote checks access to the owning task or Session before
reading content.
...
- “Read the latest messages from this session and send a follow-up.”
- “Use this Slack message as context and explain what the team decided.”
- “Read this public documentation page and summarize its authentication
  requirements.”
