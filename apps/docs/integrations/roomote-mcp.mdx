...
Roomote MCP can fetch text from a public HTTP or HTTPS URL without sending
credentials, cookies, or caller-selected headers. Roomote agents use this tool
instead of the client runtime's generic fetch tool so Fast Sessions and coding
tasks get the same redirect, timeout, response-size, and public-destination
checks.

The fetch tool returns text only and treats it as untrusted data. It rejects
private, internal, reserved, and other non-public network destinations, checks
every redirect, and supports only the default HTTP and HTTPS ports. These are
