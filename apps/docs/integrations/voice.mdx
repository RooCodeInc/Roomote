...
voice on. An environment-managed key cannot be edited or disconnected in
Settings and uses Marin because it has no stored voice selection, but a
deployment admin can still turn Voice off and back on for the deployment from
the Voice card. A key saved in Settings is kept while Voice is off.
