...
any session, on the home page, or in the **New Session** dialog.

## How setup works

A deployment admin connects Voice once from the **Integrations** page with an
...
voice on. An environment-managed key cannot be edited or disconnected on the
**Integrations** page and uses Marin because it has no stored voice selection,
but a deployment admin can still turn Voice off and back on for the deployment
from the Voice card. A key saved on the **Integrations** page is kept while
Voice is off.
