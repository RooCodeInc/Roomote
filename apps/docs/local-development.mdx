...
- **A public HTTPS URL** — set `R_PUBLIC_URL` to a URL that reaches the local
  Caddy edge on port 18080. An ngrok domain, Cloudflare Tunnel, Tailscale
  Funnel, or your own domain works. Install ngrok only when you use an ngrok
  domain.
...
service code, resolves your public callback URL, starts Postgres, Redis, MinIO,
and the required Brain (`gbrain`) service if needed, builds the local worker
release archive, and starts the Roomote services.
