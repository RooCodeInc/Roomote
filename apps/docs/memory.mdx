...
- **Linear issues** belonging to public teams in the connected workspace
...
Linear collection includes only issues belonging to public teams. Issues in
private teams are excluded even when the deployment's Linear app can read them,
and an authoritative visibility census removes a previously collected page if
its issue moves to a private team.
