...
a bearer key. The upstream must serve the exact model names configured above,
and `R_BRAIN_RERANKER_MODEL` keeps the `openrouter:` prefix because Roomote
removes it when forwarding to the rerank upstream.
