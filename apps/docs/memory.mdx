...
The **Enable Memory** toggle at the top of **Settings → Memory** is what
turns it on; no provider key is required for that. Synthesis runs through
your deployment's helper model — the same small model that already writes
task titles and summaries. Deployments that enabled Memory before the toggle
existed, by setting `R_BRAIN_OPENROUTER_API_KEY` or `R_BRAIN_OPENAI_API_KEY`,
stay enabled without doing anything; using the toggle stores an explicit
choice that wins over the key from then on.
...
<Note>
  ...
  container up, then enable Memory in Settings. Everything after that is the
  same.
</Note>
