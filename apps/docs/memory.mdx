...
should guide future work. Only a confident yes leads to a save; the helper
model then writes the memory, which goes through the same redaction and
filing. A task memory written this way says so, grows turn by turn, and is
