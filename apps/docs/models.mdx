...
configure hourly checks for queryable limits from ChatGPT Subscription,
OpenRouter, Kimi for Coding, OpenCode Go, Z.AI, and Z.AI Coding Plan. Roomote
posts one warning when a credential crosses its configured threshold in a limit
window, plus a critical warning at 100%. Alerts can report through Slack or
Discord destinations and supported primary conversations in Teams, Telegram,
or Discord. The warning names the provider, credential fingerprint or label,
limit window, and reported usage. A provider that does not return usable quota
data is skipped.
