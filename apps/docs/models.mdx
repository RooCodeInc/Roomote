...
| Vision model         | Image inspection; optional audio transcription and video descriptions                   | Image input support; audio and video input only when enabled                              |
...
inspect them. Audio transcription and video descriptions are off by default.
To enable them, turn on **Also use for audio and video** under **Vision model**
in **Settings > Models**, then choose a model that supports both inputs. When
enabled, audio and video use only the Vision model; Roomote does not fall back
to the helper or coding model for those attachments.

After upgrading, audio and video remain off until an admin enables this setting.
This avoids sending attachments to a model that cannot process them.
