...
**GPT-6 Luna** is the default coding model for Roomote's default OpenRouter
...
GitHub Copilot mapping presets also select Luna with medium reasoning. Existing
saved defaults and role mappings do not change automatically.
...
A TypeSafe key alone turns on **Jev via TypeSafe**. The Roomote judgment
model, OpenRouter, and Vercel AI Gateway answer decisions only when you select
them. The one exception is `R_JUDGMENT_SHADOW=on`, which also sends every Jev
decision to the Roomote judgment model to compare answers, without changing the
answer Roomote acts on.
Many deployments already have OpenRouter or Vercel AI Gateway keys for task
inference, and choosing them sends decision text to a third party.
