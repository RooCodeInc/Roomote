...
and integration-tool ranking. Roomote does not automatically send those new
high-volume workloads to an ordinary language model. In **Settings > Models >
Available Models**, turn on **High-volume decisions** only for a model that the
deployment has verified is suitable for that workload. The setting is off by
default and is preserved when catalog metadata is refreshed. A configured Jev
judgment model is already suitable for these batches.
