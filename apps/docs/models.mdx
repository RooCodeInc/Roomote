...
fast call. Roomote currently supports TypeSafe's Jev through these routes:
...
`R_JUDGMENT_MODEL` to `typesafe`, `openrouter`, `vercel`, or `off`. A TypeSafe
key alone turns on **Jev via TypeSafe**. OpenRouter and Vercel AI Gateway are
used only when you select them, because many deployments already have those
keys for task inference.
...
- checking a finished coding turn against the request: whether a requested
  change is missing, the report claims a change the diff lacks, or debug
  leftovers slipped in. This check has no helper-model fallback; without a
  judgment model the agent runs its own slower review pass instead
...
selected provider; keys stay on the control plane and are never sent to task
sandboxes.
...
through an explicitly configured Jev judgment backend. A helper model never
opts into these batches, so deployments without Jev keep the existing
conservative behavior and do not incur new high-volume language-model calls.
