...
Roomote also checks queryable limits every hour for OpenRouter, Kimi for
...
warning at 100%. The warning names the provider, credential fingerprint or
label, limit window, and reported usage. A provider that does not return usable
quota data is skipped.
