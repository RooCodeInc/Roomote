...
- Subscription models keep the `openai/` model ID prefix (for example
  `openai/gpt-5.4`), so they are selected and displayed like other OpenAI
...
- Once connected, the subscription's recommended models appear in
  **Available Models** (toggled off until you enable one). You can also add
  any other model by entering its slug, such as `gpt-5.4`, with **ChatGPT
...
Moonshot, MiniMax, OpenCode, Amazon Bedrock, and Google Vertex AI keys. Use
`ROOMOTE_MODEL_ENV_KEYS` when a provider key uses a custom env var name:
