...
- whether a settled session turn, or a finished task whose agent recorded no
  memory, holds something worth saving to [Memory](/memory)
...
relevance and integration-tool ranking, or a check after every session turn
and finished task, such as the Memory check. Roomote only runs those high-volume workloads
