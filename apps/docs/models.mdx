...
fast call. Roomote currently supports TypeSafe's Jev, through either route:
...
`R_JUDGMENT_MODEL` to `typesafe`, `vercel`, or `off`. A TypeSafe key alone
turns on **Jev via TypeSafe**. Vercel AI Gateway is used only when you select
it, because many deployments already have that key for task inference.
