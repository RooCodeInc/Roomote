...
- an advisory ranking of a task's own changed hunks by defect risk during its
  self-review, before it pushes or opens a pull request (the same pre-screen
  pull request reviews use), so the agent can re-read the riskiest hunks
