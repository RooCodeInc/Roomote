...
| Media model          | Image inspection, audio transcription, and video descriptions                            | Image, audio, and video input support with clear text output                               |
...
The **Media model** handles image, audio, and video attachments when a separate
model is needed. Choose a model that accepts the attachment types you use; image
support alone does not imply audio or video support. Audio transcription may
also fall back to the helper or coding model when it supports audio input.
...
- a specialized model, such as a media model, only exists behind one provider
...
- **An image, audio, or video attachment cannot be read.** Check the selected
  **Media model** in **Settings > Models** for support for that input type.
