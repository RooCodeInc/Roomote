...
- **Fast response mode** is enabled by default for new homepage prompts and
  messages sent from your linked Slack and Discord accounts. Turn it off here
  if you prefer task execution by default. An explicit homepage workspace
  choice takes precedence. The chat preference does not apply to GitHub, Teams,
  or Telegram. You can still use `!fast` explicitly in Slack whether the
