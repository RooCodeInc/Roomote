...
- **Slack peer conversations** under **Settings > Experimental** to let an
  established Fast Slack thread continue after another participant is mentioned
...
### Continue Slack peer conversations

**Slack peer conversations** is an opt-in experiment owned by the person whose
Fast Session is bound to the Slack thread. When enabled, eligible human replies
can continue reaching Fast after someone mentions another participant. Fast is
instructed to stay quiet around peer discussion unless it has a useful reason to
participate. Direct Roomote mentions work normally with the experiment on or off.
