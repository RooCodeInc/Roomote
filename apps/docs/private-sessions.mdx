...
Private Sessions are experimental and disabled by default. Enable **Private Sessions** in **Personal Settings → Experimental** to show the Private option in the new Session composer. A deployment exposes private integrations only after it also adds and enables an integration that explicitly declares private data. Existing integrations and Sessions remain shared unless their implementation opts into this contract.
...
The Session owner can delete a private Session and its Roomote-managed descendants. Deletion removes the Session, linked tasks and runs, transcript records, artifacts, audits, and Roomote's native-session reference. If an artifact upload URL is still valid, Roomote keeps the records and asks the owner to retry after that URL expires; this prevents a delayed upload from recreating private content after deletion. Disconnecting an integration does not automatically delete existing private Sessions.
