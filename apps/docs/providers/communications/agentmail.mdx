...
time, so it also turns on account email verification (see
[Account email verification](#account-email-verification) below).
...
channel enabled, that guarantee starts at the account:

- New email-and-password sign-ups receive a verification email and must
  confirm the address before signing in.
- Existing accounts that never verified (sign-ups from before the channel
  was enabled) receive a verification email the next time they sign in.
