...
Replying to a Roomote-initiated email works like any other inbound message: the
sender address must first be verified on the account and pass DMARC. Until then,
the reply cannot continue the conversation or start a task.
...
verification. With the email channel enabled:
...
- each account has one email address for outbound delivery; that login email
  must be verified before it can send commands or replies to Roomote, and there
  is no way to add other sender addresses
