...
an unknown address may receive a polite refusal with a link to connect the
address to a Roomote account. Refusals are limited to one per thread and one per
sender each day, with a deployment-wide limit of 25 per day. After the address
is linked, Roomote automatically processes up to 10 previously refused emails
from that sender from the preceding seven days, oldest first. Older or
additional messages need to be resent. Forwarding a Roomote email thread to
someone else does not give them access to the original conversation or task;
...
This is opt-in, never a fallback. The picker offers only the automation owner's
account email when Roomote records it as verified; addresses listed only as
**Linked** senders are not eligible. Each run uses that exact selected identity
for its initial message and threaded report. If verification or membership is
...
- Email automation destinations are private; channel-style posting is not
  supported
- the current account model stores explicit verification for one account email;
  additional **Linked** sender addresses are not automation destinations
