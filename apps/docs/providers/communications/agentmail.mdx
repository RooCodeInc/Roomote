...
trusts the sender address. Sender domains therefore need valid SPF and DKIM
alignment plus a published DMARC policy. Mail that does not pass DMARC is
refused with setup guidance instead of being matched to a Roomote account.
