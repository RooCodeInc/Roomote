...
and Roomote never sends email. New Roomote Cloud deployments have a managed
`<workspace-slug>@roomote.me` inbox by default. Roomote Cloud creates it the
first time the deployment sends an email, such as a verification email or a
notification. Older deployments stay disabled until Roomote enables them.
...
account. Verified addresses can start work immediately. An unverified address
can also reply to a Roomote-initiated email when the reply passes DMARC and
quotes its single-use reference token; that reply verifies the address
automatically. Roomote replies in the same email thread, and follow-up emails in
that thread continue the same conversation; a running delegated task receives
them as follow-ups. Start a new request with a new email so it opens a new
thread.

Roomote also requires AgentMail to report a passing DMARC result before it
trusts the sender address. Sender domains therefore need a published DMARC
policy and either SPF or DKIM aligned with the From domain. Mail that does not
pass DMARC is silently dropped instead of being matched to a Roomote account or
receiving a reply.
...
mailing lists, and similar automated senders from creating reply loops. With
the optional [judgment model](/models#judgment-model) on, Roomote also ignores
mail without those headers that the judgment model is confident is an
automatic reply, such as an out-of-office message or a bounce notice.
...
- Roomote only initiates email to your own account address, including before
  you verify it for inbound email. It
...
Custom automations and built-in automations with a **Destination** control can
explicitly select **Email** as their report destination. This is opt-in, never
a fallback. Custom automations offer the owner's account email; built-in
automations offer the account email of the admin selecting the destination,
even when that address has not been verified yet. Each run uses that exact
selected identity for its initial message and threaded report. If membership
is removed, the address changes, delivery is suppressed, or AgentMail is
disabled or unconfigured, Roomote records a delivery failure and does not
substitute another address or provider.

Replying to a Roomote-initiated email verifies an unverified account address
implicitly: each email Roomote sends to an unverified address carries a
single-use reference token, and a reply from that exact address that passes
DMARC and quotes the token proves the mailbox received it. Roomote marks the
address verified and processes the reply like any other verified reply — no
separate verification message or Settings visit is needed. The token expires
after 30 days and can verify only the account and address it was sent to, so
a forwarded thread cannot verify anyone else, and unsolicited email from an
unverified address is still refused. If the quote is missing (some clients
strip it) or the token has expired, verify from **Personal settings > Linked
Accounts** instead.

## Account email verification

Roomote requires verification before an account email can initiate work or
reply on the email channel. Receiving Roomote-initiated email does not require
verification — and replying to it verifies the address implicitly, as
described above. With the email channel enabled:
...
- Email/password users can request a one-hour reset link from sign-in. Admins
  can still create reset links under **Settings > Users**; those links are
  emailed to the user and also shown to the admin.
...
- Email automation destinations are private; channel-style posting is not
  supported
- each account has one email address for outbound delivery; that login email
  must be verified before it can send commands or replies to Roomote (replying
  to an email Roomote sent it verifies it), and there is no way to add other
  sender addresses
