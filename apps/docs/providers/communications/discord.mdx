...
- use `/skills` to list the exact `$skill-name` tokens available to your linked
  account; set the optional `page` field to continue the list
...
**Peer conversations** under **Settings > Experimental** is off by default. The
deployment admin can enable it so linked participants' messages in established
Roomote Discord threads continue reaching Roomote after a peer is mentioned. Roomote
may ignore ambient discussion, while explicit bot mentions, commands, DMs, and
replies to Roomote continue to receive a response normally. This applies only
inside the established thread; it does not make Roomote monitor the surrounding
channel.
