...
a turn and routes when it is addressed to Roomote. Messages meant for another
participant stay silent; acknowledgements addressed to Roomote reach the
session, which then decides whether a response is warranted. Explicit bot
mentions, commands, DMs, and replies to Roomote always receive a response. This
applies only inside the established thread; it does not make Roomote monitor the
surrounding channel.
...
Roomote can attach task-generated images, such as visual proof, directly to a
Discord reply. Reconnecting or replacing that reply keeps the selected images
attached.
