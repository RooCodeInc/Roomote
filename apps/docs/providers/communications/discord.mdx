...
When the fast orchestrator delegates work, it remains the owner of the Discord
conversation. It relays useful progress and questions from the delegated task,
adds task context when reporting an opened pull request, and reports review
feedback or when the pull request is merged or closed. The delegated task does
not post a separate competing conversation into the thread.
