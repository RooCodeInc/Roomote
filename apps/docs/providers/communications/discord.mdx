...
uses Redis and a dedicated internal shared secret. Set the same
`R_DISCORD_GATEWAY_SECRET` value on the API and BullMQ processes (installers
and platform templates generate this for you). Keep it separate from
`ENCRYPTION_KEY` so a leaked transport secret cannot unlock secrets sealed at
rest.
