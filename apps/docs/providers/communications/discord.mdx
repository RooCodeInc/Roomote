...
the `/new`, `/goal`, `/stop`, `/link`, and `/help` commands automatically. You
...
on the **Automations** page, the same way you would pick a Slack channel.
...
- use `/new request:<request>` in a server channel or thread to open a fresh
...
- use `/goal objective:<objective>` to make the current session pursue an
  objective across multiple turns; it may delegate child tasks as needed
- use `/stop` in a Roomote session thread or DM to resumably stop all active
  tasks linked to that session, including delegated tasks; send another message
  to continue later
- Discord DMs, mentions, attachments, and eligible thread replies from your
  linked account always enter a session, which answers directly or
  delegates repository work into tasks
- replies in a Roomote-owned DM or thread continue that session; replies in an
...
- paste a Discord message link (`https://discord.com/channels/.../.../...`) in
  any session so the agent can pull that message and nearby channel/thread
  context, including when the session started on another chat provider (lookups
  outside the originating Discord conversation require a linked Discord account)
- use `/help` to see the available commands

Roomote can read bounded context from the current Discord thread or channel, use
...
own [custom automations](/automations#custom-automations) through a session; deployment
...
Roomote does not read other channels implicitly
or inspect repository contents without delegating the work to a task. When the
request spans every repository, Roomote can delegate it to the deployment's
all-repositories environment.

Session automation reports can be delivered to configured Discord channels or the
automation owner's DM. Replies continue the report's session in Discord.

Linked participants' messages in established Roomote Discord threads keep
reaching Roomote after a peer is mentioned. When a
[judgment model](/models#judgment-model) is configured, every unmentioned
reply in such a thread with more than one person is judged before it can start
a turn and routes when
it is addressed to Roomote and is more than a closing acknowledgement;
otherwise Roomote may ignore ambient discussion. Explicit bot mentions, commands, DMs, and replies to
Roomote always receive a response. This applies only inside the established
thread; it does not make Roomote monitor the surrounding channel.
