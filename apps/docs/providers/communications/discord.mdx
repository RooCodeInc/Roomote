...
the `/new`, `/goal`, `/link`, and `/help` commands automatically. You do not need to
...
- use `/new request:<request>` to force a fresh task instead of continuing the
  current one
- use `/goal objective:<objective>` in an active task thread or DM to keep the
  current task working toward that objective across multiple turns; this does
