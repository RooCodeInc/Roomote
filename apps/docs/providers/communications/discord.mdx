...
the `/new`, `/goal`, `/fast`, `/link`, and `/help` commands automatically. You
...
- use `/fast request:<request>` to ask the fast orchestrator a question or
  delegate work into a new task; unlike `/goal`, it does not require an active
  task
- when your deployment exposes **Default messages to fast mode**, enable it
  under **Settings > Personal** to send ordinary Discord DMs, mentions, and
  eligible thread replies through the same fast orchestrator without `/fast`
...
A Fast conversation can run multiple independent tasks at the same time. Start
new work naturally while another task is active. When you want to follow up on
or cancel one of several active tasks, identify it by its title or task ID;
Roomote asks which task you mean when the request is ambiguous. Anyone in a
Fast thread can reply without first mentioning Roomote, although you should
mention the bot again after side conversation so the next message is treated
as an instruction.

Roomote posts useful task updates back to the Fast conversation, including
links when a task opens a pull request. If a sandbox fails to start for a reason
that may be temporary, Roomote can retry it; configuration, authentication,
permission, billing, quota, and other permanent failures are reported without
retrying.
...
starter, or anyone who tagged Roomote earlier); Fast threads are open to any
participant as described above. If somebody else posts or is mentioned after
Roomote's last reply, tag Roomote again so side chat is not treated as a new
instruction. Native forwarded messages contribute their text and supported
attachments to the task context. Image attachments are passed to the task as
image input, and supported documents are downloaded and added to the request.
Attachment downloads are size-limited and restricted to Discord's media hosts.
