...
owner of an established Fast Discord thread can enable it so linked participants'
messages continue reaching Fast after a peer is mentioned. Roomote may ignore
ambient discussion, while explicit bot mentions, commands, DMs, and replies to
Roomote continue to receive a response normally. This applies only inside the
established thread; it does not make Roomote monitor the surrounding channel.
