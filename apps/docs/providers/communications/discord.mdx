...
(including supported attachments, up to Discord's history limits) so the agent
has that background. Image attachments are passed to the task as image input,
and supported documents are downloaded and added to the request. Attachment
