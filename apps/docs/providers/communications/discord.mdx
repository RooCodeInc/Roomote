...
For env-var-only setups, set the same value on the API and BullMQ processes
yourself (installers and platform templates also generate one). Keep it separate
from `ENCRYPTION_KEY` so a leaked transport secret cannot unlock secrets sealed
at rest.
