...
Eligible new Teams task-entry messages from linked users start a Fast session.
Unlinked senders continue to receive the account-link prompt. Existing task and
Fast-session replies keep their current continuation behavior.
