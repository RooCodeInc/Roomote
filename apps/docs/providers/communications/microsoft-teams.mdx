...
only when the model finds it is addressed to Roomote. An acknowledgement that
passes that addressee check reaches the session, which then decides whether a
