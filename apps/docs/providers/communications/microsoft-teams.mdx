...
5. send `/skills` to list callable `$skill-name` tokens; send `/skills 2` to
   continue the list
...
The **Peer conversations** experiment does not apply to Teams. Bot Framework
webhooks provide addressed activity, and delegated Microsoft Graph access can
supply bounded history for eligible thread replies, but Roomote still keeps
each person's session separate. Those signals are not treated as one
shared, owner-bound conversation or as permission to monitor a channel.
