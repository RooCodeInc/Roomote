...
3. confirm Roomote answers in the session and includes a web continuation link
4. reply to the Roomote answer, or reply in the same Roomote-owned thread, and confirm
   the same session continues

Ordinary task-entry messages from linked users start in sessions in personal chats,
...
In an existing session conversation, send `/stop` (or mention the bot with
`/stop` in a channel) to soft-stop every active task linked to that session,
including delegated tasks. The work remains resumable: send another message to
continue later. Only the linked session owner can stop its tasks.

Peer conversations do not apply to Teams. Bot Framework webhooks provide
addressed activity, and delegated Microsoft Graph access can supply bounded
history for eligible thread replies, but Roomote still keeps each person's
session separate. Those signals are not treated as one shared, owner-bound
conversation or as permission to monitor a channel. When a
[judgment model](/models#judgment-model) is configured and more than one
person is in the thread, an eligible unmentioned thread reply still routes
only when the model finds it is addressed to Roomote and more than a closing
acknowledgement.

Replies to Roomote messages continue the bound session. Existing task
conversations keep their active-task and resumable-snapshot behavior, and Roomote
delegates repository work into normal Roomote tasks. If Roomote cannot start a
...
render interactive answer buttons. Session automation reports can target a Teams
channel, chat, or owner direct message. Replies continue the report's
session.
