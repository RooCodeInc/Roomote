...
In an active Roomote task thread, send:

```text
@Roomote goal <objective>
```
...
Slack uses this message command instead of a native slash command so Roomote can
identify the exact task thread. See [Goal Mode](/goal-mode) for examples and
cross-surface usage.
...
6. send `@Roomote goal <objective>` in the thread and confirm Roomote replies
   with **Goal Mode enabled.**
