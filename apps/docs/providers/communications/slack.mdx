...
Fast checks the available conversation and integration evidence before it
answers. It responds directly when that evidence is sufficient, and delegates
repository or workspace inspection, editing, execution, and validation to a
Roomote task. It can also inspect delegated task status, history, transcripts,
and supported compute logs before answering a progress question or routing a
follow-up. If more than one active task could match, Fast asks which task you
