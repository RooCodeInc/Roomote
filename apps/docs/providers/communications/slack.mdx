...
**Fast response mode** is always on: ordinary messages from your linked Slack
and Discord accounts go through the fast orchestrator, which can delegate work
into tasks. `!fast` remains available for an explicit Fast request.
