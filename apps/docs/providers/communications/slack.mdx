...
Send `skills` in a Roomote DM, or mention Roomote followed by `skills`, to list
the skills available to your linked account. Send `skills 2` for the next page.
Start a request with an exact token from the list, such as
`$review-code review these changes`.
