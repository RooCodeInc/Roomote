...
judged before it can start a turn: it routes when the model finds it is
addressed to Roomote. Messages meant for a colleague stay silent;
acknowledgements addressed to Roomote reach the session, which then decides
whether a response is warranted. Without a judgment model, Roomote stays quiet
