...
Roomote asks which task you mean when the request is ambiguous. Anyone in the
Fast thread can reply without first mentioning Roomote, although you should
mention the app again after side conversation so the next message is treated as
an instruction.

Roomote posts useful task updates back to the Fast thread, including links when
a task opens a pull request. If a sandbox fails to start for a reason that may
be temporary, Roomote can retry it; configuration, authentication, permission,
billing, quota, and other permanent failures are reported without retrying.
