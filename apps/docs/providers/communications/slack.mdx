...
Longer Fast turns show Slack's native processing state and return to active when
the reply finishes; quick replies can complete before the indicator appears.
Roomote-generated Fast Session titles and titles you edit in the web app also
sync to the Slack Agent session.
