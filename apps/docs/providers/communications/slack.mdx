...
ensures the bot is a member, and sets it as the **Default destination** under
**Settings > Automations > Default destination**.

An existing default destination and explicit automation
...
Slack account again to retry, or configure the default destination manually.
...
**Peer conversations** under **Settings > Experimental** is off by default. A
deployment admin can enable it so eligible human discussion in an
established Roomote Slack thread continues reaching Roomote after another participant
is mentioned. Roomote stays quiet around peer discussion unless directly
addressed, and direct mentions continue to work normally whether the experiment
is on or off.
