...
opens session responses in threads, and allows Slack to render native loading
states and streamed task cards for tasks delegated by Roomote. These cards stream
coding progress and settle to their final result in the same thread.
...
**Session responses** are always on across Slack, Discord, Microsoft Teams, and
Telegram: ordinary messages from your linked account go through the session
...
job`. Replies in a Roomote-owned Slack thread continue that session; replies
in an existing task-only thread continue the task.

To stop work without discarding it, reply with `stop` in the existing session
thread; in a channel, mention the app with `stop`. Roomote soft-stops every
active task linked to that session, including delegated tasks; the work remains
resumable by sending another message. Only the linked session owner can stop its
tasks.

Roomote can read a bounded history from the current Slack channel, discover other
channels in the same workspace, and post standalone updates to channels where
the Roomote app is authorized. From another session surface, such as Telegram, you
can provide a Slack link or explicitly name Slack to read authorized context,
then ask Roomote to send an update to an explicit workspace and destination.
Roomote uses the same destination lookup and send action for your own linked Slack
account, an explicitly named linked member, or an authorized Slack channel or
thread. People and channel lookups require a targeted search or exact destination,
return bounded pages, and can be limited to one workspace. A self destination is
always resolved from your authenticated identity without enumerating the Slack
directory; it is not a caller-supplied user ID. Public channels require app membership;
private channels also require your linked Slack account to be a member, and
direct messages to another member require workspace-scoped linked identities for
both participants. Reactions remain scoped to the current inbound message. Roomote
can also use MCP servers and user-scoped integrations that you are allowed to
access and delegate repository work to Roomote tasks. Members can
create and manage their own [custom automations](/automations#custom-automations)
through a session; deployment admins can manage all custom automations, including
those without a creator. Built-in automations and deployment settings remain
admin-only. Roomote does not read other
channels implicitly or inspect repository contents without delegating the work
to a task. When the request spans every repository, Roomote can delegate it to the
deployment's all-repositories environment.

Eligible human discussion in an established Roomote Slack thread keeps reaching
Roomote after another participant is mentioned. When a
[judgment model](/models#judgment-model) is configured, every unmentioned
reply from a linked member in a Roomote thread with more than one person is
judged before it can start a turn: it routes when the model finds it is addressed to Roomote and is more
than a closing acknowledgement, so messages meant for a colleague and simple
acknowledgements stay silent. Without a judgment model, Roomote stays quiet
around peer discussion unless directly addressed. Direct mentions always work.

Every Roomote reply ends with a compact footer: **Reply anytime**, a plain
...
4. confirm Roomote answers in the session and includes a web continuation link
5. reply in the same thread and confirm the message continues the same
   session; when Roomote delegates work, confirm the task card stays in that thread
