...
prevent native delivery, Roomote shares an artifact viewer link instead and
reports that the video could not be attached. A viewer link is not a native
Slack attachment.
...
allowed to access and delegate repository work to Roomote tasks. Deployment
admins can manage custom automations through Fast. Fast does not read other
