...
**Fast response mode** is enabled by default. Turn it off under
**Settings > Personal** if you want ordinary messages from your linked Slack
and Discord accounts to start tasks instead of going through the fast
orchestrator. You can still use `!fast` for an explicit Fast request.
