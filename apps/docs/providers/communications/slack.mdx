...
Under **Settings > Experimental**, **Slack peer conversations** lets Fast
observe human-to-human discussion in its existing Slack threads. Fast stays
quiet unless someone addresses it directly, replies to it, or clearly continues
its work. The setting is off by default and applies only to the person who
enables it.
...
the Roomote app is authorized. From another Fast surface, such as Telegram, you
can provide a Slack link or explicitly name Slack to read authorized context,
then ask Fast to post an update to an explicit workspace and destination. Public
...
linked identities. Reactions remain scoped to the current inbound message. Fast
...
**Slack peer conversations** under **Settings > Experimental** is off by
default. A Session owner can enable it so eligible human discussion in an
established Fast Slack thread continues reaching Fast after another participant
