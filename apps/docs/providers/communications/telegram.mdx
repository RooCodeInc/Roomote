...
5. while a task is active, use `/goal <objective>` and confirm Goal Mode starts
...
chat the request joins that chat's conversation. `/goal` keeps an active task
working toward the supplied objective; it does not start a task by itself.
