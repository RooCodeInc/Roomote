...
chat the request joins that chat's conversation. `/goal` makes the Fast Session
own and pursue the supplied objective; Fast may delegate child tasks as needed.
...
Roomote keeps Telegram Fast Sessions isolated per linked sender rather than
...
Fast-delegated coding tasks use one editable live status message with elapsed
...
Fast reply ends with a compact footer: **Reply anytime**, a plain
