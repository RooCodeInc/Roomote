...
registers the Bot API webhook, and adds `/start`, `/help`, `/new`, `/stop`, and `/goal`
to the bot's command menu for you.
...
Roomote replies as conversation input. Roomote does not request
...
2. confirm Roomote answers in the session and includes a web continuation link
3. reply to the Roomote answer, or reply in the same Roomote-owned topic, and confirm
...
5. use `/stop` in a Roomote conversation with active tasks and confirm they stop
   without losing resumability
6. use `/goal <objective>` and confirm the acknowledgement repeats the objective
7. reply in the task topic to send a follow-up

Every message from a linked user enters a session: in private chats
directly, and in groups from bot mentions. Roomote answers directly or delegates
a task into the right environment. A Roomote-owned topic can continue without
another mention, and a direct reply to a Roomote message resumes the bound
...
chat the request joins that chat's conversation. `/goal` makes the session
own and pursue the supplied objective; Roomote may delegate child tasks as needed.
Use `/stop` in a private chat or Roomote topic to soft-stop all active tasks
linked to the current session, including delegated tasks. In a group without a
topic, reply to a Roomote message with `/stop@botname`. The linked owner can
continue the same session later by sending another message.

Peer conversations do not apply to Telegram. Telegram's
privacy settings can prevent ambient group messages from reaching bots, and
Roomote keeps Telegram sessions isolated per linked sender rather than
treating a group or topic as one owner-bound conversation. Roomote does not
claim to observe group messages that Telegram does not deliver.

While a private-chat session turn is running, Telegram shows a non-empty
**Roomote is working...** rich-message draft, then replaces it with rich response
text as generation continues. The completed response is always finalized as a
durable rich message so it remains in the conversation. Roomote keeps activity active across
...
and resumable-snapshot behavior. If Roomote cannot start a conversation, it
says so in the chat instead of starting a task another way. Session automation
reports can target a Telegram chat, topic, or owner direct message; replies
continue the report's session.

Session-delegated coding tasks use one editable live status message with elapsed
time, expandable progress details, and a link to the selected task. When a task
finishes successfully, Telegram leaves its last useful progress update in place
instead of replacing it with a generic **Completed** message. If the task later
resumes, Roomote continues updating the same live message.

Roomote sends all Telegram text replies as rich messages, including
acknowledgements, progress, final replies, and editable live-task status. Every
Markdown-authored reply uses Telegram's native Rich Markdown support, preserving
GitHub Flavored Markdown features where Telegram supports them. Plain text and
Roomote-generated controls use supported Rich HTML embedded in the same Markdown
payload, so sends, edits, and drafts share one formatting path. Each
Roomote reply ends with a compact footer: **Reply anytime**, a plain
...
Telegram renders this through its native rich-message footer presentation and
controls its exact appearance. Rich text replies can remain in one message up to
Telegram's 32,768-character rich-message limit; longer replies are split while
keeping the footer on the final text message.
...
When someone continues a Telegram conversation from the Roomote web app, the
next Telegram reply quotes that authenticated sender and web message above the
response. The quote is also sent before an image-only response. Replies to
messages that originated in Telegram remain unquoted.
...
are described. Roomote can explicitly forward those current-message attachments to
