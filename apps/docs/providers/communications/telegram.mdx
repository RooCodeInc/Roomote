...
registers the Bot API webhook, and adds `/start`, `/help`, `/new`, and `/goal`
...
5. use `/goal <objective>` and confirm the acknowledgement repeats the objective
6. use `/skills` to list callable `$skill-name` tokens; use `/skills 2` to
   continue the list
...
The **Peer conversations** experiment does not apply to Telegram. Telegram's
