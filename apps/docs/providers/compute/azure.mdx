...
# Sandbox size (XS/S/M/L/XL, default M) and egress TLS inspection (default Partial)
AZURE_SANDBOX_SIZE=...
...
The ACA egress proxy can TLS-inspect (MITM) outbound traffic. Roomote creates
sandboxes with `trafficInspection: Partial`, so with no egress rules
configured nothing is inspected: package managers (npm, Maven/Gradle, pip)
see normal public certificate chains, and non-HTTP traffic such as SSH git
works. The service default (`Full`) resigns all TLS traffic with the proxy CA
