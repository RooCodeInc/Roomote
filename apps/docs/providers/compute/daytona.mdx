...
When you connect Daytona from **Settings > Sandboxes**, Roomote provisions the
worker snapshot and saves its name. Environment-only deployments must build or
provision that worker snapshot separately and set `DAYTONA_SNAPSHOT_NAME`.
