...
- **Roomote couldn't find or pull the worker image.** Build or publish the
  worker image configured in `DOCKER_WORKER_IMAGE`. For a source checkout, use
  `pnpm dev`, or build the default local tag directly from the repository root:

  ```sh
  docker build -f apps/worker/Dockerfile -t roomote-worker:local .
  ```

  For a Compose deployment, include `docker-compose.compute-docker.yml` and run
  the documented `docker compose ... up -d --build` command; the `worker-image`
  service builds the configured tag before the controller starts. For a
  deployment that reuses a prebuilt registry-qualified image instead of
  building the source Compose service, run `docker pull "$DOCKER_WORKER_IMAGE"`
  on the controller's Docker host and run `docker login` first when the
  registry is private.
...
- **The worker release archive is missing.** The Docker provider installs the
  worker from a local release archive. For local development, run `pnpm dev`
  without `--use-release` so the local worker-release build step creates the
  archive before starting tasks.
