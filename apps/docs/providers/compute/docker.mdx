...
Roomote attaches only the API and preview proxy to that network; sibling task
containers and datastore containers are not reachable. The controller removes
empty task networks periodically and reaps stale provisioning containers after
a restart.

Workers default to 2 CPUs, 4 GiB of memory with no additional swap, 512 PIDs,
a 20 GiB writable layer, and three 10 MiB JSON log files. The variables above
can tune those bounds. The writable-layer quota requires a Docker storage
driver that supports `--storage-opt size`; worker creation fails closed when
the configured daemon cannot enforce it.
...
public-egress next hop. Local development retains gateway access because its
API runs on the host. The worker never receives the network-administration
capability needed to remove the policy. Set
...
- **A worker fails with a storage option error.** Configure a Docker storage
  driver with per-container writable-layer quota support; Roomote does not run
