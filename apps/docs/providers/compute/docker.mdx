...
Docker is the simplest compute provider for local development and trusted
...
Docker compute requires the Roomote controller to create and manage worker
containers. Treat the Docker host and Docker socket as trusted infrastructure.
Do not use Docker compute as a multi-tenant isolation boundary for untrusted
...
- **Tasks slow down the app.** Docker tasks share the deployment host's CPU,
  memory, disk, and network. Use hosted compute or a larger host for heavier
  workloads.
