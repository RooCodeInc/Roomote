...
Modal is a hosted compute provider for running task sandboxes away from the
...
Add Modal from **Settings > Compute**, or provide the values as deployment env
vars:
...
# Settings > Compute. Comma-separated Modal regions, for example us or us-west.
MODAL_REGIONS=us
```
