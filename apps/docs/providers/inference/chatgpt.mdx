...
The account-level **Default Fast mode** setting is used by per-model choices set
to **Inherit**. In **Settings > Models**, set an eligible model to **Normal** to
keep it on standard processing or **Fast** to request faster processing for that
model. The subscription gateway does not currently expose a reliable response
signal confirming which tier served an individual request.
