...
The endpoint must be reachable from the Roomote deployment. Roomote accepts the
service root or its `/v1` API path, so a Compose deployment can use
`http://lm-studio:1234` or `http://lm-studio:1234/v1` when the endpoint runs on
the same private network. Do not use `localhost` unless the endpoint runs in the
same network namespace as the Roomote service that proxies inference requests.
...
- **No models appear.** Confirm the endpoint URL is the service root or its
  OpenAI `/v1` API path and that the API key can list models when auth is
  required.
