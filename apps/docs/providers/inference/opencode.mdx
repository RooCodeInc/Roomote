...
description: Use OpenCode's routed Zen or Go model service with Roomote.
---

OpenCode Zen and Go provide an OpenCode-managed catalog of coding models behind
one account and API key. Use this provider when you want OpenCode routing and
...
your Zen or Go account. The account must have an active balance or plan that can
...
OpenCode applies the pricing and plan rules shown for Zen or Go and the routed
...
- **A routed model is unavailable.** Check the model's availability and your
  Zen or Go plan, then select another model from the connected catalog.
