...
Add the **OpenCode Zen / Go** option in **Settings > Models** and paste the key,
or set:
...
Roomote exposes routed Zen models under the `opencode/` prefix. OpenCode Go's
separate `opencode-go/` models and endpoint are not currently supported. In
task sandboxes, use Zen models that support the OpenAI-compatible chat
completions endpoint; models that require OpenCode's Anthropic, Gemini, or
Responses API routes are not currently available through Roomote's gateway.
