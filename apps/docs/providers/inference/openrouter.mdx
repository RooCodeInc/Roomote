...
The same connection can route Roomote's optional [judgment model](/models#judgment-model).
Choose **Jev via OpenRouter** under **Judgment model** in **Settings > Models**,
or set `R_JUDGMENT_MODEL=openrouter`. Roomote sends typed decision requests to
OpenRouter's Decisions API using `typesafe/jev-1.13`; the API key stays on the
control plane and is never sent to task sandboxes. Connecting OpenRouter does
not enable Jev automatically.
