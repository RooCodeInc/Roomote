...
A TypeSafe key alone turns on **Jev via TypeSafe**. To use OpenRouter or Vercel
AI Gateway instead, or to turn the judgment model off while keeping the key,
choose the **Judgment model** in **Settings > Models**, or set
`R_JUDGMENT_MODEL` to `typesafe`, `openrouter`, `vercel`, or `off`. The
environment variable overrides the Settings choice.
