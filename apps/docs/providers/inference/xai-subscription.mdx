...
**Grok 4.6** as the default coding model. Roomote then lists every current Grok
chat model from the live provider catalog, so newer Grok releases appear in
**Settings > Models** and the task model switcher without a Roomote update.
Image and video Grok models are omitted. When both subscription and [xAI API
