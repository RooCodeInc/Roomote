...
**Grok 4.6** as the default coding model and lists every current Grok chat
model from the live provider catalog, so newer Grok releases are switchable
without a Roomote update. If both xAI API and a Grok subscription are
connected, Roomote prefers the subscription for `xai/` models.
