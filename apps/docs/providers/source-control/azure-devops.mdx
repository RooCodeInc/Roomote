...
Optional advanced values (self-hosted Azure DevOps, custom Git username,
Azure DevOps OAuth/account linking, or an explicit tenant override):
...
`ADO_TENANT_ID` is an optional override for Azure DevOps OAuth; when unset,
Roomote falls back to `R_MICROSOFT_TENANT_ID` if Microsoft sign-in is
configured. Service hook webhook secrets are generated automatically when
Roomote configures service hooks—operators do not enter them during setup.
Advanced values remain editable later in Settings → Source Control.
