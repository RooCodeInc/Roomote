...
description: Configure Gitea repository sync, pull request webhooks, and Actions failure triage for Roomote.
---
...
```text
<public-url>/api/source-control/gitea/oauth/callback
```
...
Active members and admins can ask Fast to read and merge a pull request in an
active connected Gitea repository without starting a coding task. The bounded
`get_pull_request` and `merge_pull_request` tools require
...
Fast merges only after an explicit request in the current human message. It
