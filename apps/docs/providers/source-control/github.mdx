...
Fast can create a gist owned by the requesting member's linked GitHub account.
This account-level action does not use a repository installation token and does
not require a connected repository. Link GitHub under **Settings -> Linked
Accounts** first. The deployment's GitHub App must have the **Gists: Read and
write** account permission; after an operator adds that permission, members must
reauthorize or reconnect their linked GitHub accounts before creating gists.
...
Gist creation is available in Fast, not from coding-run GitHub tools, and the
result belongs to the authenticated requesting member rather than the Roomote
GitHub App or an installation owner.
