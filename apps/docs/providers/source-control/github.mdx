...
In Fast, ask Roomote to update a pull request's title or body, close or reopen
it, or add, edit, or reply to a pull request comment without starting a coding
task. Specify the repository and pull request number.

You must be an authenticated Roomote member. The repository must be active and
synced, with an active GitHub App installation. These actions use the
deployment's installation credential; they do not require a linked personal
GitHub account or Roomote administrator access. Fast records these tool calls
in its audit trail and reports provider errors or unsupported capabilities.

These tools do not merge pull requests, change branches, write code, or
administer repositories. Existing coding-task permissions are unchanged.
