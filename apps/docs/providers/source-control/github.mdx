...
before the mutation, binds the request to the current head SHA when the native
schema supports it, leaves branch protections, required checks, and merge
permissions to GitHub, and reads the pull request again before reporting
success or retrying an ambiguous result. Auto-merge is only enabled when
