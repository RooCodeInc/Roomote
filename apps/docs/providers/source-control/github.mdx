...
Roomote can use the GitHub App's Actions access to inspect workflow runs, jobs, and
...
Roomote members can inspect any public `github.com` repository in a session without
...
Roomote can use GitHub's native tools directly, without starting a coding task:
...
or change files. A merge or auto-merge request requires an explicit request
from the current human message. Roomote reads the pull request immediately
before the mutation and binds the request to the current head SHA; for
auto-merge, GitHub enforces that binding atomically and rejects the call when
the head has moved. Branch protections, required checks, and merge
permissions stay with GitHub, and Roomote reads the pull request again before
reporting success or retrying an ambiguous result. Auto-merge is only enabled when
GitHub confirms it, and only where the repository allows auto-merge and the
requested merge method. Work that needs a
...
and [Bitbucket Cloud](/providers/source-control/bitbucket#api-first-work-in-sessions)
have their own supported session operations and limits.
