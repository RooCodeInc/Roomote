...
If you add the Gists account permission to an existing app, each member must
reauthorize the GitHub App or reconnect GitHub under **Settings -> Linked
Accounts** before Roomote can create gists for that account.
...
connected to the requested active repository, with one token covering every
repository connected on that installation. Suspended
...
Identify the repository with `owner` and `repo` for repository tools. Code,
...
Fast can use GitHub's native tools directly, without starting a coding task:
pull request and issue edits, comments, replies and reactions, reviews, labels,
branches, and small file changes. Their discovered descriptions and schemas
define the supported arguments. These actions use the deployment GitHub App,
require an active Roomote member, and run under a token that reaches only the
repositories connected on that installation, the same boundary a coding task
has. What the token may do there is set by the App's permissions on GitHub. No
personal GitHub OAuth connection is required. Ask for the specific change and
identify the repository and target issue, pull request, or comment. Every call
appears in the Session transcript with its inputs and result.
...
again before reporting success or retrying an ambiguous result. Work that needs a
checkout, a build, or tests to get right still belongs in a coding task, which
also remains read-only on this GitHub tool path and writes through its own
checkout instead.
...
Fast Sessions and coding tasks can create a gist owned by the acting member's
linked GitHub account, and read or update a gist the member created through
Roomote or shares a link to; GitHub keeps every revision. Roomote never lists a member's
existing gists and never deletes one. Tool discovery requires the deployment's normal eligible
GitHub App installation and active connected repository, but gist creation uses
the member's user access token rather than a repository installation token. Link
GitHub under **Settings -> Linked Accounts** first: gist tools are offered only
to members with a linked account. The deployment's GitHub App
must have the **Gists: Read and write** account permission; after an operator
adds that permission, members must reauthorize or reconnect their linked GitHub
accounts before creating gists.

Gist visibility is always explicit. Roomote uses `public: false` unless the user
explicitly requests public publishing. GitHub calls this a secret gist: anyone
with the URL can access it, so it is link-accessible rather than private. Public
gists are discoverable and should be created only after an explicit request.
The result belongs to the acting member rather than the Roomote GitHub App or an
installation owner. Coding runs without a human actor cannot create gists.
