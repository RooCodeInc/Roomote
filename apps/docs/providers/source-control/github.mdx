...
Read-only requests run directly. Before creating, editing, assigning, or moving
an item, Roomote shows the proposed change and asks for confirmation. Deletes
require a separate explicit confirmation that names the resource. After a write,
Roomote reads the result back and shares its GitHub URL.
