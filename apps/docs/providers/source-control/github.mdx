...
Native reads of unconnected public repositories are limited to 2 MiB of response
...
The anonymous visibility check has a separate 64 KiB limit and 15-second deadline.
These additional limits do not apply to connected repository calls.

Before reading an unconnected target, Roomote checks its public visibility
without credentials. If visibility cannot be confirmed, the read is denied.
