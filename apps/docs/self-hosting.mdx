...
acts on a decision only when the model is confident. To serve a different
checkpoint, set `JUDGMENT_MODEL` to a Hugging Face id or path (and
`JUDGMENT_HF_TOKEN` if it is private).
