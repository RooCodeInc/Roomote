...
- **Everyone** sees a short "what's new" notice once after the app is upgraded
  to a new version. The release dialog reads the changelog bundled with the
  running deployment and lets you browse notes for that release and earlier
  releases. If notes for a newer available release are not bundled yet, the
  dialog links to its GitHub release instead.
