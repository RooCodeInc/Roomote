...
in one structured multi-select question. Choosing at least one completes setup
immediately, then Roomote launches each choice as visible work attached to the
same session. A launch failure does not reopen setup; ask Roomote to retry that
item. Automation recommendations are analyzed after repository sync and appear
only after at least one starter task launches. They remain optional.
