...
<Tip>
  ...
  follow the [automated self-hosting contract](/self-hosting/agent-installation)
  for platform detection, safe confirmation boundaries, installation, and
...
The [agent installation contract](/self-hosting/agent-installation) gives coding
agents a deterministic path from host detection through a verified,
recoverable deployment without replacing the human setup guide.
