...
SSH into a fresh Ubuntu or Debian server (x86_64 or arm64, with 4 GB+ RAM
recommended) and run:
...
```sh
sudo roomote upgrade   # pull and roll out newer images
sudo roomote rollback  # return to the release before the last upgrade
sudo roomote backup    # create an encrypted deployment recovery bundle
sudo roomote logs      # tail service logs
```

`sudo roomote backup` prompts for a passphrase and writes a versioned `.roomote`
...
Use `sudo roomote backup --include-redis` when queued work, BullMQ schedules,
...
`sudo roomote upgrade` is designed so a bad release cannot strand your
deployment:
...
- **Rollback is one command.** Every release's schema keeps the previous
  release working, so `sudo roomote rollback` re-deploys the prior release
  without touching the database. `sudo roomote upgrade <tag>` does the same for
  any retained tag, and restoring the pre-upgrade bundle is the last-resort
  path that also rewinds data.

Use `sudo roomote upgrade` instead of updating application image references
alone. The command refreshes the release's Compose and Caddy configuration
together; mixing newer application images with an older Caddyfile can leave
routes used by the new controller unavailable until the deployment
...
- **Admins** on self-hosted deployments see an update notice in the sidenav when
  ...
  GitHub. Use `sudo roomote upgrade` (or your image roll-forward process) to install
  it.
...
- **Memory climbs for hours, or a service is OOM-killed under a container
  ...
  controller and BullMQ). When a readable cgroup memory limit would make the
  service default too large, the cap is reduced to about 75% of that limit.
  Set `NODE_OPTIONS=--max-old-space-size=<MB>` on a service to override the
  default, or on older images to add the cap manually.
