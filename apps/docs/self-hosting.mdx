...
acts on a decision only when the model is confident. Tool-call auto-approval
still requires Jev. To serve a different checkpoint, set `JUDGMENT_MODEL` to a
Hugging Face id or path (and `JUDGMENT_HF_TOKEN` if it is private).
...
time, so a burst of them queues and more cores make each one faster. On a
host with an NVIDIA GPU, `JUDGMENT_DEVICE=cuda` brings a decision to about
0.5 seconds under load.
...
loading its model until `R_JUDGMENT_UPSTREAM_URL` points at it. Each
template's README has the steps.
