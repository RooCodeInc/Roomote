...
On deployments with optional source-control setup enabled, inference and a
sandbox provider are sufficient to complete setup. Source control can be
connected later in **Settings > Source control** when you need repository work.
Repository starter tasks and repository recommendations still require connected,
synchronized repositories. The default setup flow continues to require source
control; disabling the optional flow does not reopen already completed setup.
