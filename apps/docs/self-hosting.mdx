...
- **Migrations run before services are replaced.** Database migrations apply
  ...
  migration runner retries a transient Postgres connection failure up to three
  times. If migration still fails, the schema rolls back, the previous
  configuration is restored, and the previous release stays up.
