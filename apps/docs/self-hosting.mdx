...
<Tip>
  ...
  follow the [local Docker setup](/self-hosting/agent-installation) to start the
  Compose stack and configure a temporary HTTPS callback URL.
</Tip>
...
## Local Docker setup for agents

The [agent setup page](/self-hosting/agent-installation) is a short Docker
Compose recipe for trying Roomote locally on macOS, Windows, or Linux.
