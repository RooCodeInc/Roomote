...
messages for source control, integrations, first work, sandbox configuration,
and automation recommendations. Detailed provider instructions and credential
entry open in a dialog from the source-control card; Roomote never asks for
credentials in chat. You can connect source control or choose **Not now** at any
point in that card.

After the source-control decision, the conversation asks briefly about the
tools your team uses for documents, monitoring, and project tracking.
...
repositories, Roomote offers starter tasks in one structured multi-select
question after integrations. Choosing **I'll type it myself** completes setup
without a sandbox. Selected starter work waits for a sandbox, then Roomote
