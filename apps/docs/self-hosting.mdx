...
The detailed guide for operating your own server lives in
...
## Mount declarative environments

To provision [environments from definition files](/environments#provision-environments-declaratively),
mount the directory into the `api` service and point
`ROOMOTE_ENVIRONMENTS_DIR` at the container path:
...
Only the API service reads these files. Restart Roomote after changing them.
Definitions are skipped until all repositories they reference are linked.
