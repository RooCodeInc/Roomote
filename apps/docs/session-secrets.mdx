...
description: "Approve a key for one service in a Session so agents can use it without ever seeing it."
---

Session secrets let a Roomote agent use a key you hold, for one HTTPS service,
without the key ever entering chat, a prompt, a task environment, or a
transcript. You enter the key once in the Session; the agent works with an
opaque reference or a substitute token, and Roomote's API adds the real key on
the way out. They are not a general credential vault and not a replacement
for [integrations](/integrations).

<Note>
  Session secrets are gated per user. Turn on **Session secret tools** under
...
2. The agent reads the service's documentation and prepares an approval with
   ...
   `https://api.stripe.com`), the header that carries the key, the HTTP methods
   the work needs (read-only by default), and a lifetime. It replies with a
   secure link into the Session.
3. You open the link and enter the key in the **Session secrets** panel. The
   form bypasses chat and is excluded from capture and replay. If the agent
   asked for write methods, the form shows them and you confirm them.
4. The Session resumes on its own. From then until the approval expires or you
   revoke it from the Session header, agents in that Session can use the key
   within its policy.

Tick **Save integration** on the form to keep the key for every Session you
own instead of this one; see [Saved integrations](#saved-integrations).

An approval covers one origin. Nothing the agent sends can change the origin,
widen the methods, or extend the lifetime.
...
An approval is normally bound to the Session it was made in and disappears
with it. If you expect to use the same service again, tick **Save integration**
when you enter the key. The approval then belongs to your account: every
Session you own, and every coding task launched from one, sees it as a ready
reference with the same origin, header, and method policy, so the agent never
has to prepare it again.

Saved integrations are listed under **Settings → Personal → Saved
integrations**, where you can revoke them at any time. They still expire on
the lifetime the approval was prepared with, and they stay private to you:
collaborators on a shared task never receive them.
...
approval, and the task's current generation, and it stops working when the
task ends, the approval expires or is revoked, or you turn Session secret tools
off. Within that window, a copy of the substitute could be used from elsewhere
...
- One origin per approval; paths under it are not restricted beyond the
  approved methods.
...
No configuration is required. Optionally set `R_SESSION_EGRESS_PROXY_HOST` on
