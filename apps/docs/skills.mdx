...
Every Fast turn starts with the names and descriptions of your instance skills
...
demand: Fast sees which marketplace sources each environment installs and looks
them up when one seems relevant. When you ask Fast which skills are available,
...
You can also ask Fast to update an instance skill you created. Fast reloads the
...
- **Fast mode sees instance and inline environment skills on every turn** and
  ...
  repository skill Markdown only from repositories in configured environments. If the workflow needs a workspace, Fast starts a task in a
...
- **Roomote does not use the skill.** Make the description more specific: Fast
