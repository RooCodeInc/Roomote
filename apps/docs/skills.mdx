...
Every session turn starts with a bounded inventory of the packaged, instance,
authorized environment, marketplace, and repository skills available to you.
Each entry includes its scope so Roomote can recognize a matching playbook from
your request without guessing an environment or repository first. When the
inventory reaches its marketplace or repository discovery limit, Roomote reports
the omission and `list_skills` can inspect additional source pages or an exact
authorized scope.
When you ask Roomote which skills are available, its unscoped inventory includes
the same packaged, instance, authorized environment, marketplace, and repository
sources. Repository-defined skills still apply only when work runs in their
authorized repository or environment scope.

The packaged `roomote-partnership` skill helps Roomote collaborate with an
external MCP client during joint investigations, design or prompt reviews, and
sustained discussions. It is supplemental guidance for that exchange, not a
routine task-dispatch workflow, self-contained local work procedure, or source
of authorization.
...
- **Sessions include the bounded authorized skill inventory on every turn** and
  can discover additional source pages before starting a task. It can load
  instance skills directly, environment skills only from authorized
  environments, and it reads repository skill Markdown only from repositories
  in configured environments. If the workflow needs a workspace, Roomote starts
  a task in a matching environment and the task loads the environment-scoped or
  checked-out copy of the skill.
