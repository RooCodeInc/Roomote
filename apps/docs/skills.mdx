...
Admins can expand **Marketplace & environment skills** below the list when
environments or existing installations are present. Marketplace search and
environment-only creation and management remain there. These skills stay attached to selected environments. They are
not automatically migrated, copied, or removed when an instance skill is created.
...
| Environment skill | Admins manage custom or marketplace skills in Settings | Selected environments only                                             |
| Repository skill  | Maintain a skill in the repository                    | The authorized repository or environment scope                          |

Instance skills do not have an environment selection. Environment skills keep
their existing availability controls.
