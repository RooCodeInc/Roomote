...
When a task started from the web completes, fails, or is canceled while its
initiator is away from the Session, Roomote sends a personal notification with
a link back to the task. Notifications go to the initiator's linked Slack,
Teams, Telegram, and Discord chats where available; verified email is used only
when no chat delivery succeeds. Roomote skips this notification while the
