...
Switch between **Mine** and **Everyone** to focus on your work or the whole
deployment. Each card shows relevant work-type and source badges, creator and
participant avatars, recent activity, and available workspace or pull-request
context. The Done column keeps the six most recent completed tasks so finished
work does not overwhelm active work. Board and list choices are stored in the
URL, which makes filtered views shareable and preserves them when you return.
