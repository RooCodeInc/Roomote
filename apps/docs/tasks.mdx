...
HTML and XHTML artifacts open in a sandboxed preview by default. Use the
**Preview** and **Code** control in the artifact viewer to switch between the
rendered result and its source.
...
transcript. When an agent provides a text fallback, Slack and Discord receive it
as a chat preview with a link to the rendered widget; Teams and Telegram receive
the fallback text.
