...
When a web Session has a new response or a task needs input while its initiator
is away, Roomote sends a personal notification with a link back to the Session.
The notification includes Roomote's response and goes to the first successful
personal destination in this order: Slack, Teams, Telegram, Discord, then
verified email. Later notifications for the same Session reuse that provider
thread when it remains available. Roomote skips the notification while the
initiator is present in the Session.
