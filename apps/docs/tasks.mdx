...
to two times in the same task. If those attempts fail, select **Retry** on the
failed startup message to start another run immediately without creating a new
task. Explicit retries remain available after the automatic retry budget is
exhausted, as long as the failed run is still eligible for startup recovery.
...
Roomote can also share a standalone link that opens an artifact without first
opening its parent task or session. The link opens the exact artifact version,
keeps the normal task or session access checks, and provides a link back to the
source workspace.
...
When visual proof kept screenshots or recordings, a separate reviewer opens
them and checks that they show the shipped change before the task reports back.
