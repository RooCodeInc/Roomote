...
Notifications go to the initiator's linked Slack, Teams, Telegram, and Discord
chats where available; verified email is used only when no chat delivery
succeeds. Roomote skips the notification while the initiator is present in the
Session.
