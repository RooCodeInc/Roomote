...
the standard Session footer. It goes to the first successful personal
destination in this order: Slack, Teams, Telegram, Discord, then verified email.
Later notifications reuse that provider thread when it remains available, and
the standard footer moves to the latest supported message. Roomote skips the
