...
Keys are verified offline; usage reporting does not activate or validate a key.
Licensed deployments send their license ID and current user count to Roomote's
Ping service for subscription billing, even when anonymous analytics is
disabled. The rest of the instance report remains controlled by the anonymous
analytics setting.
