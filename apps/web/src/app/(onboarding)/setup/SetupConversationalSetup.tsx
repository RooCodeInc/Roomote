...
export function SetupConversationalSetup() {
  ...
  const rail = statusQuery.data?.rail;
  ...
  const timelineExtras = (
    <div className="space-y-4">
      ...
      {sourceControlConnected && sessionIdValue ? (
        <SetupRecommendationsInlineCard sessionId={sessionIdValue} />
      ) : null}
    </div>
  );
  ...
  return (
    ...
    <div className="relative left-[calc(-50vw+50%)] flex h-[calc(var(--effective-viewport-height)-8rem)] w-screen flex-col gap-4 px-2 lg:flex-row">
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-2xl border border-border bg-background">
        {rail ? (
          <ol className="flex flex-wrap gap-x-4 gap-y-1 border-b px-4 py-2 text-xs text-muted-foreground">
            {[
              ['Account', rail.account],
              ['Inference', rail.inference],
              ['Compute', rail.compute],
              ['Source', rail.source],
              ['First work', rail.firstWork],
            ].map(([label, state]) => (
      ...
      {sourceControlSetup && !sourceControlConnected && sessionIdValue ? (
