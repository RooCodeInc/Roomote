...
describe('useCloudSession', () => {
  ...
  it('refreshes the session before fetching a token for the latest cloud job id', async () => {
    ...
    const { result } = renderHook(() => useCloudSession('task-1'));
    ...
    expect(fetchQueryMock).toHaveBeenCalledWith(
      expect.objectContaining({
        queryKey: ['auth.sandboxToken', { cloudJobId: 'job-new' }],
        staleTime: 0,
  ...
  it('keeps refreshConnection stable across rerenders', () => {
    ...
    const { result, rerender } = renderHook(() => useCloudSession('task-1'));
