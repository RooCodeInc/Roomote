...
import type { TaskMessageEnvelopePage } from '@/types';

import type { SandboxConnectionTarget } from './use-task-session';
import type { createSandboxStore } from './use-sandbox-store';
import { updateTaskMessageEnvelopePage } from './use-task-message-envelopes';
      ...
      queryClient.setQueryData<TaskMessageEnvelopePage | undefined>(
        ...
        (current) =>
          updateTaskMessageEnvelopePage(current, (messages) =>
            replaceOptimisticPromptEnvelope(messages, taskId, event),
          ),
      );
    };
