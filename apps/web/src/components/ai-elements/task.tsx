...
import { ChevronDownIcon, SearchIcon } from '@/components/system';
...
export const TaskTrigger = ({
  ...
  <CollapsibleTrigger
    asChild
    className={cn('group/task-trigger', className)}
    {...props}
  >
    {children ?? (
      <div className="flex w-full cursor-pointer items-center gap-2 text-muted-foreground text-sm transition-colors hover:text-foreground">
        ...
        <ChevronDownIcon className="size-4 transition-transform group-data-[state=open]/task-trigger:rotate-180" />
      </div>
    )}
  </CollapsibleTrigger>
);
