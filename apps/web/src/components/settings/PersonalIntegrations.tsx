...
const readOnly: SessionEgressMethod[] = ['GET', 'HEAD'];

function describeExpiry(secret: SessionSecretMetadata) {
  ...
  return (
    ...
    >
      <p className="text-sm text-muted-foreground">
        ...
        key. Add one here, or approve it when an agent asks in a Session.
      </p>
      {error ? (
...
function AddIntegrationForm({ onSaved }: { onSaved: () => Promise<void> }) {
  ...
  const [methods, setMethods] = useState<SessionEgressMethod[]>(readOnly);
  return (
    <form
      ref={formRef}
      className="space-y-3"
      ...
      <fieldset disabled={busy} className="space-y-3">
        ...
        <fieldset className="space-y-1">
          ...
          <div className="flex flex-wrap gap-3">
            {SESSION_EGRESS_METHODS.map((method) => (
