...
3. Fix the root cause:
   - For environment-definition problems (commands, env vars, port settings, services, docker projects), prepare the smallest revised definition and call the ${PRODUCT_NAME} MCP tool \`manage_environments\` with \`action: "preview"\` and \`environmentId: "${input.environmentId}"\`. Tell the user the exact changes, maximum configured setup time, and disruption impact, then use \`request_user_input\` to ask whether to proceed. Only after an affirmative response, call \`action: "update"\`. Keep every other environment setting unchanged.
