...
3. Fix the root cause:
   - For environment-definition problems (commands, env vars, port settings, services, docker projects), prepare the smallest revised definition and call the ${PRODUCT_NAME} MCP tool \`manage_environments\` with \`action: "preview"\` and \`environmentId: "${input.environmentId}"\`. Explain the exact changes, maximum configured setup time, and disruption impact, then use \`request_user_input\` for explicit approval. Only after approval, call \`action: "update"\` with the exact preview hash as \`approvedProposalHash\`. Keep every other environment setting unchanged. A material revision requires a new preview and approval.
