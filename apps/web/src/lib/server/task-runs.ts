 ...
 * task_pull_requests.
 */
export const getLatestTaskRunsByTaskId = async (
  taskIds: string[],
): Promise<Record<string, SimpleTaskRun>> => {
  if (taskIds.length === 0) {
    return {};
  }
  ...
  // DESC id ordering).
  const latestByTask = new Map<string, SimpleTaskRun>();
 ...
 */
export const findActiveSuccessorTaskRun = async (
  sourceRunId: number,
  _auth: { userId: string },
  taskId?: string | null,
  ...
  const jobFilter = taskId
    ? and(eq(taskRuns.taskId, taskId), not(eq(taskRuns.id, sourceRunId)))
    : eq(taskRuns.sourceRunId, sourceRunId);
