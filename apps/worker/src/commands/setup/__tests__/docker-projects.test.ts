...
describe('initializeDockerProjects', () => {
  ...
  it('starts Blaxel projects without wait when only WORKER_TARGET is set', async () => {
    await fs.writeFile(
      path.join(repositoryPath, 'compose.yaml'),
      'services:\n  web:\n    image: nginx:alpine\n',
    );
    ...
    await initializeDockerProjects(
      logger,
      {
        ...
        envVars: { WORKER_TARGET: 'blaxel', PATH: '/usr/bin' },
        taskRunType: TaskPayloadKind.StandardTask,
      },
      {
        workspacePath,
  ...
  it('starts without wait when WORKER_TARGET is blaxel even if COMPUTE_PROVIDER differs', async () => {
    await fs.writeFile(
      path.join(repositoryPath, 'compose.yaml'),
      'services:\n  web:\n    image: nginx:alpine\n',
    );
