...
describe('SLACK_SILENCE_HOOK_SCRIPT', () => {
  ...
  const reminder =
    'Slack has not received a visible update for this turn. Your next action must be a Slack-visible update to the originating thread using send_chat_reply or use send_chat_reaction_emoji only when the latest user turn itself came from Slack and that message can receive a reaction. Use request_user_input only when you genuinely require structured input from the user. Normal assistant messages do not count. Do not run more tools first. The only exception is tool_search when the needed Slack reply/post tool is not visible. After sending the Slack update, continue the work you were doing.';
  ...
  it.each([
    'roomote_post_to_slack_channel',
    'roomote_add_reaction_to_slack_message',
  ])('rejects %s from late-bound automation tasks', (toolName) => {
    const stateFilePath = writeState({
      startedAtMs: Date.now(),
      currentTurnRequiresInitialAck: false,
      requiresTerminalCloseoutWithoutTurn: true,
