 ...
 * Roomote shared env file can still contain Vertex credentials. Point
 * BASH_ENV at an overlay that sources the shared file first, then explicitly
 * removes the disabled provider's credential again.
 */
async function materializeOpenCodeBashEnvOverlay(options: {
  ...
  await fs.writeFile(
    ...
    [
      ...
      `unset ${GOOGLE_APPLICATION_CREDENTIALS_ENV_VAR_NAME}`,
      '',
    ].join('\n'),
    { mode: 0o600 },
  );
