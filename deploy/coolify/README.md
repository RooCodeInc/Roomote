...
| `judgment`      | `roomote-judgment:develop` + volume | no (internal only) | image healthcheck        |
...
Decisions run one at a time, so more CPUs make each one faster. Tool-call
auto-approval still requires Jev. Delete the service and its volume if the
deployment should not run one.
