import jwt from 'jsonwebtoken';
import { z } from 'zod';

import {
  type JobTokenPayload,
  type JobTokenContext,
  jobTokenPayloadSchema,
} from '@roomote/types';

import {
  getJobAuthPrivateKey,
  getJobAuthPublicKey,
  isAuthClientTestEnv,
} from './client-runtime';

const ISSUER = 'rcc';

export const createJobTokenOptionsSchema = z.object({
  cloudJobId: z.number(),
  userId: z.string(),
  timeoutMs: z.number(),
});

export type CreateJobTokenOptions = z.infer<typeof createJobTokenOptionsSchema>;

interface ValidateJobTokenOptions {
  ignoreExpiration?: boolean;
}

export async function createJobToken({
  cloudJobId,
  userId,
  timeoutMs,
}: CreateJobTokenOptions): Promise<string> {
  const now = Math.floor(Date.now() / 1000);
  const gracePeriod = 5 * 60; // 5 minutes
  const clockSkewGrace = 30; // 30 seconds

  const payload: JobTokenPayload = {
    iss: ISSUER,
    sub: cloudJobId.toString(),
    exp: now + Math.floor(timeoutMs / 1000) + gracePeriod,
    iat: now,
    nbf: now - clockSkewGrace,
    v: 1,
    r: {
      u: userId,
      t: 'cj',
    },
  };

  const privateKey = Buffer.from(getJobAuthPrivateKey(), 'base64').toString(
    'utf-8',
  );

  return jwt.sign(payload, privateKey, { algorithm: 'ES256' });
}

export async function validateJobToken(
  token: string,
  options: ValidateJobTokenOptions = {},
): Promise<JobTokenContext> {
  const publicKey = Buffer.from(getJobAuthPublicKey(), 'base64').toString(
    'utf-8',
  );

  const rawPayload = jwt.verify(token, publicKey, {
    algorithms: ['ES256'],
    clockTolerance: 60,
    ignoreExpiration: options.ignoreExpiration === true,
    ignoreNotBefore: isAuthClientTestEnv(),
    issuer: ISSUER,
  });

  const parseResult = jobTokenPayloadSchema.safeParse(rawPayload);

  if (!parseResult.success) {
    const validationErrors = parseResult.error.errors
      .map((err) => `${err.path.join('.')}: ${err.message}`)
      .join(', ');

    throw new Error(`Invalid job token structure: ${validationErrors}`);
  }

  const payload = parseResult.data;

  return {
    cloudJobId: Number(payload.sub),
    userId: payload.r.u,
    tokenType: payload.r.t,
    version: payload.v,
  };
}
