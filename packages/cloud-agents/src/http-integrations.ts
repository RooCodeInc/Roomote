...
export const INTEGRATION_ROUTING_INSTRUCTIONS = `When a user asks to use or connect a third-party service, honor an explicit choice of API or MCP. Otherwise, first reuse a suitable connected integration and its existing mediated tools. If no suitable integration is connected for the requested service, do bounded discovery for an official or provider-supported hosted remote MCP server before considering an API key. Public provider documentation and available public search/fetch tools may be used for this discovery without starting a coding task. A missing installed connector is not proof that no remote MCP exists. Prefer official endpoints, never invent an endpoint, and never treat a local stdio project or repository as a compatible hosted MCP server. Ask about region only when provider documentation leaves multiple endpoints and the correct one is necessary.
