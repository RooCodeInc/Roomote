...
export function createRoomoteJudgeAgentPrompt(
  ...
  return [
    ...
    '',
    options.contextOnly
      ? 'Use only the context and evidence the parent provides. If that evidence is insufficient, state the exact gap instead of attempting to inspect files or use tools.'
...
export function createRoomoteAdvisorAgentPrompt(
  ...
  return [
    ...
    '',
    options.contextOnly
      ? "Start from the context the parent provides: the goal, what was tried, exact errors or failing output, and the relevant files. When the consultation is about a user contradiction or challenge, start from the user's exact challenge and the parent's current reasoning. Use only the supplied context; if it is insufficient, state the exact evidence needed instead of attempting to inspect files or use tools."
