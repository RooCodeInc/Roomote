// pnpm --filter @roomote/cloud-agents test src/server/__tests__/cloud-job-queue.test.ts
import Redis from 'ioredis-mock';

import { TASK_TIMEOUT_MS } from '@roomote/types';

import { CloudJobQueue, type CloudJobQueueEntry } from '../cloud-job-queue';
...
function createTestQueue(timeout = 1) {
  ...
  return { queue: new CloudJobQueue({ redis, timeout }), redis };
}

function createTestEntry(id: number, scope: string): CloudJobQueueEntry {
  return { id, scope };
}

describe('CloudJobQueue - Basic Operations', () => {
  let queue: CloudJobQueue;
...
describe('CloudJobQueue - Scope-Based Locking', () => {
  let queue: CloudJobQueue;
  let redis: InstanceType<typeof Redis>;
  ...
  describe('lock collision handling', () => {
    it('should re-enqueue entry when lock acquisition fails', async () => {
      ...
      const queueLength = await redis.llen('queue:cloud-jobs');
...
describe('CloudJobQueue - Lock Management', () => {
  let queue: CloudJobQueue;
  let redis: InstanceType<typeof Redis>;
...
describe('CloudJobQueue - Edge Cases', () => {
  let queue: CloudJobQueue;
  let redis: InstanceType<typeof Redis>;
  ...
  describe('invalid data handling', () => {
    it('should handle invalid JSON gracefully', async () => {
      ...
      await redis.rpush('queue:cloud-jobs', 'invalid-json{');
      await redis.rpush(
        'queue:cloud-jobs',
    ...
    it('should skip entries with missing scope', async () => {
      await redis.rpush('queue:cloud-jobs', JSON.stringify({ id: 999 }));
      await redis.rpush(
        'queue:cloud-jobs',
        JSON.stringify(createTestEntry(1, 'valid')),
      );
    ...
    it('should handle entries with extra properties', async () => {
      ...
      await redis.rpush('queue:cloud-jobs', JSON.stringify(entryWithExtra));
  ...
  describe('retry behavior', () => {
    it('should detect and skip duplicate entries', async () => {
      ...
      await redis.rpush('queue:cloud-jobs', entryJson);
      await redis.rpush('queue:cloud-jobs', entryJson);
...
describe('CloudJobQueue - Re-enqueue Behavior', () => {
  let queue: CloudJobQueue;
  let redis: InstanceType<typeof Redis>;
...
describe('CloudJobQueue - Scope Deduplication', () => {
  let queue: CloudJobQueue;
  let redis: InstanceType<typeof Redis>;
