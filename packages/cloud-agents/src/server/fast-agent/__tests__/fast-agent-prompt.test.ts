  ...
  it.each(['human', 'automation', 'scheduled_wakeup'] as const)(
    ...
    (turn) => {
      ...
      for (const guidance of [
        ...
        'filter on the merge timestamp (`is:pr is:merged merged:<window>`), not `updated:`',
        '`author:<login>` only finds PRs GitHub records as personally authored',
        '`assignee:<login>` and `in:body "Opened on behalf of @<login>"`',
        'verify the attribution in each result, and deduplicate the combined results',
