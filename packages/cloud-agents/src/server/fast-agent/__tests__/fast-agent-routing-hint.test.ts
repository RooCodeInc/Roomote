...
describe('resolveFastAgentRoutingHint', () => {
  ...
  it('returns a hint naming a confidently chosen environment', async () => {
    ...
    expect(hint?.context).toBe(
      'Routing hint: api [id: env-api] looks like the best environment (judgment model confidence 0.82). Verify against the configured routing rules before delegating, and ask when the request is still ambiguous.',
    );
  });
