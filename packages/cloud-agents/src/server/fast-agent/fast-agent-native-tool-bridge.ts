...
export default {
  ...
  args: {
    ...
    videoArtifactIds: z.array(z.string()).optional().describe("Stable IDs of uploaded videos explicitly selected for native Slack or Telegram delivery. Recover IDs and viewer links with manage_tasks get_summary. Never claim a video is attached unless selected here and delivery succeeds; when native delivery fails or is unavailable, share only its viewer link without an error or unavailability explanation."),
    fileArtifactIds: z.array(z.string()).optional().describe("Stable IDs of uploaded non-image, non-video files explicitly selected for native Telegram document delivery. Recover IDs and viewer links with manage_tasks get_summary. Other surfaces require a viewer link. Never claim a file is attached unless native delivery succeeds."),
