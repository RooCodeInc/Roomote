...
export default {
  description: "Add or reconnect one deployment-shared remote MCP integration from a verified HTTPS streamable-HTTP endpoint. Use this after bounded discovery identifies an official or provider-supported hosted endpoint, or when an administrator explicitly provides one; never invent a URL or use a local stdio project. The server verifies the endpoint before saving it, reuses an existing matching integration, and returns either connected tools, a secure OAuth authorization link, or the existing Settings link for static headers/manual OAuth client setup. Authorization-required and client-registration-required results are pending setup, not absence of MCP and not permission to create an API-key fallback. Server-backed results include integrationId, the actual Fast catalog ID: use that exact integrationId with find_integration_tools and call_integration_tool, never a server UUID, but do not narrate IDs or catalog checks to the human. Share authorizeUrl and settingsUrl exactly unchanged, labeled 'Authorize <name>' and 'Integration settings' respectively; never rewrite either target to /settings. The conversation resumes automatically after the requesting administrator authorizes, so never ask them to send a follow-up. In user-visible progress say at most that you are checking. Never ask for or accept secrets in chat or tool arguments.",
  args: {
...
export default {
  description: "List custom key-based HTTPS API integrations only after the integration routing policy selects that fallback or the user explicitly chooses it. Returns integrations available to this human (their own and deployment-visible grants, with origin, header, allowed methods, visibility, and expiry) and this Session's pending approvals, plus sessionUrl, the secure link where the human enters a key, without exposing credentials. Call this before preparing a new approval; for a pending approval, re-share sessionUrl rather than preparing again, and never ask the human to copy an opaque reference. A missing connected connector is not proof that no compatible remote MCP exists, and pending remote-MCP authorization or manual registration must not trigger an API approval. Ready integrations are delivered automatically to coding tasks launched from this Session.",
  args: {},
