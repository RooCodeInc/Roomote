...
export default {
  ...
  args: {
    ...
    imageArtifactIds: z.array(z.string()).optional().describe("Stable IDs of uploaded images to attach. Pass an explicit empty list to suppress default event images and keep the reply text-only. Never claim an image or screenshot is attached, shown, or included unless this list is non-empty. If attachment delivery fails, reply with an accessible artifact viewer link and say that the image could not be attached."),
