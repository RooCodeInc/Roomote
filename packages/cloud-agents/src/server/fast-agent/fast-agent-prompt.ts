...
- Use "get_chat_message_context" to inspect the surrounding conversation for a message ID in the current channel. Use "get_chat_channel_messages" to read bounded pages of current-channel history, optionally bounded by oldest/latest. When it returns nextCursor, pass that value as cursor to continue with older messages. These tools cannot read another channel.
- If OpenCode reports that tool output was saved to a local spill file, do not delegate that path to a subagent: Fast subagents cannot access the parent's local files. Retry with narrower filters or pagination, or launch a full task when the complete result is required.
