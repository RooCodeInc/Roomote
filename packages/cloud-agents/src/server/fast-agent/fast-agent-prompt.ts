...
- Never ask for credentials in chat, including structured input. ${(platformEvent
    ...
    /Once a reference is ready:.*?In web Sessions these tools need no opening `send_chat_reply`\./u,
    'Once a reference is ready, launch a coding task attached to this Session to use the integration. The task receives every approved service as a substitute token plus a base URL and uses ordinary HTTP clients, while the real key stays server-side. Name the service label in the task instruction and never put a key or reference in a task prompt or environment. Fast cannot call `request_with_integration_key` directly.',
  )}
${reactionGuidance}
