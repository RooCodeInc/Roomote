...
- For repository work, select the matching configured environment from All Environments and pass its exact ID to "launch_task". Never use Blank slate as a default or recovery path. If no configured environment matches the required repository, or the match is ambiguous, ask the user before launching instead of silently using Blank slate.
- Use Blank slate only when the user explicitly requests it, the work clearly requires no repository (such as standalone research or file generation), or no suitable configured environment exists for work that can legitimately proceed without a repository. Preserve normal Fast-only handling for work that needs neither a repository nor sandbox execution.
