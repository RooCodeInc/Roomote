...
import { INTEGRATION_ROUTING_INSTRUCTIONS } from '../../http-integrations';
...
- Never ask for credentials in chat, including structured input. ${
    platformEvent
      ...
      : serviceCredentialToolsEnabled
        ? 'Only after the integration routing policy selects the custom HTTPS API fallback, call `list_integration_keys`: a ready reference is usable here, while a pending approval means the human still has to enter it, so re-share its `sessionUrl` instead of preparing again. If nothing exists, verify the service\'s exact HTTPS API origin and credential header from available documentation; never guess either one or default blindly to `Authorization: Bearer`. Then call `prepare_integration_key` with only a label, origin, header name, optional scheme prefix, and the exact HTTP methods the work needs (omit for read-only); omit the lifetime unless the human asked for a temporary key. New approvals default to everyone in the deployment; use owner visibility only when the human asked to keep it private, and the approval form still lets them choose. Share the returned secure link so the human enters the key privately. Label that link with the service, for example "Connect Figma securely", never with words like reference, secret, or credential. If the service later rejects a saved key (401 or 403), prepare a fresh approval with the same policy before sharing the link again, and tell the human the old key can be revoked under Settings → Integrations. Never tell the human to enable the Integration keys setting while these tools are available to you. Never ask for the key in chat or ask the human to copy a reference. Preparation is not approval. Once a reference is ready, launch a coding task attached to this Session to use it for scripts or many calls; name only the service label in the task instruction, never a key or reference. Never invent a reference or substitute another credential. In web Sessions these tools need no opening `send_chat_reply`.'
        : 'Integration-key tools are turned off for this user. Follow the integration routing policy first. Only when it selects a key-based HTTPS API fallback, say that this route needs an integration key, which the human can turn on under Settings → Experimental ("Integration keys"); do not treat an MCP authorization denial or pending MCP setup as a reason to use this fallback.'
  }
${reactionGuidance}
