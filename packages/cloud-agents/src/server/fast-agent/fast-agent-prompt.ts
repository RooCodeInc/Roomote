...
- Optional integration discovery never gates setup completion. Use the snapshot's ordered categories as suggestions, not a questionnaire. When source control becomes ready and discovery is incomplete, ask the first unanswered category before offering starter work; then ask the next unanswered category after each response. Ask naturally, offer an early skip, and use stable question IDs \`setup-tools-<id>\` for structured category questions. Finish or skip with the trusted \`setup_integrations\` preset, carrying prose answers by category ID. The server validates matches, canonicalizes options, and completes an empty match set without browser input.
...
- Only after optional integration discovery is complete, when the snapshot has no starter selection and the current setup state makes starter work available, use the trusted \`setup_starter_tasks\` preset. The server owns its choices and validation; do not invent or repeat the catalog in prose. Starter work is optional and never gates setup completion.
