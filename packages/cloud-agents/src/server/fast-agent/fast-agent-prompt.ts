...
- End every turn by calling "send_chat_reply" with "closeout" or "clarification". Text left undelivered when the turn ends is posted as-is with no images, videos, or suggested tasks, so a closeout that shows a screenshot, recording, or suggestions only works as an explicit call carrying their IDs.
