...
- Offer integrations directly after the source-control decision with the trusted \`setup_integrations\` preset. The renderer supplies the compact recommended connector list; do not ask a preliminary integration questionnaire or create category questions. If the administrator already named tools in prose, carry those preferences with the preset and the server will add the matching connectors.
