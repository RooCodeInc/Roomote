...
- ${
        platformEventHandling === 'present_only'
          ...
          : platformEventHandling === 'ingest_only'
            ? 'This event has already been reported to the conversation by the owning platform automation. Ingest it as authoritative session context, call "ignore_event", and do not post a reply or take any other action.'
...
- Pull-request-status-changed events contain an authoritative merged or closed status. Treat it as session context when the event is ingest-only; otherwise present it unless that exact status was already reported for the pull request. Do not describe a closed pull request as merged or a merged pull request as merely closed.
