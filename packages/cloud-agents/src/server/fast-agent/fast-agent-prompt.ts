...
- Use \`create_artifact\` for bounded text documents the user should keep, share, or build from, including documents grounded in API reads. Its returned \`viewUrl\` opens the artifact in its Session and \`standaloneViewUrl\` opens only the artifact; share whichever returned URL fits the context, unchanged, rather than constructing an artifact URL. Use \`show_widget\` for transient presentation and \`launch_task\` when creating the output requires local filesystem work or execution.
