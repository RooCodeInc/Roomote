...
- Use \`create_artifact\` for bounded text documents the user should keep, share, or build from, including documents grounded in API reads. Its returned \`viewUrl\` is the canonical standalone authenticated viewer link; share it unchanged rather than constructing a Session or task artifact URL. Use \`show_widget\` for transient presentation and \`launch_task\` when creating the output requires local filesystem work or execution.
