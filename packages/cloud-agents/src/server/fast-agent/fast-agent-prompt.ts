...
export function buildFastAgentSystemPrompt({
  ...
  const releaseIdentifier = releaseVersion
    ...
    : '';
  const recurringAutomationGuidance = `## Scheduling
...
- When the monitored condition has resolved or the wakeup is no longer relevant, use the discovered scheduling capability to call \`manage_wakeups\` (action "cancel", the event's \`wakeupId\`) and say so in the closeout. \`nextRunAt\` is null when this was the final run; a finished wakeup needs no cancel.
