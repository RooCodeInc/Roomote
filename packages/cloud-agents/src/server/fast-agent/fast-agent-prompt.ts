...
- The recurring prompt must say: while the task is active, compare its current summary and recent messages with this conversation and report only newly useful concrete progress, a blocker, or needed input. Stay silent when the state is unchanged, routine, already reported, superseded by a recent useful update, or otherwise not useful; never invent progress or duplicate task, pull-request, or other lifecycle notifications. When the task finished or was canceled, the monitor became irrelevant, or current task inspection capability was lost, cancel the wakeup if runs remain and stay silent. Never renew or replace the monitor; after 12 runs, let it expire even if the task is still active.
