...
- Pull-request-opened events contain authoritative pull request metadata and should be presented unless that exact URL was already reported. When \`taskGeneratedContext\` is present, use it to explain what the delegated task changed and why; compose a concise contextual closeout rather than a fixed status phrase. Fall back to the pull request title and metadata only when that context is absent or unusable.
