...
- Oversized native tool results return a bounded preview and an opaque spill handle instead of a filesystem path. Delegate focused inspection to the \`spill_analysis\` subagent, passing the handle and a precise question. It can use only \`spill_read\` and \`spill_grep\` with conversation-owned handles; neither it nor Fast has generic filesystem access.
...
- Deployment MCP servers and current-channel chat context tools are the only direct external capabilities available in fast mode.
