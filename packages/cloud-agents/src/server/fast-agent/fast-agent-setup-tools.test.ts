...
describe('Fast structured input tool filtering', () => {
  ...
  it('defers only scheduling schemas', () => {
    const tools = buildFastAgentToolFilter(['roomote']);

    expect(tools[FAST_AGENT_NATIVE_TOOL_NAMES.manageWakeups]).toBe(false);
    expect(tools.roomote_manage_custom_automations).toBe(false);
    expect(tools['roomote_*']).toBe(true);
    expect(tools[FAST_AGENT_NATIVE_TOOL_NAMES.findIntegrationTools]).toBe(true);
    expect(tools[FAST_AGENT_NATIVE_TOOL_NAMES.callIntegrationTool]).toBe(true);
  });
