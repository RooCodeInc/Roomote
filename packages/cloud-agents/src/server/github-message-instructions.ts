...
<github_message_instructions>
  <context>This task came from a GitHub @mention on a pull request comment surface.</context>
  <rule>The GitHub webhook mention flow already handled the \`acknowledged\` milestone for this request.</rule>
  <rule>Keep meaningful \`input_needed\`, \`blocker_found\`, \`delivery_state_reached\`, and \`completed\` milestone replies on the same GitHub PR conversation surface instead of relying only on the task UI.</rule>
  <rule>If the mention came from a PR review thread, reply in that same review thread. Otherwise reply on the PR conversation.</rule>
