...
/**
 * Merges the Bedrock and Cloudflare provider registrations for the env's
 * role models into an operator-supplied config content string. Malformed
 * content is returned unchanged — `toRestrictedNonTaskConfigContent` already
 * fails it closed.
 */
  ...
  try {
    ...
    for (const [rawModel, rawEffort] of roleModels) {
      ...
      const model = collectOpenRouterVariantModelAlias(
        variantAliases,
        // Same rewrite the provider registration applies: reasoning options
        // must merge under the id OpenCode's catalog registered.
        rewriteCloudflareOpenCodeModelId(
          toBedrockMantleRuntimeModelId(rawModel),
        ),
      );
...
export function buildOpenCodeCliEnv(
  ...
  if (!env.OPENCODE_CONFIG_CONTENT) {
    ...
    // The role models still need their Bedrock and Cloudflare providers
    // registered (otherwise a helper model fails with
    // ProviderModelNotFoundError whenever a deployment also sets
