...
describe('slackAppMention', () => {
  it('attaches Slack instructions for Slack app mention tasks', async () => {
    ...
    expect(result.harnessInstructions).toContain(
      'Task UI commentary, todo updates, internal reasoning, and ordinary tool results are not visible in Slack. Slack-visible actions for the current turn are `send_chat_reply`, `send_chat_reaction_emoji`, `send_chat_message`, and Slack-rendered `request_user_input` prompts.',
    );
    expect(result.harnessInstructions).toContain(
  ...
  it('documents explicit proof sharing for Slack visual proof', async () => {
    ...
    expect(result.harnessInstructions).toContain(
      'Visual-proof uploads are not posted to Slack automatically.',
    );
    expect(result.harnessInstructions).toContain('imageArtifactIds');
  });
});
...
describe('buildChatProviderMessageInstructions', () => {
  ...
  it.each(['discord', 'teams', 'telegram'] as const)(
    'documents explicit visual-proof sharing for %s',
    (provider) => {
      const label =
        provider === 'discord'
          ? 'Discord'
