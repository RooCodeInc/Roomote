...
<rule>Read `attemptId` from `/tmp/roomote-visual-proof-attempt.json` and save captures under `/tmp/capture-visual-proof/&lt;attemptId&gt;/` so timeout recovery identifies current uploads. Invalid state means `proof runtime unavailable`; do not upload. Never print or read image bytes; the judge reviews them.</rule>
