    ...
    <steps>
      <step number="14">
        ...
        <actions>
          ...
          <action>Default final sections to: `Summary`, `Screenshots` when present, `Screencasts` when present, `Sharing note` when artifacts are present, and `Cleanup` only when needed, and use the delegated proof-runner report's `manage_artifacts` upload results as the source of truth for uploaded proof references.</action>
          ...
          <action>Only when the proof report contains uploaded screenshots, include each screenshot's `artifactId`, `viewUrl`, `rawUrl`, and one short sentence explaining what the still proves; only when it contains uploaded screencasts, include each clip's `artifactId`, `viewUrl`, `rawUrl`, one short sentence explaining what the clip proves, and every retained keyframe's `artifactId`, `viewUrl`, and `rawUrl`.</action>
