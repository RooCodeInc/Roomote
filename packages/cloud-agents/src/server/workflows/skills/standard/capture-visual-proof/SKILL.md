...
<role>
Capture the smallest honest proof with `agent-browser`, visually inspect the exact final evidence, upload via `manage_artifacts`, and report how the state was produced. The `judge` later performs an independent second check.
</role>
...
<rule>Keep browser output out of the transcript. Write screenshots, recordings, and keyframes under `/tmp/capture-visual-proof/` and never print image bytes. Before upload or sharing, open every exact final screenshot and every retained keyframe from the exact final clip with an image-capable tool, then compare what is visible with its checklist item and planned `Proves` claim. Browser assertions, snapshots, filenames, intended UI state, capture or upload success, and `ffprobe` are not visual verification. The later judge independently reopens the evidence; it does not replace this inspection.</rule>
    ...
    <steps>
      <step number="2">
        ...
        <actions>
          ...
          <action>Reject any final inspection that is blank, loading, errored, on the wrong page, stale, misleading, clipped, or missing the claimed state. Recapture once within the existing deadline; if it still fails, omit the invalid evidence, mark its checklist item missing, and plainly report the limitation without making that proof claim. That is the only recapture this skill allows.</action>
      ...
      <step number="3">
        <title>Upload</title>
        <actions>
          <action>Upload only screenshots, screencasts, and keyframes that passed the final inspection, using the `manage_artifacts` MCP tool with the `upload` action and `type` set to `visual-proof`.</action>
    ...
    <steps>
      <step number="4">
        ...
        <actions>
          ...
          <action>For each uploaded screenshot include its local path, `artifactId`, `viewUrl`, `rawUrl`, state provenance, and short `Proves` and `Does not prove` statements grounded only in what the final inspection showed. For each screencast include the same plus every retained keyframe's local path, `artifactId`, `viewUrl`, and `rawUrl`. Captions, the sharing note, and parent closeout claims must not exceed the inspected evidence.</action>
...
<criterion>Every kept screenshot and every retained keyframe from the exact final clip was visually opened and verified before upload or sharing; invalid evidence was recaptured once or omitted and reported without unsupported claims.</criterion>
