...
<rule>Verify the preview serves the changed app and code; a different prepared app is not proof. Otherwise report the target as unreachable.</rule>
...
<error_handling>
<scenario name="proof_capture_timed_out"><recovery>At the deadline, stop without retry and report `proof capture timed out`.</recovery></scenario>
<scenario name="proof_runtime_unavailable"><recovery>If `agent-browser` is unavailable, report `proof runtime unavailable`; do not substitute another browser.</recovery></scenario>
