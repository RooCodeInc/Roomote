...
<rule>Before the first browser command, snapshot the shipped diff with `mkdir -p /tmp/capture-visual-proof && git diff HEAD > /tmp/capture-visual-proof/diff-at-start.patch` (run it per repository in a shared-root workspace, appending each). The judge compares that snapshot with the delivery diff. Any source change you make after the snapshot, whether for simulation or for a fix, must be listed in the `Simulation disclosure` section or reverted before this skill returns.</rule>
