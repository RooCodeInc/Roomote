...
<rule>Judge proof truthfulness by the claim being proved. When the claim is about rendered UI output such as copy, layout, styling, ordering, visible state, interaction sequence, navigation, persistence, revisit, resume, replay, or animation, seeded local sandbox data, API fixtures, feature flags, test records, or normal form submissions are allowed when they establish realistic representative preconditions for a state that could be reached through a supported product flow and the UI still renders through its normal code path.</rule>
<rule>The exact proof data does not need to have arisen naturally in the current dev DB. Never use setup to create impossible or contradictory states, and never modify application source, patch UI components, inject hardcoded state, or bypass normal rendering to force a visual outcome.</rule>
    ...
    <steps>
      <step number="7">
        ...
        <actions>
          ...
          <action>When the proof claim is about rendered UI output, local sandbox data seeding, API fixtures, existing feature flags, test-record creation, or normal form submissions are valid only when they establish realistic representative preconditions and keep the UI on its normal rendering path.</action>
          <action>When the claim includes a data-derived UI outcome such as ordering, grouping, filtering, prioritization, or badges, require the seeded inputs to still exercise the relevant logic path instead of pre-encoding the final derived outcome.</action>
          <action>If the required state cannot be reached confidently from current context, mark the proof branch blocked instead of inventing setup.</action>
        </actions>
    ...
    <steps>
      <step number="11">
        ...
        <actions>
          ...
          <action>When the proof runtime is available, launch the `proof-runner` subagent with the Task tool and pass the full proof brief: the proof claim, the requested proof package, the coverage checklist, one proof sentence per planned artifact, and the required setup notes from planning.</action>
        </actions>
      ...
      <step number="12">
        ...
        <actions>
          ...
          <action>Treat the artifact URLs the subagent attributes to `manage_artifacts` upload tool results as the source of truth for uploaded screenshots and screencasts.</action>
    ...
    <steps>
      <step number="14">
        ...
        <actions>
          ...
          <action>Default final sections to: `Summary`, `Screenshots` when present, `Screencasts` when present, and `Cleanup` only when needed, and use the delegated proof-runner report's `manage_artifacts` upload results as the source of truth for uploaded proof links.</action>
          <action>In `Summary`, answer briefly what proof was captured, what it proves, what was partial supporting evidence, and what was skipped or blocked and why.</action>
          ...
          <action>Only when the proof report contains uploaded screenshots, include each screenshot `viewUrl` and one short sentence explaining what the still proves; only when it contains uploaded screencasts, include each clip's `viewUrl` and one short sentence explaining what the clip proves.</action>
          <action>If the proof branch was skipped or blocked, report the blocker briefly and include a short `Blocker type` chosen from: `proof runtime unavailable`, `browser surface unavailable`, `browser surface broken`, `claim not visually provable`, `state not reachable on current browser surface`, `fixture missing on current browser surface`, `external side effect risk`, or `upload failed`.</action>
          ...
          <action>Before returning the final result, run only lightweight planned cleanup that can be completed confidently from the same normal product flow. If cleanup would require deeper investigation, report the remaining cleanup briefly in `Cleanup`.</action>
...
<scenario name="simulated_proof_required"><problem>The only apparent way to show the target state is to modify application source, patch UI components, inject hardcoded state, mutate hidden payloads that bypass the real path, or otherwise short-circuit normal rendering to force a visual outcome.</problem><recovery>Do not perform those changes. Report the proof branch as blocked, explain why the real product path could not be reproduced, and point to stronger non-visual evidence when available.</recovery></scenario>
