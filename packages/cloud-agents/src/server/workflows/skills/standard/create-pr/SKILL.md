    ...
    <steps>
      ...
      <step number="6">
        ...
        <pr-metadata-update-recipe>
          ...
          <item>Call `mcp__roomote__manage_source_control` with `action: "create_or_update_pull_request"`, `repositoryFullName: "<OWNER/REPO-or-provider-full-name>"`, `sourceBranch: "<current-branch>"`, `targetBranch: "<base-branch-for-this-repo>"`, `title: "<TITLE>"`, and `body` set to the exact `/tmp/pr-body.md` contents. Include `prAttribution` when the conversation establishes a clear participant, include `labels` only when a current conflict-resolver label is provided, and include `assignees` only when provider-compatible assignee usernames are available for this run. The tool creates a new pull request or refreshes the open one for the branch in a single call.</item>
        </pr-metadata-update-recipe>
  ...
  <phase name="validation">
    ...
    <steps>
      ...
      <step number="8">
        ...
        <description>After successful pull request delivery, ask once with a lightweight conversational question whether the user wants this agent to automatically address later CI failures or review feedback on those pull requests.</description>
        <actions>
          ...
          <action>After the PR summary, ask once in plain language whether they want the agent to automatically address CI failures or review feedback on the opened pull request(s). Keep it short—one brief natural question nestled in the delivery summary, not a form or multi-part questionnaire. Do not coach the answer format (do not say "reply yes or no" or similar). Do not start that follow-up work until they opt in.</action>
          ...
          <action>Do not use `request_user_input` for this ask. A single casual conversational offer is enough; structured input is too heavy for this follow-up offer.</action>
