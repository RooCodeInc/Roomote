    ...
    <steps>
      ...
      <step number="6">
        ...
        <pr-metadata-update-recipe>
          ...
          <item>Call the Roomote MCP tool `mcp__roomote__manage_artifacts` with `action: "list"` and `artifactType: "visual-proof"` for the current task immediately before writing PR metadata. Reconcile that result with the latest `capture-visual-proof` handoff and its proof-runner upload results: listed artifacts from the latest proof attempt refresh signed URLs and remain authoritative when a timeout or abort raced with completed uploads. When a timeout summary conflicts with verified uploads from that same latest attempt, include the verified current-attempt artifacts and do not claim proof timed out. Never infer that an older listed artifact belongs to the current proof cycle; if current-attempt provenance cannot be established from matching artifact IDs, paths, or upload results, keep the latest honest blocked or no-op outcome instead of attaching stale proof.</item>
