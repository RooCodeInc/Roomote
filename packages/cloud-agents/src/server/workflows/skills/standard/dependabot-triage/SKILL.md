...
<workflow>
  ...
  <phase name="report">
    <steps>
      ...
      <step>End the task response with a terse internal note after sending the report, including when there are no actionable alerts or GitHub access is blocked.</step>
    </steps>
  </phase>
</workflow>
...
<criterion>Every scan with a configured communication destination posted exactly one final result with the critical/high/medium/low severity breakdown, related open PR state, and remediation coverage count.</criterion>
</completion_criteria>
