...
<workflow>
  <overview>Use the GitHub access already available in the task environment. Prefer `gh api` for alert retrieval, keep scheduled/background runs read-only, and honor any repository scope, communication destination, run mode, or automation policy supplied in the request. Human-triggered or automation-started follow-up execution work belongs in the dependency-update specialist, not in this triage scan itself. Make each scan completion-oriented: track the current alert count, account for related open PRs, and launch the smallest remediation that can clear a cohesive alert bundle.</overview>

  <phase name="setup">
    <steps>
      <step>Parse the request for `repository_scope`, a destination in either `slack_channel_id` or `channel_id`, `run_mode`, trigger source, any optional `Repository environments` section, and any recent thread feedback.</step>
  ...
  <phase name="report">
    <steps>
      ...
      <step>Do not submit suggestion work items (they are rejected) or fall back to bare-repo execution. Do not duplicate work already covered by a related open PR; include the PR and any uncovered alerts in the status message instead. The later execution task stays silent while work is in flight and reports to its configured communication surface only when it needs input, hits a blocker, or has a meaningful result.</step>
      ...
      <step>Do not post an opening acknowledgement, scan announcement, progress update, or partial finding. When every repository in scope was scanned successfully, the total open alert count is 0, and no remediation work item was submitted or started, do not call `send_chat_reply`; end with only a terse internal no-op note. Otherwise, when either `slack_channel_id` or `channel_id` is present, send exactly one concise report with `send_chat_reply` using purpose `closeout` only after triage reaches a final result; the first reply creates the standard automation result thread in the configured communication conversation. Include the total open alert count with critical/high/medium/low severity breakdown, per-repository counts, relevant open PRs (or that none are open), and how many currently open alerts are covered by newly started remediation task(s), existing related PRs, or neither.</step>
      <step>Keep the `send_chat_reply` report plain-language and manager-readable, and do not paste raw GitHub CLI commands, `gh api` invocations, or command transcripts into the configured conversation. The exact commands belong only in work item `investigationContext`, never in the conversation report.</step>
      <step>End the task response with a terse internal note after sending any required report. A conclusive zero-alert no-op ends with the internal note only; GitHub access blockers still require a communication report.</step>
    </steps>
  </phase>
</workflow>
...
<criterion>The final report or submitted work items were concise, prioritized, plain-language, and free of raw command transcripts, so they are safe to post to the configured communication surface.</criterion>
<criterion>Every scan with a configured communication destination posted exactly one final result with the critical/high/medium/low severity breakdown, related open PR state, and remediation coverage count, except a conclusive all-repository zero-alert scan with no remediation work, which posted nothing.</criterion>
</completion_criteria>
