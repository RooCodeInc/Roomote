...
<workflow>
  ...
  <phase name="report">
    <steps>
      ...
      <step>Do not post a Slack opening acknowledgement, scan announcement, progress update, or partial finding. Default to finishing silently. If `slack_channel_id` is present, post one concise scan summary there with `post_to_channel` only when there is something a human should see now: uncovered actionable alerts, newly started remediation work, a related open PR that needs attention, a material worsening in risk, or a GitHub setup/auth blocker. Include the total open alert count with critical/high/medium/low severity breakdown, per-repository counts, relevant open PRs, and how many currently open alerts are covered by newly started remediation task(s), existing related PRs, or neither. When there are no actionable findings or blockers, do not post and end with a terse internal note.</step>
      ...
      <step>End the task response with a terse internal note after posting a warranted scan summary or after a clean run.</step>
    </steps>
  </phase>
</workflow>
...
<criterion>Every warranted Slack report included the critical/high/medium/low severity breakdown, related open PR state, and remediation coverage count; clean runs stayed silent.</criterion>
</completion_criteria>
