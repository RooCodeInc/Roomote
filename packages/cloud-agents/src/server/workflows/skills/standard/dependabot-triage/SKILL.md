...
<workflow>
  ...
  <phase name="report">
    <steps>
      ...
      <step>Do not post a Slack opening acknowledgement, scan announcement, progress update, or partial finding. If `slack_channel_id` is present, post exactly one concise scan summary there with `post_to_slack_channel` only after triage reaches a final result: total open alert count, per-repository counts, relevant open PRs (or that none are open), and whether a remediation task was started, deferred because an existing PR covers the alerts, or blocked.</step>
...
<criterion>Every configured Slack scan posted exactly one final result with the current alert count, related open PR state, and remediation disposition.</criterion>
</completion_criteria>
