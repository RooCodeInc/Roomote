    ...
    <steps>
      <step number="1">
        ...
        <actions>
          ...
          <action>If the target was not explicit, read only `ROOMOTE_TASK_ID` from the current runtime, then call `mcp__roomote__manage_tasks` with `action: "get_summary"` for that task and use its linked environment ID. Never dump the full process environment. Cross-check that exact ID and name against an immediate `list_environments` result. The `__all_repositories__` organization-wide target is not a named environment and is never a fallback for a missing linked environment. If the current task has no linked environment or the target remains ambiguous, ask the user instead of guessing.</action>
    ...
    <steps>
      <step number="3">
        ...
        <actions>
          ...
          <action>If fallback polling observes a settled task before the notification arrives, inspect its messages, then allow one polling interval for the pending settle notification before returning the final Doctor outcome. Consume a late notification as confirmation and do not repeat an outcome already reported.</action>
