...
<workflow>
  <overview>Inspect repository evidence, infer the smallest correct Roomote environment config, validate install/test/start behavior locally on localhost when practical (delegating browser-level verification of a validated UI surface to the hidden proof-runner subagent), create or update the environment when validation is sufficient, launch one lightweight verification task against the resulting environment as the single final end-to-end confirmation, monitor that spawned task through the Roomote task summary surface while narrating concise progress updates, and when verification surfaces fixable setup errors, revise the environment and repeat the create-or-update plus verification cycle before returning a concise final outcome summary with explicit assumptions, validated observations, and blockers.</overview>
  <scope>
  ...
  <phase name="implementation">
    ...
    <steps>
      ...
      <step number="4">
        ...
        <description>Use runtime evidence to confirm install/start assumptions without over-expanding scope. Local validation should prove that the app serves successfully on localhost when a browser UI exists. Browser-level verification is delegated to the hidden `proof-runner` subagent; the parent workflow itself never issues browser commands.</description>
        <actions>
...
11a. When a browser UI surface passed loopback validation, additionally delegate one verification brief to the hidden `proof-runner` subagent with the Task tool instead of judging the rendered page from HTTP responses alone: name the exact validated absolute localhost URL as the browser target for that run, state the claim that the app at that URL serves its expected landing page without an error state, request a screenshot proof package with a one-item coverage checklist, and include any setup notes (such as authentication steps) needed to reach the page. Treat a `Blocked: true` report — with its observed port state, HTTP response, or visible browser error — as concrete validation-failure evidence for revising the config, and a `Blocked: false` report with its screenshot as the strongest local evidence that the app actually runs. Do not ask the proof runner to recover the environment, diagnose source code, or fix anything: configuration fixes stay in this task, and after each config revision that affects install/start behavior, rerun the affected validation including a fresh proof-runner check before persisting.
11b. If the `proof-runner` subagent is unavailable in this task, fall back to loopback reachability and startup evidence alone and note that limit; do not issue browser commands from the parent workflow as a substitute.
...
24. After environment persistence succeeds, use the Roomote MCP task tools to launch one lightweight follow-up verification task against that environment before finishing this setup task. This spawned task is the single final end-to-end confirmation that the platform provisions the persisted config correctly (fresh workspace, setup command execution, ports, services); it is not the iteration vehicle. Iterate on config problems with local validation plus the in-task proof-runner check before and between launches instead of re-running the full environment pipeline to test each revision.
...
26. Then call the Roomote MCP tool `mcp__roomote__manage_tasks` with `action: "launch"`, `environmentId` set to that created or updated environment ID, and a concrete prompt such as `Confirm that this environment is running correctly. First read .roomote/setup-status.json in the workspace root: while its state is "running", environment setup commands are still executing in the background — wait for it to reach a terminal state (you will get an in-session notification when it settles) instead of concluding startup is stuck; setup can take 5 minutes or more. Once setup has settled, confirm the expected service responds at localhost or the environment's initial URL. If the environment has a browser UI, then delegate one proof brief to your hidden proof-runner subagent with the claim that the app serves its expected landing page at the environment's initial URL, and relay its Blocked verdict and any uploaded screenshot URLs in your report instead of judging the page yourself; if there is no browser surface, verify loopback reachability directly. If any setup command failed, report each failing command's name and exit code from .roomote/setup-status.json plus the relevant error lines from its log under .roomote/setup-logs/. Report the exact step that fails plus any visible error messages or logs. If everything works, say that the environment looks ready.`.
...
35. If the monitored summary reaches `Failed`, `Canceled`, exposes a startup or runtime error, or its `Environment Setup` line reports `failed` or `completed with warnings`, inspect the exact status and error — asking the verification task for the failing command names, exit codes, and log excerpts from `.roomote/setup-status.json` and `.roomote/setup-logs/` when they are not already surfaced — and decide whether the problem appears fixable within environment-setup scope, such as revising commands, services, environment variables, startup order, readiness checks, or other environment-definition details.
36. When the spawned verification task reveals a fixable setup or environment-definition error, try to fix it yourself, rerun any affected local validation including a fresh in-task proof-runner check when a browser surface exists, recreate or update the environment with the revised YAML, launch a fresh verification task, and repeat the monitoring process instead of stopping after the first failure. Converge locally first: a revision should pass local validation and the in-task proof-runner check before it earns another spawned end-to-end launch.
...
<criterion>If the app exposes a browser UI and local startup succeeds, the localhost URL is verified through loopback reachability plus a delegated proof-runner check (or other non-browser startup evidence when the proof runner is unavailable) before persistence continues.</criterion>
...
<rule>Do not depend on prompt-wide browser bootstrap or direct browser automation inside `environment-setup`; delegate browser-level verification to the hidden `proof-runner` subagent with a brief that names the validated absolute localhost URL as the browser target, and keep parent-side localhost validation to loopback reachability and startup evidence.</rule>
...
<rule>For this skill, create a new environment or update the specified existing environment only after localhost startup and loopback reachability are successful enough to proceed, including validation of `initialUrl` through loopback evidence plus a delegated proof-runner check when the app exposes a browser UI (falling back to non-browser evidence alone only when the proof runner is unavailable).</rule>
...
<rule>When a browser UI is validated locally, report the exact localhost URL and the evidence that confirmed it, including the proof-runner verdict and its screenshot artifact URLs when a proof-runner check ran; never capture screenshots directly from the parent workflow.</rule>
