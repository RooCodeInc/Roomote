    ...
    <steps>
      <step number="3">
        ...
        <actions>
          ...
          <action>When a setup-required environment key is clear but its value is unavailable, request it immediately instead of waiting for a command to fail: in web dashboard tasks and Slack-started setup tasks, use `request_environment_variables`; for credentials and tokens, include the exact read or write access and relevant resource scope, research current provider documentation before including exact permission names, optionally link to the provider's HTTPS token-creation or permission documentation, and disclose when the provider only offers broader access instead of inventing granular scopes. Omit credential access metadata for noncredential variables. For Slack-started setup tasks, still send a concise `send_chat_reply` message with `purpose` set to `progress` naming the required keys and what they unblock, but do not add the secure `/setup` link yourself because the platform automatically accompanies that request with a standardized secure-entry link reply; in other surfaces, ask the user to add it locally in the current task. Keep the YAML best-effort and ready for user-provided values (for example `${KEY}` placeholders when appropriate), without guessing secret values.</action>
...
22. Only after the relevant local validation works (or is otherwise sufficiently validated), call `manage_environments` with action `preview` for the final drafted definition. Tell the user the concrete repositories, commands, Docker or analysis recipe work, maximum configured setup time, and create/update disruption returned by the preview, then use `request_user_input` to ask whether to proceed. The original request to run or analyze something is not approval to mutate an environment. Admin authorization is separate from this consent step.
23. Create or update only after the user responds affirmatively. No approval token or identifier is required. If the user declines, do not persist the environment. If environment persistence fails because the YAML still needs adjustment, revise it, explain the corrected change, ask again, and retry at most 2 times.
...
<rule>Before every agent-driven create or update, preview the final definition, tell the user what will change and its setup/build time and disruption impact, then use `request_user_input` to ask whether to proceed. Create or update only after an affirmative response. Passing admin authorization or asking to run an analysis is not consent.</rule>
<rule>After successful environment persistence, do not launch another Roomote task from this sandbox and do not record successful verification from current-sandbox evidence.</rule>
<rule>Report local validation separately from fresh-task verification. When fresh verification is still needed, state that Fast or an authenticated user must launch a top-level task against the persisted environment.</rule>
