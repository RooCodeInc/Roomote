...
- Default branch: `<default-branch>`</action>
      ...
      <step number="1a">
        ...
        <actions>
          ...
          <action>Commit directly to the branch the workspace is already on (the unborn branch was already pointed at the repository's stored default branch). A repository with no commits has no hooks checked in, so there is nothing for pre-commit to run here.</action>
          <action>Before pushing, review the outgoing diff for secrets. The bootstrap commit should contain only `README.md` and `.gitignore`, so confirm with `git show --stat HEAD` that nothing else was committed, and read both files to be sure neither carries API keys, tokens, private keys, or `.env` contents. Treat any real credential as a hard blocker: do not push, remove it from the commit before any retry, report it, and tell the user the credential must be rotated.</action>
          <action>Always push the bootstrap commit with `git push --no-verify -u origin <default-branch>`. Never force-push. This is the one delivery push that targets the default branch, so the secret review above is not optional. Pre-push hooks commonly re-run full lint/typecheck/test suites, and in Roomote sandboxes those frequently fail on host tooling differences rather than on real defects, which agents then misreport as missing Git credentials. Skipping pre-push also skips any local secret or policy scanner a repository attaches to that hook, and equivalent server-side coverage is not guaranteed, which is why the secret review above is mandatory rather than optional.</action>
          <action>If the push still fails, classify the error before reporting it. A rejection that names a secret, credential, key, or policy violation is a real finding, not an environment problem: that covers `remote:` push-protection rejections such as GitHub `GH013`. Stop, report it, strip the secret from the commit, and never retry those with `--no-verify`. Otherwise, `remote:` lines, HTTP 403, `Permission denied`, `Authentication failed`, or a credential prompt are remote or auth failures. Report the exact error text and never substitute a credentials story for a hook failure.</action>
