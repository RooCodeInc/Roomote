...
<action>Frame rate is opt-in: omit top-level `fps` for ordinary 30 FPS demos. For motion-heavy demos where smoother motion matters, set `"fps": 60`; supported values are integers from 1 to 60. Higher FPS increases capture and render work, so do not force it globally. The runner passes this rate to native recording and writes `timeline.fps`, which the renderer uses for composition and narration timing.</action>
...
<action>The runner requires agent-browser v0.37.0 or newer and records headless using native `record start ... --fps`. It stops any existing daemon, opens the target page, sets the viewport, then attaches recording to that active page. Native recording preserves wall-clock holds without a synthetic frame ticker.</action>
<action>Above 30 FPS the runner uses native H.264 `raw.mp4` and lossless faststart remuxing, avoiding VP8 encoder backpressure; ordinary capture keeps `raw.webm`. Inspect `capture-stats.json`: `capturedFrames` counts incoming frames, `frames` includes held duplicates, and `wallSeconds` records interaction time. Check actual motion-frame changes and file duration before claiming higher capture FPS; an encoded 60 FPS stream or a 60 FPS render alone does not prove 60 distinct captured frames. Static holds legitimately repeat frames. Disclose reduced cadence or pacing failures instead of increasing output FPS.</action>
...
<name>Deliver</name>
<actions>
<action>Prefer final MP4 with H.264, `yuv420p`, and `+faststart` for playback compatibility. Keep the native `raw.webm` source (or `raw.mp4` for higher-FPS capture). If the rendered MP4 lacks faststart, remux a separate copy with `ffmpeg -i out/demo-wide.mp4 -c copy -movflags +faststart out/demo-wide-faststart.mp4` and verify it before upload; do not retime or alter content for format conversion.</action>
