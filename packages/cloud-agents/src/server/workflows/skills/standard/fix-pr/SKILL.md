  ...
  <phase name="implementation">
    ...
    <steps>
      ...
      <step number="7">
        ...
        <actions>
          <action>Commit the resulting fixes on the existing PR branch, capture `git rev-parse HEAD`, determine the correct push target from the PR head metadata, and always push with `--no-verify` to the existing PR branch without assuming a same-repository `origin` push. Roomote sandboxes rely on CI and server-side checks for full-suite pre-push validation and secret/policy gates; do not treat pre-push hook failures as missing credentials.</action>
