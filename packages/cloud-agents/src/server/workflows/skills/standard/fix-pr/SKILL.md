  ...
  <phase name="implementation">
    ...
    <steps>
      ...
      <step number="7">
        ...
        <actions>
          <action>Commit the resulting fixes on the existing PR branch and capture `git rev-parse HEAD`. If pre-commit hooks fail, fix the underlying formatting or validation issue. Do not use `git commit --no-verify` unless no safer option remains.</action>
          <action>Before pushing, review the outgoing diff for secrets. List what changed with `git diff --stat <baseSha>...HEAD`, then scan the patch content itself with `git diff <baseSha>...HEAD | grep -nEi '(api[_-]?key|secret|passwo?rd|BEGIN [A-Z ]*PRIVATE KEY|xox[baprs]-|ghp_|github_pat_|sk-[A-Za-z0-9]{20,}|AKIA[0-9A-Z]{16})'` and read every hit in context. Take `<baseSha>` from the latest `get_pull_request` result rather than assuming `origin/<base>` exists locally, and keep the three-dot form so commits that only landed on the base branch are excluded. Do not limit the read to configuration files: a leaked token is just as likely to sit in source, a test fixture, or a captured log. If `gitleaks` is on PATH, also run `gitleaks git --log-opts='<baseSha>...HEAD'`; its absence does not excuse the manual review. Treat any real credential as a hard blocker: do not push, remove it from the commit before any retry, report it, and tell the user the credential must be rotated.</action>
          <action>Determine the correct push target from the PR head metadata and always push with `git push --no-verify` to the existing PR branch, without assuming a same-repository `origin` push. Pre-push hooks commonly re-run full lint/typecheck/test suites, and in Roomote sandboxes those frequently fail on host tooling differences rather than on real defects, which agents then misreport as missing Git credentials. Skipping pre-push also skips any local secret or policy scanner the repository attaches to that hook, and equivalent server-side coverage is not guaranteed, which is why the secret review above is mandatory rather than optional.</action>
