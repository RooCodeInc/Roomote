...
<workflow>
  ...
  <phase name="report">
    <steps>
      ...
      <step>Create follow-up work only when authorized by the request or automation context and supported by available capabilities. Honor the supplied submission limits, repository eligibility, environment requirements, and reporting policy using the advertised schemas. Do not guess repository ownership or bypass required environment coverage. Otherwise recommend concrete actions in ordinary prose in the report and ask what the user wants started; use the normal task-start flow after acceptance. Do not emit suggestion cards or structured suggestions. Explicitly authorized autonomous automation act work remains autonomous.</step>
