      ...
      <step number="4">
        ...
        <actions>
          <action>For recurring maintenance runs, use the package manager's non-mutating outdated-report command before selecting updates when one is available. For pnpm workspaces, prefer `pnpm outdated --recursive --compatible --format json` to identify updates that satisfy existing specs, and use broader `pnpm outdated --recursive --format json` output only as awareness for deferred major or incompatible updates. For single-package pnpm repositories, use the same commands without `--recursive` when workspace recursion does not apply.</action>
           ...
           <action>For Dependabot alert follow-up runs, resolve any explicitly configured review ownership before changing dependencies. First inspect `.github/dependabot.yml` and match the alert's package ecosystem and manifest directory to the applicable `updates` entry. If that entry has no usable review owner, inspect supported local Renovate config names in precedence order: `.github/renovate.json`, `.github/renovate.json5`, `renovate.json`, `renovate.json5`, `.renovaterc`, and `.renovaterc.json`; resolve the effective `reviewers`, `additionalReviewers`, `assignees`, and `additionalAssignees` from the base config and matching `packageRules` for the affected package, manager or ecosystem, and manifest path. Respect Renovate's override and additive field semantics, and do not treat unrelated update entries or package rules as ownership for the remediation.</action>
           <action>Keep only concrete GitHub user or team targets from the matching config. Preserve the config's distinction between `reviewers` and `assignees`, normalize supported GitHub team forms such as `org/team-slug` or Renovate's `team:team-slug` for the GitHub API, and ignore templates, dynamic values, unsupported host-specific syntax, or inherited presets that cannot be resolved from repository files. Do not infer reviewers from `CODEOWNERS` or enable Renovate `reviewersFromCodeOwners` behavior.</action>
  ...
  <phase name="delivery">
    ...
    <steps>
      <step number="1">
        ...
        <actions>
          <action>For Dependabot alert follow-ups, pass separately configured assignees from the matching dependency-update config into the PR-delivery skill's existing `assignees` contract rather than converting assignees into reviewers.</action>
          ...
          <action>After `create-draft-pr` (or an explicitly requested PR-delivery skill) returns the GitHub pull request number, request the concrete reviewers resolved from the matching dependency-update config. Call `manage_source_control` with action `request_pull_request_reviewers`, passing user logins as `reviewers` and team slugs as `teamReviewers`.</action>
          <action>If no supported config exists, no update entry or package rule matches, or the matching rule contains no concrete reviewer or assignee, preserve the existing delivery behavior and finish without an ownership mutation. A genuine GitHub API rejection for a resolved target must be reported with the PR result; do not claim the reviewer or owner was assigned.</action>
        </actions>
...
<criterion>Repository-changing successful runs were delivered through `create-draft-pr` (or an explicitly-requested override), the PR body summarized the applied updates, validation and runtime-proof result, and any deferred packages, and Dependabot follow-up PRs requested any concrete reviewers or assignees from the matching local dependency-update config after creation.</criterion>
...
<patterns>
  ...
  <pattern name="dependabot_alert_follow_up">
    ...
    <template>classify run as `alert_follow_up` immediately -> re-fetch the open alert via GitHub before any generic outdated scan -> resolve concrete reviewers or assignees from the matching local Dependabot or Renovate rule -> constrain the update set to the named package or manifest or workspace -> apply the smallest safe update that is likely to clear the alert, widening only to a tightly related workspace or dependency bundle when necessary -> run the strongest practical repository-defined validation plus any required service-level proof -> create the draft PR -> request the resolved GitHub reviewers or assignees, or preserve the existing no-owner behavior when none resolve -> report whether the alert-backed update landed, was already closed, or was deferred</template>
  </pattern>
</patterns>
