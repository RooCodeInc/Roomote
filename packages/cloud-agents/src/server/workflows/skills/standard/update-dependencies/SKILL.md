    ...
    <steps>
      <step number="1">
        ...
        <actions>
          ...
          <action>After `create-draft-pr` (or an explicitly requested PR-delivery skill) returns the GitHub pull request number, apply the concrete review ownership resolved from the matching dependency-update config. Request user and team reviewers with `POST repos/{owner}/{repo}/pulls/{pull_number}/requested_reviewers`, passing users as `reviewers` and team slugs as `team_reviewers`. Assign separately configured assignees with GitHub's issue-assignee API rather than converting assignees into reviewers. Do not make reviewer assignment part of generic cross-provider PR creation.</action>
