...
export async function cleanupAzureInstance(options: {
  ...
  console.warn(`[${logPrefix}] Cleaning up Azure instance after ${reason}`, {
    instanceId,
    phase,
    error:
  ...
  try {
    await onMutation?.({
      ...
      eventType: 'started',
      instanceId,
      message: `Calling destroyInstance for Azure instance ${instanceId}.`,
    ...
    await onMutation?.({
      ...
      eventType: 'completed',
      instanceId,
      message: `destroyInstance completed for Azure instance ${instanceId}.`,
      details: mutationDetails,
    });

    console.log(`[${logPrefix}] Cleaned up Azure instance`, {
      instanceId,
      phase,
      reason,
    });
    ...
    await onMutation?.({
      ...
      eventType: 'failed',
      instanceId,
      message: `destroyInstance failed for Azure instance ${instanceId}.`,
      details: {
        ...mutationDetails,
        error:
    ...
    console.error(`[${logPrefix}] Failed to clean up Azure instance`, {
      instanceId,
      phase,
      reason,
      cleanupError:
