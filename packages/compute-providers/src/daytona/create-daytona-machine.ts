...
export async function createDaytonaMachine(
  ...
  for (
    ...
    try {
      ...
      console.warn(
        `[createDaytonaMachine] Bootstrap attempt ${bootstrapAttempt}/${MAX_BOOTSTRAP_ATTEMPTS} failed in phase ${bootstrapPhase}; retrying with a fresh instance in ${BOOTSTRAP_RETRY_DELAY_MS}ms ${JSON.stringify(
          { error: error instanceof Error ? error.message : String(error) },
        )}`,
      );
  ...
  return {
    ...
    domain: (port: number) => {
      ...
      throw new Error(
        `No Daytona preview link resolved for port ${port} on ${bootstrappedMachine.instanceId}`,
      );
    },
  };
}
