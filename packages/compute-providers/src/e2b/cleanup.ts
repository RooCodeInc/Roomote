...
export async function cleanupE2bInstance(options: {
  ...
  console.warn(`[${logPrefix}] Cleaning up E2B instance after ${reason}`, {
    instanceId,
    phase,
    error:
  ...
  try {
    await onMutation?.({
      ...
      eventType: 'started',
      instanceId,
      message: `Calling destroyInstance for E2B instance ${instanceId}.`,
    ...
    await onMutation?.({
      ...
      eventType: 'completed',
      instanceId,
      message: `destroyInstance completed for E2B instance ${instanceId}.`,
      details: mutationDetails,
    });

    console.log(`[${logPrefix}] Cleaned up E2B instance`, {
      instanceId,
      phase,
      reason,
    });
    ...
    await onMutation?.({
      ...
      eventType: 'failed',
      instanceId,
      message: `destroyInstance failed for E2B instance ${instanceId}.`,
      details: {
        ...mutationDetails,
        error:
    ...
    console.error(`[${logPrefix}] Failed to clean up E2B instance`, {
      instanceId,
      phase,
      reason,
      cleanupError:
