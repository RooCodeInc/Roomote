ALTER TABLE "deployment_settings" ALTER COLUMN "id" SET DEFAULT 'default';--> statement-breakpoint
...
ALTER TABLE "tasks" ADD CONSTRAINT "tasks_surface_check" CHECK ("tasks"."surface" in ('web', 'api', 'slack', 'teams', 'telegram', 'discord', 'linear', 'github', 'gitlab', 'gitea', 'ado', 'bitbucket', 'system'));--> statement-breakpoint
CREATE UNIQUE INDEX "task_runs_discord_source_event_unique" ON "task_runs" USING btree (("payload"->>'communicationSourceEventId')) WHERE "task_runs"."payload"->>'communicationProvider' = 'discord' AND "task_runs"."payload"->>'communicationSourceEventId' IS NOT NULL AND "task_runs"."canceled_at" IS NULL;
