-- Azure joins the sleep-check partial indexes on task_runs (Roomote's hottest
-- table). Built under new names FIRST so sleep-check queries keep index
-- coverage throughout; the old names are dropped only after the replacements
-- exist. Each plain CREATE INDEX still takes a brief write lock on task_runs,
-- chosen deliberately over CREATE INDEX CONCURRENTLY because drizzle wraps
