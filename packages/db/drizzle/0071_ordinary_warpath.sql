...
CREATE TABLE "agentmail_conversations" (
	...
	"bound_task_run_id" integer,
	"fast_session_id" uuid,
...
--> statement-breakpoint
CREATE TABLE "agentmail_pending_inputs" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
	"conversation_id" uuid NOT NULL,
	"task_run_id" integer,
	"prompt" text NOT NULL,
	"expected_responder_user_ids" jsonb DEFAULT '[]'::jsonb NOT NULL,
	"answer_text" text,
	...
	CONSTRAINT "agentmail_pending_inputs_state_check" CHECK ("agentmail_pending_inputs"."state" in ('pending', 'claimed', 'delivered', 'expired'))
);
--> statement-breakpoint
...
ALTER TABLE "agentmail_conversations" ADD CONSTRAINT "agentmail_conversations_bound_task_run_id_task_runs_id_fk" FOREIGN KEY ("bound_task_run_id") REFERENCES "public"."task_runs"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "agentmail_pending_inputs" ADD CONSTRAINT "agentmail_pending_inputs_conversation_id_agentmail_conversations_id_fk" FOREIGN KEY ("conversation_id") REFERENCES "public"."agentmail_conversations"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "agentmail_pending_inputs" ADD CONSTRAINT "agentmail_pending_inputs_task_run_id_task_runs_id_fk" FOREIGN KEY ("task_run_id") REFERENCES "public"."task_runs"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "agentmail_pending_inputs" ADD CONSTRAINT "agentmail_pending_inputs_answered_by_user_id_users_id_fk" FOREIGN KEY ("answered_by_user_id") REFERENCES "public"."users"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "agentmail_pending_inputs_one_pending_per_conversation" ON "agentmail_pending_inputs" USING btree ("conversation_id") WHERE "agentmail_pending_inputs"."state" = 'pending';--> statement-breakpoint
CREATE INDEX "agentmail_pending_inputs_state_idx" ON "agentmail_pending_inputs" USING btree ("state","created_at");--> statement-breakpoint
