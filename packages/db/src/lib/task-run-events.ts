...
interface RecordTaskRunEventInput {
  runId: number;
  taskId?: string;
...
interface ListTaskRunEventsOptions {
  runId: number;
  limit?: number;
  order?: 'asc' | 'desc';
}
...
export async function recordTaskRunEvent(
  database: DatabaseOrTransaction,
  input: RecordTaskRunEventInput,
) {
...
export async function recordSnapshotResumeEvent(
  ...
  return recordTaskRunEvent(database, {
    runId: input.runId,
    taskId: input.taskId,
...
export async function recordTaskRunLifecycleEvent(
  database: DatabaseOrTransaction,
  input: {
    runId: number;
    taskId?: string;
...
export async function recordComputeProviderMutationEventSafe(
  ...
  try {
    ...
    logger.warn(
      `[${prefix}] Failed to persist compute-provider event for task run #${
        input.runId
...
export async function listTaskRunEvents(
  database: DatabaseOrTransaction,
  options: ListTaskRunEventsOptions,
) {
  const orderBy =
    options.order === 'asc'
