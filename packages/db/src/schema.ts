...
/**
 * task_follow_up_messages
 *
 * Durable admission for task follow-ups that arrive before the sandbox
 * harness can accept a command. RuntimePromptQueue remains the in-process
 * delivery buffer; this table is the restart-safe handoff between API and
 * worker processes.
 */
export const taskFollowUpMessages = pgTable(
  'task_follow_up_messages',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    workerQuoteUserName: text('worker_quote_user_name'),
    clientMessageId: text('client_message_id').notNull(),
    deliveryMode: text('delivery_mode').notNull().$type<'send' | 'steer'>(),
    status: text('status')
      .notNull()
      .default('pending')
      .$type<'pending' | 'accepted' | 'delivered' | 'discarded'>(),
  ...
  (table) => [
    uniqueIndex('task_follow_up_messages_task_client_id_unique').on(
      table.taskId,
      table.clientMessageId,
    ),
    uniqueIndex('task_follow_up_messages_sequence_unique').on(table.sequence),
    index('task_follow_up_messages_run_pending_idx').on(
      table.runId,
      table.status,
      table.sequence,
    ),
    index('task_follow_up_messages_task_pending_idx').on(
      table.taskId,
      table.status,
      table.sequence,
    ),
...
export const taskFollowUpMessagesRelations = relations(
  taskFollowUpMessages,
  ({ one }) => ({
    run: one(taskRuns, {
      fields: [taskFollowUpMessages.runId],
      references: [taskRuns.id],
    }),
    task: one(tasks, {
      fields: [taskFollowUpMessages.taskId],
      references: [tasks.id],
    }),
    user: one(users, {
      fields: [taskFollowUpMessages.userId],
      references: [users.id],
    }),
  }),
);
