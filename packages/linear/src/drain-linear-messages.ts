 ...
 */
export interface DrainSourceRun {
  id: number;
...
/**
 * Peek for pending Linear messages on a completed task run, and if any exist,
 ...
 *      prevent duplicate resume task runs for the same Linear issue
 ...
 *   is fully updated)
 */
export async function drainLinearMessagesToResumeRun(
  sourceRun: DrainSourceRun,
  snapshotIdOverride?: string,
): Promise<DrainResult> {
  if (!sourceRun.linearSessionId) {
  ...
  const messageCount = await peekLinearMessageCount(sourceRun.id);
  ...
  // both code paths from creating duplicate resume task runs for the same issue.
  const redis = getRedis();
  const issueId = sourceRun.linearIssueId;
  ...
  try {
    messages = await getLinearMessages(sourceRun.id);
    ...
    try {
      if (messages.length > 0) {
        await prependLinearMessages(sourceRun.id, messages);
      }
      if (issueId) {
        await redis.del(`${RESUME_LOCK_PREFIX}${issueId}`);
      }
  ...
  console.log(
    `[drainLinearMessagesToResumeRun] Created resume task run ${resumeLaunch.id} with ${messages.length} Linear message(s)`,
  );
