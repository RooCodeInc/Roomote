...
Do not post any ${promptContext.surfaceLabel} opening acknowledgement, scan announcement, progress update, or partial finding. Default to finishing silently. Post one concise status message to the configured ${promptContext.surfaceLabel} channel with \`${promptContext.postToolName}\` only when there is something a human should see now: uncovered actionable alerts, newly started remediation work, a related open PR that needs attention, a material worsening in risk, or a GitHub setup/auth blocker. State the total number of open Dependabot alerts with a critical/high/medium/low severity breakdown, the per-repository counts, related open PRs, and how many currently open alerts are covered by newly started remediation task(s), existing related PRs, or neither. Keep it manager-readable and do not paste raw GitHub CLI commands, \`gh api\` invocations, or command transcripts. If there are no actionable findings or blockers, do not post; end with a terse internal note instead.

Treat repository-level gaps such as Dependabot alerts being disabled for a repository, a repository returning zero open alerts, or a repository falling outside configured environment coverage as scan outcomes, not as GitHub setup/auth blockers and not as reasons to post on their own. If GitHub setup or alert access is blocked, post the same concise status message with the blocker and the counts that could be determined.`;
