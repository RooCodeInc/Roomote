 ...
 * task so the surface-generic worker tools target the destination conversation.
 * Slack keeps its channel normalization and membership checks, but must carry
 * the owning workspace so downstream token selection cannot choose another bot.
 */
