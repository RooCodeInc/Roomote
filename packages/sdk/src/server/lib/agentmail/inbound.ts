...
async function maybeSendStrangerRefusal(input: {
  ...
  try {
    const refusalText = STRANGER_REFUSAL_TEXT[input.reason];
...
export async function processAgentMailWebhookEvent(
  ...
  try {
    ...
    // and the webhook payload may omit the authentication verdicts; re-fetch
    // the full message before routing in either case.
    if (
      (!message.text && !message.extracted_text && !message.html) ||
      !hasAgentMailAuthenticationResults(message)
    ) {
