...
/**
 * Resolves the originating conversation for a cloud job across communication
 * providers. Slack payloads (including legacy field shapes) resolve through
 * the snapshot-resume source-job chain; Teams and Telegram resolve from the
 * provider-neutral communication payload fields, which are copied forward
 * onto resume-job payloads.
 */
export async function resolvePrReviewNotificationRoute(
  job: PrReviewNotificationRoutingJob,
  ...
  const { channel, threadTs } = await resolveSlackJobRouting(job);
