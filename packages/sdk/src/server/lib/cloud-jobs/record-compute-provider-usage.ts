...
interface ComputeProviderUsageCloudJobRecord extends ComputeProviderUsageCloudJob {
...
interface ProviderUsageContext {
  cloudJob: ComputeProviderUsageCloudJob;
...
interface LoadedComputeProviderUsageContext {
  cloudJob: ComputeProviderUsageCloudJobRecord;
...
async function loadComputeProviderUsageContext(
  ...
  if (!cloudJob) {
    throw new Error(
      `[recordComputeProviderUsage] Cloud job not found: ${input.cloudJobId}`,
    );
  }
  ...
  const providerUsageId = buildComputeProviderUsageId({
    ...
    instanceId: cloudJob.machineId ?? null,
  });

  const completedAt = input.completedAt ?? new Date();

  const startedAtSource: ComputeProviderUsageStartedAtSource =
    cloudJob.provisionStartedAt !== null
      ? 'provisionStartedAt'
  ...
  return {
    ...
    resources: resolveConfiguredResources({
      provider,
      configuredVcpus: cloudJob.configuredVcpus ?? null,
      configuredCpuCores: cloudJob.configuredCpuCores ?? null,
      configuredMemoryMiB: cloudJob.configuredMemoryMiB ?? null,
    }),
...
export async function recordComputeProviderUsage(
  ...
  if (
    !context.cloudJob.taskId ||
    !isFinalLifecycleAction(input.lifecycleAction)
  ) {
    return;
  }

  await recomputeTaskComputeUsageRollup(context.cloudJob.taskId);
}
