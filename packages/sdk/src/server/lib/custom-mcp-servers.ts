...
type UserAuthSuccess = { userId: string; isAdmin: boolean };

function captureIntegrationLifecycleEvent(
  event:
...
export async function createCustomMcpServerCommand(
  ...
  captureIntegrationLifecycleEvent(
    options.enabled === false ? 'integration_disabled' : 'integration_enabled',
    integrationId,
    auth.userId,
  );
  if (
    options.enabled !== false &&
    (input.transport === 'stdio' || input.authType !== 'oauth')
  ) {
    captureIntegrationLifecycleEvent(
      'integration_connected',
      integrationId,
...
export async function updateCustomMcpServerCommand(
  ...
  options: { prepareOnly?: boolean; expectedUpdatedAt?: string } = {},
) {
  assertAdmin(auth);
  assertCustomMcpEnabled();
  ...
  await db.transaction(async (tx) => {
      ...
      .set({
        ...
        ...(credentialTargetChanged
          ? {
              ...
              oauthServerMetadataFetchedAt: null,
              ...(options.prepareOnly
                ? {
                    headers: null,
                    manualClientId: null,
      ...
      .where(
        and(
          ...
          options.expectedUpdatedAt
            ? sql`${customMcpServers.updatedAt} = ${options.expectedUpdatedAt}::timestamp`
            : undefined,
        ),
      )
      .returning({ id: customMcpServers.id });
    ...
    if (!updated.length)
      throw new Error('Configuration changed; inspect and retry.');
...
export async function listCustomMcpServerToolsCommand(
  auth: UserAuthSuccess,
  input: { id: string },
  snapshot?: typeof customMcpServers.$inferSelect & { version?: string },
  ...
  if (
    ...
    throw new Error(
      'Authentication is required before testing this connection.',
    );
  }
  ...
  // OAuth lookup can await a refresh. Do not send credentials acquired after
  // a concurrent retarget to the endpoint from an older snapshot.
  const unchanged = await db.query.customMcpServers.findFirst({
    where: and(
      eq(customMcpServers.id, server.id),
      sql`${customMcpServers.updatedAt} = ${server.version ?? server.updatedAt.toISOString()}::timestamp`,
      eq(customMcpServers.url, server.url),
      eq(customMcpServers.authType, server.authType),
    ),
    columns: { id: true },
  });
  if (!unchanged) throw new Error('Configuration changed; inspect and retry.');
