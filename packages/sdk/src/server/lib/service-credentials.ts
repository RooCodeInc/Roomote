import {
  insertServiceCredentialApproval,
  insertUserIntegration,
  finalizeServiceCredential,
  listOwnedServiceCredentialApprovals,
...
export async function listServiceCredentialApprovals(
  context: ServiceCredentialContext,
) {
  try {
    return await listOwnedServiceCredentialApprovals(context);
  } catch {
    throw new Error(ERROR);
  }
}
...
export async function listServiceCredentials(
  ...
  try {
    return await listOwnedServiceCredentials(context);
  } catch {
    throw new Error(ERROR);
  }
}
...
export async function revokeServiceCredential(
  ...
  try {
    ...
    await revokeOwnedServiceCredential(context, secretRef);
  } catch {
    throw new Error(ERROR);
  }
}
