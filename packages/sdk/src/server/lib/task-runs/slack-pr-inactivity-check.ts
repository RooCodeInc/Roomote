...
export async function fetchPullRequestSnapshotForTaskRun({
  taskRun,
  repository,
  prNumber,
}: {
...
export async function enqueueSlackPrInactivityCheck(
  ...
  const taskRun = await db.query.taskRuns.findFirst({
    where: eq(taskRuns.id, parsedInput.runId),
    with: { task: true },
  });

  if (!taskRun) {
    return { enqueued: false, reason: 'task_run_not_found' };
  }
  ...
  const channel =
    taskRun.task.slackChannelId ?? getSlackChannelFromPayload(taskRun.payload);
  ...
  const baseline = await fetchPullRequestSnapshotForTaskRun({
    taskRun,
  ...
  const redis = getRedis();
  const dedupeKey = buildSlackPrInactivityTaskDedupKey(taskRun.taskId);
