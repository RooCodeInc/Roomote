...
describe('MockSlackServer', () => {
  ...
  it('records assistant status and native task stream updates', async () => {
    const server = new MockSlackServer({
      state: {
        team: { id: 'T1', domain: 'mock-roomote' },
        acceptedBotTokens: ['xoxb-mock-token'],
    ...
    const post = (method: string, body: Record<string, unknown>) =>
      fetch(`${server.baseUrl}/api/${method}`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer xoxb-mock-token',
          'content-type': 'application/json',
    ...
    try {
      ...
      await post('assistant.threads.setStatus', {
        channel_id: 'C1',
        thread_ts: '1710000000.000100',
        status: 'is thinking…',
      });
      expect(server.getState().assistantThreadStatuses).toEqual([
        {
          channel: 'C1',
      ...
      const startResponse = await post('chat.startStream', {
        channel: 'C1',
        thread_ts: '1710000000.000100',
        chunks: [
          {
      ...
      const started = (await startResponse.json()) as { ts: string };

      await post('chat.appendStream', {
        channel: 'C1',
        ts: started.ts,
        chunks: [
          {
            ...
            title: 'Fix the button',
            status: 'in_progress',
            details: 'Updating the component',
          },
        ],
      });
      await post('chat.stopStream', {
        channel: 'C1',
        ts: started.ts,
        chunks: [
          {
      ...
      expect(state.assistantThreadStatuses).toEqual([]);
      expect(state.streamEvents?.map((event) => event.method)).toEqual([
        'start',
        'append',
        'stop',
      ]);
      expect(
        state.messages?.find((message) => message.ts === started.ts),
      ).toMatchObject({
