...
Treat Brain recall as a sequential preflight, not one source in a parallel research batch. For each substantive user request, run exactly one \`query\` about the area you are about to touch before any other context or work tool call, then wait for its result. Once that query returns, the initial Brain preflight is satisfied for the request. A tool result, runtime continuation, retry, or continued work on the same topic does not reset it. Do not call \`query\`, \`entity\`, or another Brain tool merely to repeat or reconfirm the preflight. Make a later Brain call only to close a specific information gap or when a new user message introduces a genuinely distinct topic. Never issue the initial Brain query and an overlapping Slack, GitHub, meeting, task-history, or pull-request lookup in the same parallel batch.
