...
export const ciFailureTriageRulesSchema = z
  .object({
...
export type CiFailureTriageRules = z.infer<typeof ciFailureTriageRulesSchema>;

/** undefined preserves the shipped default; null is invalid and must fail closed. */
export function getCiFailureTriageRules(
  settings: Record<string, unknown> = {},
): CiFailureTriageRules | null | undefined {
  const text = settings.additionalRules;
  if (text === undefined || text === '') return undefined;
  if (typeof text !== 'string') return null;
  const parsed = ciFailureTriageRulesSchema.safeParse(settings.compiledRules);
  return parsed.success && parsed.data.text === text ? parsed.data : null;
}

export function isCiFailureTriageRepositoryAllowed(
  settings: Record<string, unknown> | undefined,
  repositoryId: string,
): boolean {
  const rules = getCiFailureTriageRules(settings);
  return (
    rules !== null &&
