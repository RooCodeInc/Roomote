 ...
 */
export const ENVIRONMENT_PREVIEW_REPAIR_CHANGE_REQUEST = `Live previews are configured for this environment, but the user reports the preview does not load or work correctly. Start the environment, validate each configured port's surface on localhost, and diagnose why it would fail behind the preview proxy. Check the common causes: dev servers that reject unknown hosts (allowed-hosts or host-header checks) or listen only on a loopback interface, hardcoded localhost or 127.0.0.1 origins in client code or API calls (the sandbox exposes the public preview origin for each port as \`ROOMOTE_<PORT_NAME>_HOST\`), CORS failures on cross-origin API requests, response headers that block framing (\`X-Frame-Options\`, \`Content-Security-Policy\` \`frame-ancestors\`), and websocket or HMR endpoints that bypass the proxy. Prefer environment-definition fixes (commands, env vars, port settings); make minimal application code changes only when the app itself blocks proxied or framed access. Keep every other environment setting unchanged.`;
