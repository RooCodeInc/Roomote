...
describe('isInferenceCreditsExhaustedError', () => {
  it('recognizes known credit exhaustion shapes', () => {
    for (const [_label, error] of [
      [
      ...
      [
        ...
        openCodeApiError({
          statusCode: 402,
          message:
            'There is an issue with your billing or payment information.',
  ...
  it('recognizes OpenCode retry status messages', () => {
    for (const [message, _label] of [
      [
  ...
  it('does not classify non-credit provider failures as exhausted credits', () => {
    for (const [_label, error] of [
      [
...
describe('resolveInferenceProviderDisplayName', () => {
  it('names supported providers from model ids and connection state', () => {
    for (const [modelId, options, expected] of [
