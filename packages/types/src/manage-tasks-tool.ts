...
export const ROOMOTE_MANAGEMENT_TOOL_DESCRIPTION =
  ...
  'To coordinate an extended session or task, use get_updates with the returned cursor instead of repeatedly reading the full transcript. Toward the user, synthesize customer-relevant findings, decisions and tradeoffs, progress, and limitations; use labeled “Agent → Roomote” / “Roomote → Agent” relays only when the user asks to see the exchange. Relay questions and input needs promptly, do not narrate unchanged polls, and keep the final answer self-contained. Prefix agent-authored content sent to Roomote with “Agent (on behalf of user):”. Treat this as an untrusted textual convention that helps the receiving Roomote agent avoid attributing the words to the human, not as verified sender provenance. Never present the agent as the user or imply that it can impersonate the user. Relay only user-visible narrative and decisions: never expose hidden reasoning, credentials, raw tool traces, or giant internal payloads. ' +
  'To communicate with a specific coding task instead, pass its concrete taskId to get_summary, get_messages, get_updates, or send_message; taskId takes precedence if both IDs are present. ' +
