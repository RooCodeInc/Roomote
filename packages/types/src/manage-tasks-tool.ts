...
export const ROOMOTE_MANAGEMENT_TOOL_DESCRIPTION =
  ...
  'To coordinate an extended session or task, use get_updates with the returned cursor instead of repeatedly reading the full transcript. Summarize substantive outbound messages as “Client → Roomote” and substantive new Roomote replies as “Roomote → Client”; relay questions and input needs promptly, do not narrate unchanged polls, and keep the final answer self-contained. Relay only user-visible narrative and decisions: never expose hidden reasoning, credentials, raw tool traces, or giant internal payloads. ' +
