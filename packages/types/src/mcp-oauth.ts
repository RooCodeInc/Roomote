...
export const MCP_INTEGRATIONS: McpIntegration[] = [
  ...
  },
  {
    ...
    instructions: [
      ...
      '',
      'Save sparingly: treat saving a memory as a rare, high-bar action. Save only durable, reusable, cross-task knowledge:',
      '- explicit user or team preferences (for example "always open draft PRs")',
      '- deployment-wide conventions or decisions with rationale that future tasks must respect',
      '- recurring gotchas that cost real effort and will recur in later tasks',
      '',
  ...
  },
  {
    ...
    description: `Connect your Zero wallet so ${PRODUCT_NAME} agents can discover and pay for external capabilities from tasks`,
    icon: 'zero',
    ...
    instructions: [
      'Zero Path C is available because an admin enabled Zero for this deployment: the `zero` CLI and skill are installed for this task, and the Zero MCP connector handles authentication and funding.',
      '',
      ...
      '',
      'Capability usage belongs to the connected user wallet. Search is free; paid fetches spend from that wallet. Set `--max-pay` on unfamiliar paid calls.',
      '',
